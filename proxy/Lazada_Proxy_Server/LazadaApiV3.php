...
class LazadaApiV3{
	...
	static $API_URL;
	static $API_KEY;//token
	static $APP_KEY;
	static $APP_SECRET;
	static $MARKETPLACE;
	
	static $LazadaSite = ['co.id','my','ph','sg','co.th','vn','id','th'];
	// 部分站点未开通国际业务，所以不需对接。 lazada貌似越南没有开， Linio有5个，听说jumia只有一个
	static $Lazada_API_URL_CONFIG = array(
		// lazada站点
        'co.id'	=>"api.lazada.co.id/rest",// 印尼
	    'id'	=>"api.lazada.co.id/rest",// 印尼，新国家代码
        'my'	=>"api.lazada.com.my/rest",// 马来西亚
        'ph'	=>"api.lazada.com.ph/rest",// 菲律宾
        'sg'	=>"api.lazada.sg/rest",// 新加坡
        'co.th'	=>"api.lazada.co.th/rest",// 泰国
	    'th'	=>"api.lazada.co.th/rest",// 泰国，新国家代码
        'vn'	=>"api.lazada.vn/rest",// 越南	        
	...
	static $LAZADA_SITE_TIMEZONE =array(
			// lazada站点
			'co.id'	=>"Etc/GMT-7",// 印尼
	        'id'	=>"Etc/GMT-7",// 印尼,新接口国家代码
			'my'	=>"Etc/GMT-8",// 马来西亚
			'ph'	=>"Etc/GMT-8",// 菲律宾
			'sg'	=>"Etc/GMT-8",// 新加坡
			'co.th'	=>"Etc/GMT-7",// 泰国
	        'th'	=>"Etc/GMT-7",// 泰国,新接口国家代码
			'vn'	=>"Etc/GMT-7",// 越南
			// linio站点
			'ar'=>"Etc/GMT+3",
			'cl'=>"Etc/GMT+3",// 智利
	...
	static $LAZADA_API_NO_TOKEN = array(
	    '/auth/token/create',
	    '/auth/token/refresh',
	    '/brands/get',
	);//获取token 和刷新token不用access
	static $LAZADA_GLOBAL_ERRORS = array( 
			...
			'IllegalAccessToken',// dzt20190531 新加授权失败 退出重试
	);
	
	private static function _callLazada($getParameters=array(),$postParameters=false,$auth=true){
...
// 		$url=self::$API_URL."/?";
		$url=self::$API_URL.$getParameters['Action']."?";//以前的action，改为/brand/get之类的接口
		
		if(!in_array($getParameters['Action'], self::$LAZADA_API_NO_TOKEN)){
		    $getParameters['access_token'] = self::$API_KEY;
		}
...
// 		write_log("return from lazada:".print_r($data,true),"info");
		...
		if ($httpCode == '200' ){
			...
			// write_log("test response ".json_encode($rtn['response']));
			$rtn['state_code'] = 200;
	...
	public static function setConfig($userId,$countryCode,$appKey,$appSecret,$apiKey){
...
// 		self::$API_URL=self::API_URL_PREFIX.$countryCode;
	...
	public static function getLazadaOrderList($param){
		...
		try{
			$reqParams["Action"] = '/orders/get';
			$reqParams["timestamp"] = round($current_time[0]*1000+$current_time[1]*1000);
		
			// 处理api 接口参数
			// 获取订单List 以前Update Time 作滑动窗口 ，所以fromDateTime 参数是必须的。
			if(!isset($param["created_after"]) && !isset($param["update_after"])){
				return array("success"=>false, "message"=>"created_after or update_after is needed to filter orders.", "body"=>"");
			}
...
// 			Limits the returned order list to those created after or on a specified date, given in ISO 8601 date format. Either CreatedAfter or UpdatedAfter are mandatory or else an error 'E018: Either CreatedAfter or UpdatedAfter is mandatory' wil be returned.
			if(isset($param["created_after"])){
				$CreatedAfterDateTime = new DateTime($param["created_after"]);// proxy接收到的date time string 都必须是已经转为utc 时间的
// 				self::setRequestDateTimeTimezone($CreatedAfterDateTime);// initial api site timezone
				$reqParams["created_after"] = $CreatedAfterDateTime->format(DateTime::ISO8601);
				
// 				CreatedBefore:	DateTime
				if(isset($param["created_before"])){
					$CreatedBeforeDateTime = new DateTime($param["created_before"]);// proxy接收到的date time string 都必须是已经转为utc 时间的
// 					self::setRequestDateTimeTimezone($CreatedBeforeDateTime);// initial api site timezone
					$reqParams["created_before"] = $CreatedBeforeDateTime->format(DateTime::ISO8601);
				}
			}else if(isset($param["update_after"])){
// 				UpdatedAfter:	DateTime
				$UpdatedAfterDateTime = new DateTime($param["update_after"]);// proxy接收到的date time string 都必须是已经转为utc 时间的
// 				self::setRequestDateTimeTimezone($UpdatedAfterDateTime);// initial api site timezone
				$reqParams["update_after"] = $UpdatedAfterDateTime->format(DateTime::ISO8601);
				
// 				UpdatedBefore:	DateTime
				if(isset($param["update_before"])){
					$UpdatedBeforeDateTime = new DateTime($param["update_before"]);// proxy接收到的date time string 都必须是已经转为utc 时间的
// 					self::setRequestDateTimeTimezone($UpdatedBeforeDateTime);// initial api site timezone
					$reqParams["update_before"] = $UpdatedBeforeDateTime->format(DateTime::ISO8601);
				}
			}			
...
// 				$reqParams["Limit"] = $pageSize;
				...
				write_log("ST001:Get order list. (getLazadaOrderListV2) params:".json_encode($reqParams), "info");
				$response = self::_callLazada($reqParams);
				$timeMS12=getCurrentTimestampMS();
				
				write_log("t12_11 memory usage:".round(memory_get_usage()/1024/1024)."M. (getLazadaOrderListV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"info");
				// network error or json decode error
				if($response["success"] == false){
					$errorCount++;
					$results['message'] = $response["message"];
					write_log("ST001 : ".$response["message"]." . (getLazadaOrderListV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
					sleep(1);
					continue;
				}
				...
				if($response["response"]["code"] != '0'){
				    $errorMessage = "RequestAction:".$reqParams["Action"].",ErrorType ".$response["response"]["type"].",ErrorCode:".$response["response"]["code"].",ErrorMessage:".$response["response"]["message"];
					...
					if (isset($response["response"]["code"]) and in_array($response["response"]["code"],$skipRetryErrorCodeArr)) {
						$results['success'] = false;
						$results['message'] = $errorMessage;
						break;
					}
					write_log("ST001 : $errorMessage . (getLazadaOrderListV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
					sleep(1);
					continue;
				}
...
// 				$totalCount = $response["response"]["SuccessResponse"]["Head"]["TotalCount"];
				...
				$totalCount = $response["response"]['data']['count'];
				
				// dzt20161116 lazada貌似改变了返回结构["Body"]["Orders"]["Order"] 变成["Body"]["Orders"]
				if (isset($successResponse["orders"])){// dzt20161116 现在返回尽管只有一个产品也是数组的
				    if(!empty($successResponse["orders"]))
				        $orderList = array_merge($orderList,$successResponse["orders"]);
				}else{
				    $errorCount++;
					$results['message'] = "V2:Reture data format has changed,cannot get order info.";
					write_log("ST001 : ".$results["message"]." . (getLazadaOrderListV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
					sleep(1);
					continue;
				}
...
// 			}while (($totalCount == -1 || count($orderList) < $totalCount) && $errorCount < 15);
			...
			if($results['success']){
				...
				// 转换时间,新接口格式变为正式格式 2018-02-03 12:25:09 +0800，不需要转换 
// 				$responseTime = new DateTime($successResponse['Head']['Timestamp']);
// 				foreach ($orderList as &$oneOrder){
// 					if(!empty($oneOrder['created_at'])){
// 						$createAtTime = new DateTime($oneOrder['created_at'] , $responseTime->getTimezone());
// 						$oneOrder['created_at'] = $createAtTime->format(DateTime::ISO8601);
// 					}
						
// 					if(!empty($oneOrder['updated_at'])){
// 						$UpdatedAtTime = new DateTime($oneOrder['updated_at'] , $responseTime->getTimezone());
// 						$oneOrder['updated_at'] = $UpdatedAtTime->format(DateTime::ISO8601);
// 					}
					
// 					if(!empty($oneOrder['promised_shipping_times'])){
// 						$PromisedShippingTimes = new DateTime($oneOrder['promised_shipping_times'] , $responseTime->getTimezone());
// 						$oneOrder['promised_shipping_times'] = $PromisedShippingTimes->format(DateTime::ISO8601);
// 					}
					
// 					if(!empty($oneOrder['address_updated_at'])){
// 						$AddressUpdatedAtTime = new DateTime($oneOrder['address_updated_at'] , $responseTime->getTimezone());
// 						$oneOrder['address_updated_at'] = $AddressUpdatedAtTime->format(DateTime::ISO8601);
// 					}
// 				}
			}
			if(!empty($orderList)){//防止订单号过长，所以转换成字符串
			    foreach ($orderList as $key =>$val){
			        if(!empty($orderList[$key]['order_number'])){
			            $orderList[$key]['order_number'] = (string)$orderList[$key]['order_number'];
			        }
			        
			        if(!empty($orderList[$key]['order_id'])){
			            $orderList[$key]['order_id'] = (string)$orderList[$key]['order_id'];
			        }
			    }
			}
			$timeMS2=getCurrentTimestampMS();
			write_log("getLazadaOrderListV2 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE.",TotalCount:$totalCount ,t2_1=".($timeMS2-$timeMS1),"info");
...
// 	        The order identifier that was assigned to the order by SellerCenter.
	        if(!isset($param["order_id"])){
	            return array("success"=>false, "message"=>"'order_id' is needed to get the order info.", "body"=>"");
	        }
	         
	        $reqParams['order_id'] = $param["order_id"];
	         
	        write_log("ST001:Start to get order info. (getLazadaOneOrderV2) params:".print_r($reqParams,true),"info");
	        do{
	            $timeMS1=getCurrentTimestampMS();
	            $response = self::_callLazada($reqParams);
	            // network error or json decode error
	            if($response["success"] == false){
	                $results['message'] = $response["message"];
	                $errorCount++;
	                write_log("ST001 : ".$response["message"]." . - ".$param["order_id"]." . (getLazadaOneOrderV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	                sleep(1);
	                continue;
	            }
	            ...
	            if($response["response"]["code"] != '0'){
	                ...
	                $results['message'] = $errorMessage;
	                $errorCount++;
	                write_log("ST001 : $errorMessage . - ".$param["order_id"]." . (getLazadaOneOrderV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
...
// 	            $successResponse = $response["response"]["SuccessResponse"];
	            ...
	            if (isset($response["response"]["data"])){// dzt20161116 现在返回尽管只有一个产品也是数组的
	                if(!empty($response["response"]["data"]))
	                    $order = $response["response"]["data"];
	            }else{
	                $errorCount++;
	                $results['message'] = "Reture V2 data format has changed,cannot get order info.";
	                write_log("ST001 : ".$results["message"]." . - ".$param["order_id"]." . (getLazadaOneOrderV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	                sleep(1);
	                continue;
	            }
...
// 			$results['apiCallTimes'] = self::$API_CALL_TIMES;
	        ...
	        $timeMS2=getCurrentTimestampMS();
	        write_log("getLazadaOneOrderV2 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." t2_1=".($timeMS2-$timeMS1).",result:".print_r($results,true),"info");
	...
	public function getLazadaOrderItems($param){
		...
		$current_time = explode(" ",microtime());
		try{
			$reqParams["Action"] = '/orders/items/get';
			$reqParams["timestamp"] = round($current_time[0]*1000+$current_time[1]*1000);
// 			$Timestamp = new DateTime();
// 			$Timestamp->setTimezone ( new DateTimeZone('UTC'));
// 			$reqParams["Timestamp"]=$Timestamp->format(DateTime::ISO8601);
// 			$reqParams = array_merge($reqParams,self::$CONFIG);
			...
			$reqParams['order_ids'] = json_encode(explode(',',$param["OrderId"]));
			
			write_log("ST001:Start to get order items:OrderIdList(".$reqParams['order_ids']."). (getLazadaOrderItemsV2) ","info");
			...
			do{
				...
				if($response["success"] == false){
					$errorCount++;
					$results['message'] = $response["message"];
					write_log("ST001 : ".$response["message"]." . - ".$param["OrderId"]." . (getLazadaOrderItemsV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
					sleep(1);
					continue;
				}
				...
				if($response["response"]["code"] != '0'){
				    ...
				    $results['message'] = $errorMessage;
					$errorCount++;
					write_log("ST001 : $errorMessage . - ".$param["OrderId"]." . (getLazadaOrderItemsV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
...
// 				$successResponse = $response["response"]["SuccessResponse"];
				...
				if (isset($response["response"]['data'])){
				    if(!empty($response["response"]['data']))
				        $orders = array_merge($orders,$response["response"]['data']);
				}else{
				    $errorCount++;
					$results['message'] = "Reture data format has changed,cannot get order info.";
					write_log("ST001 : ".$results["message"]." . (getLazadaOrderItemsV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
					sleep(1);
					continue;
				}
				
				foreach ($orders as $oneOrder){
					if(isset($oneOrder["order_number"])){// 订单只有一个item 与多个items是结构不一样
...
// 					    $items[$oneOrder["order_id"]] = $oneOrder["order_items"];
                        foreach ($oneOrder["order_items"] as $key =>$val){
                            if(isset($val['order_item_id'])){//防止item_id过长，所以转换成字符串
                                $oneOrder["order_items"][$key]['order_item_id'] = (string)$oneOrder["order_items"][$key]['order_item_id'];
                            }
                            if(isset($val['order_id'])){//防止item_id过长，所以转换成字符串
                                $oneOrder["order_items"][$key]['order_id'] = (string)$oneOrder["order_items"][$key]['order_id'];
                            }
                        }
					    
					    $items[$oneOrder["order_number"]] = $oneOrder["order_items"];
					}
...
//     				}
			...
			// ST002 get product photos 新版有图片和产品链接，所以暂时屏蔽
// 			write_log("ST002:get product photos. - ".$param["OrderId"]."(getLazadaOrderItemsV2)", "info");
...
// 						list($ret,$smallImageUrl,$productUrl)= self::_getLazadaProductMainImage($orderItem['shop_sku'],$orderItem['sku'],$db);
// 						if ($ret===false){
// 							$orderItem["SmallImageUrl"]="";
// 							write_log("Sku: V2".$orderItem["sku"]." marketplaceId:".self::$MARKETPLACE." error_message:".$smallImageUrl, "info");
// 							continue;
// 						}
// 						write_log("Sku: V2".$orderItem["sku"]." image:".$smallImageUrl, "info");
// 						write_log("smallImageUrl V2:".print_r( $smallImageUrl,true), "info");
// 						$orderItem["SmallImageUrl"] = $smallImageUrl;
// 						$orderItem["ProductUrl"] = $productUrl;
						
// 						if(!empty($orderItem['created_at'])){
// 							$createAtTime = new DateTime($orderItem['created_at'] , $responseTime->getTimezone());
// 							$orderItem['created_at'] = $createAtTime->format(DateTime::ISO8601);
// 						}
// 						if(!empty($orderItem['updated_at'])){
// 							$UpdatedAtTime = new DateTime($orderItem['updated_at'] , $responseTime->getTimezone());
// 							$orderItem['updated_at'] = $UpdatedAtTime->format(DateTime::ISO8601);
// 						}
// 					}
// 				}
// 			}
			...
			write_log("getLazadaOrderItemsV2 Done - ".$param["OrderId"]." t2_1=".($timeMS2-$timeMS1)."t3_2=".($timeMS3-$timeMS2)."t3_1=".($timeMS3-$timeMS1),"info");
			$results['errorCount'] = $errorCount;
			$results['state_code'] = $response['state_code'];
// 			$results['apiCallTimes'] = self::$API_CALL_TIMES;
...
// 			}
	...
	public function getLazadaOrderItemImage($param){
		...
		if ($ret===false){
			write_log("getLazadaOrderItemImage SellerSku:".$param["SellerSku"].",ShopSku:".$param["ShopSku"].",marketplaceId:".self::$MARKETPLACE.",error_message:".$smallImageUrl, "info");
			$rtn['message'] = "SellerSku:".$param["SellerSku"].",marketplaceId:".self::$MARKETPLACE.",error_message:".$smallImageUrl;
		}else{
			write_log("getLazadaOrderItemImage SellerSku:".$param["SellerSku"].",ShopSku:".$param["ShopSku"].",marketplaceId:".self::$MARKETPLACE.",purge:".$param['purge'].",image:".$smallImageUrl, "info");
			$rtn['success'] = true;
...
// 	        $reqParams['order_item_ids'] = json_encode(explode(',',$param["OrderItemIds"]));
	        $feed['order_item_ids'] = json_encode(explode(',',$param["OrderItemIds"]));
...
// 	        $reqParams['delivery_type'] = $param['DeliveryType'];
	        $feed['delivery_type'] = $param['DeliveryType'];
...
// 	        $reqParams['shipping_provider'] = $param['ShippingProvider'];
	        $feed['shipping_provider'] = $param['ShippingProvider'];
	        
	        write_log("ST001:Start to set packedByMarketplace. (packedByMarketplaceV2) params:".print_r($reqParams,true),"info");
	        do{
	            $timeMS1=getCurrentTimestampMS();
// 	            $response = self::_callLazada($reqParams);
	            ...
	            if($response["success"] == false){
	                $results['message'] = $response["message"];
	                $errorCount++;
	                write_log("ST001 : ".$response["message"]." . - ".$param["OrderItemIds"]." . (packedByMarketplaceV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	                sleep(1);
	                continue;
	            }
                ...
                if($response["response"]["code"] != '0'){
                    ...
                    $results['message'] = $errorMessage;
	                $errorCount++;
	                write_log("ST001 : $errorMessage . - ".$param["OrderItemIds"]." . (packedByMarketplacev2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	            ...
	            if(!empty($successResponse["order_items"])){
	                $items = $successResponse["order_items"];
	            }
...
// 			$results['apiCallTimes'] = self::$API_CALL_TIMES;
            ...
            $timeMS2=getCurrentTimestampMS();
            write_log("packedByMarketplace V2 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." t2_1=".($timeMS2-$timeMS1).",result:".print_r($results,true),"info");
...
// 			'TrackingNumber'=>'LN241716246SG',
			...
			// E029: Order items must be from the same order 新接口，为改动最少，只改api接口字段
			$feed['order_item_ids'] = json_encode(explode(',',$param["OrderItemIds"]));// 订单下的多个 OrderItemIds。格式为： [OrderItemIds1,OrderItemIds2]的字符串
// 			$reqParams['order_item_ids'] = json_encode(explode(',',$param["OrderItemIds"]));
...
// 			DeliveryType:One of the following: 'dropship' - The seller will send out the package on his own; 'pickup' - Shop should pick up the item from the seller (cross-docking); 'send_to_warehouse' - The seller will send the item to the warehouse (crossdocking).
			$feed['delivery_type'] = $param['DeliveryType'];
			$feed['shipment_provider'] = $param['ShippingProvider'];
			
// 			$reqParams['delivery_type'] = $param['DeliveryType'];
// 			$reqParams['shipment_provider'] = $param['ShippingProvider'];
			...
			if(!empty($param['TrackingNumber']))
			    $feed['tracking_number'] = $param['TrackingNumber'];
			
// 			if(!empty($param['TrackingNumber']))
// 			    $reqParams['tracking_number'] = $param['TrackingNumber'];
			
			write_log("ST001:V2 Start to ship order . (shipLazadaOrderV2) params:".print_r($reqParams,true),"info");
			do{
				$timeMS1=getCurrentTimestampMS();
// 				$response = self::_callLazada($reqParams);
				$response = self::_callLazada($reqParams,$feed,true);
				...
				if($response["success"] == false){
					$results['message'] = $response["message"];
					$errorCount++;
					write_log("ST001 : ".$response["message"]." . - ".$param["OrderItemIds"]." . (shipLazadaOrderV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
					sleep(1);
					continue;
				}
			    ...
			    if($response["response"]["code"] != '0'){
			        ...
			        $results['message'] = $errorMessage;
					$errorCount++;
					write_log("ST001 : $errorMessage . - ".$param["OrderItemIds"]." . (shipLazadaOrderV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
...
// 			$results['apiCallTimes'] = self::$API_CALL_TIMES;
	...
	public static function getDocument($param){
		...
		$current_time = explode(" ",microtime());
		try{
			$reqParams["Action"] = '/order/document/get';
			$reqParams["timestamp"] = round($current_time[0]*1000+$current_time[1]*1000);
// 			$Timestamp = new DateTime();
// 			$Timestamp->setTimezone ( new DateTimeZone('UTC'));
// 			$reqParams["timestamp"]=$Timestamp->format(DateTime::ISO8601);
// 			$reqParams = array_merge($reqParams,self::$CONFIG);
			...
			$reqParams['doc_type'] = $param['DocumentType'];
			...
			$reqParams['order_item_ids'] = json_encode(explode(',',$param["OrderItemIds"]));// 订单下的多个 OrderItemIds。格式为： [OrderItemIds1,OrderItemIds2]的字符串
				
			write_log("ST001:Start to getDocument. (getDocumentV2) params:".json_encode($reqParams),"info");
			do{
				$timeMS1=getCurrentTimestampMS();
				$response = self::_callLazada($reqParams);
			    ...
			    if($response["response"]["code"] != '0'){
			        ...
			        $results['message'] = $errorMessage;
					$errorCount++;
					write_log("ST001 : $errorMessage . - ".$param["OrderItemIds"]." . (getDocumentV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
			...
			$timeMS2=getCurrentTimestampMS();
			write_log("getDocumentV2 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." t2_1=".($timeMS2-$timeMS1),"info");
			$results['errorCount'] = $errorCount;
			$results['state_code'] = $response['state_code'];
// 			$results['apiCallTimes'] = self::$API_CALL_TIMES;
			return $results;
...
// 			$results['apiCallTimes'] = self::$API_CALL_TIMES;
	...
	public static function getPayoutStatus($param){
	    ...
	    $current_time = explode(" ",microtime());
	    try{
	        $reqParams["Action"] = '/finance/payout/status/get';
	        $reqParams["timestamp"] = round($current_time[0]*1000+$current_time[1]*1000);
// 	        $Timestamp = new DateTime();
// 	        $Timestamp->setTimezone ( new DateTimeZone('UTC'));
// 	        $reqParams["Timestamp"]=$Timestamp->format(DateTime::ISO8601);
...
// 				self::setRequestDateTimeTimezone($CreatedAfterDateTime);// initial api site timezone
	        ...
	        write_log("ST001:Start to getPayoutStatus. (getPayoutStatusV2) params:".json_encode($reqParams),"info");
	        do{
	            $timeMS1=getCurrentTimestampMS();
	            $response = self::_callLazada($reqParams);
                ...
                if($response["response"]["code"] != '0'){
                    ...
                    $results['message'] = $errorMessage;
	                $errorCount++;
	                write_log("ST001 : $errorMessage . (getPayoutStatusv2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	        ...
	        $timeMS2=getCurrentTimestampMS();
	        write_log("getPayoutStatusV2 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." t2_1=".($timeMS2-$timeMS1),"info");
	        $results['errorCount'] = $errorCount;
	        $results['state_code'] = $response['state_code'];
	        // 			$results['apiCallTimes'] = self::$API_CALL_TIMES;
	        return $results;
...
// 				$reqParams['CreatedAfter'] = $param["CreatedAfter"];
	            $CreatedAfterDateTime = new DateTime($param["CreatedAfter"]);// proxy接收到的date time string 都必须是已经转为utc 时间的
// 	            self::setRequestDateTimeTimezone($CreatedAfterDateTime);// initial api site timezone
	            $reqParams["create_after"] = $CreatedAfterDateTime->format(DateTime::ISO8601);
	        }
...
// 	            self::setRequestDateTimeTimezone($CreatedBeforeDateTime);// initial api site timezone
	            $reqParams["create_before"] = $CreatedBeforeDateTime->format(DateTime::ISO8601);
	        }
...
// 	            self::setRequestDateTimeTimezone($CreatedAfterDateTime);// initial api site timezone
	            $reqParams["update_after"] = $CreatedAfterDateTime->format(DateTime::ISO8601);
	        }
...
// 	            self::setRequestDateTimeTimezone($CreatedBeforeDateTime);// initial api site timezone
	            $reqParams["update_before"] = $CreatedBeforeDateTime->format(DateTime::ISO8601);
	        }
...
// 	        Only products which have the SellerSku in this list wil be returned. Input should be a JSON array. e.g., ["Apple 6s Gold","Apple 6s Black"]. It only match the Whole words.
	        if(isset($param["SkuSellerList"])){
	            $reqParams['sku_seller_list'] = json_encode(explode(',',$param["SkuSellerList"]));
	        }
	        	
	        write_log("ST001:Start to get Products (getLazadaProductsV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE.",reqParams:".json_encode($reqParams),"info");
	        do{
	            $timeMS1=getCurrentTimestampMS();
	            $response = self::_callLazada($reqParams);
	            ...
	            if($response["success"] == false){
	                $results['message'] = $response["message"];
	                $errorCount++;
	                write_log("ST001 : ".$response["message"]. ". (getLazadaProductsV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	                sleep(1);
	                continue;
	            }
                ...
                if($response["response"]["code"] != '0'){
                    ...
                    $results['message'] = $errorMessage;
	                $errorCount++;
	                write_log("ST001 : $errorMessage . (getLazadaProductsV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	            ...
	            $successResponse = $response["response"]["data"];
// 				$results['response'] = $successResponse;
	            if(!empty($successResponse)){//为空就是没有产品
	                if(isset($successResponse["products"])){// 现在返回尽管只有一个产品也是数组的
	                    if(!empty($successResponse["products"]))
	                        $products = $successResponse["products"];
	                }else{
	                    $errorCount++;
	                    $results['message'] = "Reture data format has changed,cannot get Products info.";
	                    write_log("ST001 : ".$results["message"]." . (getLazadaProductsV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	                    write_log("Products info change error: ".json_encode($response)." . (getLazadaProductsV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	                    sleep(1);
	                    continue;
	                }
	            }
	            ...
	            write_log("count:get products:".count($products).",memory usage:".round(memory_get_usage()/1024/1024,2)."M. (getLazadaProductsV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"info");
	            $results['success'] = true;
	            break;
	        }while ($errorCount < 15);
...
// 	                        }
	        ...
	        $timeMS2=getCurrentTimestampMS();
	        write_log("getLazadaProductsV2 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." t2_1=".($timeMS2-$timeMS1),"info");
...
// 				$maxCount = 2;
			...
			foreach ($param["filterStatus"] as $status){// 由于控制了只能获取固定页数，所以其他状态需要过滤 all拿下的sku,所以eagle 传过来最好安排all 在最前
				$counter = 0;
				write_log("$status _1 memory usage:".(memory_get_usage()/1024/1024)."M. (getLazadaFilterProductsV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"info");
...
// 					write_log("$status _2 memory usage:".(memory_get_usage()/1024/1024)."M. (getLazadaFilterProducts) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"info");
					...
					write_log("getLazadaFilterProductsV2 t12_11=".($timeMS12-$timeMS11) ." status:$status Offset:".$reqParams['offset'].",Limit:".$reqParams['limit'] ,"info");
					// network error or json decode error
					if($response["success"] == false){
						$results['message'] = $response["message"]." status:$status";
						$errorCount++;
						write_log("ST001 : ".$response["message"]. ". (getLazadaFilterProductsV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
						sleep(1);
						continue;
					}
					...
					if($response["response"]["code"] != '0'){
					    ...
					    $results['message'] = $errorMessage;
						$errorCount++;
						write_log("ST001 : $errorMessage . (getLazadaFilterProductsV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
...
// 					$results['response'] = $successResponse;
					...
					if(!empty($successResponse)){//为空就是没有产品
					    if (isset($successResponse["products"])){// dzt20161116 现在返回尽管只有一个产品也是数组的
					        if(!empty($successResponse["products"]))
					            $filterProducts = $successResponse["products"];
					    }else{
					        $errorCount++;
					        $results['message'] = "Reture data format has changed,cannot get Products info.";
					        write_log("ST001 : ".$results["message"]." . (getLazadaProductsV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
...
// 					write_log("$status _4 memory usage:".(memory_get_usage()/1024/1024)."M. (getLazadaFilterProducts) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"info");
				...
				if($errorCount == 15){
					$results['success'] = false;
					write_log("getLazadaFilterProductsv2 over errorCount. status:$status UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
				}
			}// end of $param["filterStatus"] foreach
			
			$timeMS2 = getCurrentTimestampMS();
			write_log("getLazadaFilterProductsV2 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." t2_1=".($timeMS2-$timeMS1),"info");
...
// 			$results['apiCallTimes'] = self::$API_CALL_TIMES;
	...
	public static function getLazadaQcStatus($param){
	    ...
	    $current_time = explode(" ",microtime());
	    try{
	        $reqParams["Action"] = '/product/qc/status/get';
	        $reqParams["timestamp"] = round($current_time[0]*1000+$current_time[1]*1000);
// 	        $Timestamp = new DateTime();
// 	        $Timestamp->setTimezone ( new DateTimeZone('UTC'));
// 	        $reqParams["Timestamp"]=$Timestamp->format(DateTime::ISO8601);
...
// 	            $reqParams['SkuSellerList'] = json_encode($param["SkuSellerList"]);
	        ...
	        for ($i = 0; $i < $length; $i = $i + $step) {
	            ...
	            $reqParams['seller_skus'] = json_encode($skusSlice);
	            
	            do{
	                $timeMS11 = getCurrentTimestampMS();
	                $response = self::_callLazada($reqParams);
	                $timeMS12 = getCurrentTimestampMS();
	                write_log("ST002 after get GetQcStatusV2 Index:$i,step:$step t12_11=".($timeMS12-$timeMS11) ." (getLazadaQcStatus) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"info");
                    ...
                    if($response["response"]["code"] != '0'){
                        ...
                        $results['message'] = $errorMessage;
	                    $errorCount++;
	                    write_log("ST001 : $errorMessage . (getLazadaQcStatusV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	                ...
	                $successResponse = $response["response"]["data"];
	                // 	            $results['response'] = $successResponse;
	                if (!empty($successResponse)){
                        $qcStatus = array_merge($qcStatus,$successResponse);
	                }else{
	                    $errorCount++;
	                    $results['message'] = "Reture data format has changed,cannot get LazadaQcStatus info.";
	                    write_log("ST001 : ".$results["message"]." . (getLazadaQcStatusV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	                    sleep(1);
	                    continue;
	                }
	        ...
	        $timeMS2=getCurrentTimestampMS();
	        write_log("getLazadaQcStatusV2 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." t2_1=".($timeMS2-$timeMS1),"info");
...
// 		        $reqParams["Action"] = 'CreateProduct';
		        $reqParams["Action"] = '/product/create';
		    }else{
...
// 				$response = self::_callLazada($reqParams,$feed,true);
				$response = self::_callLazada($reqParams,array('payload'=>$feed),true);
				...
				if($response["response"]["code"] != '0'){
					...
					$errorCount++;
					write_log("ST003 : $errorMessage . (lazadaProductCreateV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
					$results['message'] = $errorMessage;
			...
			$timeMS2=getCurrentTimestampMS();
			write_log("lazadaProductCreateV2 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." t2_1=".($timeMS2-$timeMS1),"info");
...
// 		        $reqParams["Action"] = 'UpdateProduct';
		        $reqParams["Action"] = '/product/update';
		    }else{
...
EOD;
				...
				write_log("ST002:before call _callLazada. (lazadaProductUpdateV2) param:".print_r($reqParams,true),"info");
				write_log("lazadaProductUpdate feed:".$feed,"info");
// 				$response = self::_callLazada($reqParams,$feed,true);
				$response = self::_callLazada($reqParams,array('payload'=>$feed),true);
				write_log("ST002.1: (lazadaProductUpdateV2) response:".print_r($response,true),"info");
				// network error or json decode error
				if($response["success"] == false){
					$results['message'] = $response["message"];
					$errorCount++;
					write_log("ST003 : ".$response["message"]." . (lazadaProductUpdateV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
					sleep(1);
					continue;
				}
...
// 					}
					...
					$errorCount++;
					write_log("ST003 : $errorMessage . (lazadaProductUpdateV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
					$results['message'] = $errorMessage;
			...
			$timeMS2=getCurrentTimestampMS();
			write_log("lazadaProductUpdateV2 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." t2_1=".($timeMS2-$timeMS1),"info");
...
// 	            $reqParams["Action"] = 'UpdatePriceQuantity';
	            $reqParams["Action"] = '/product/price_quantity/update';
	        }else{
...
EOD;
	            ...
	            write_log("ST002:before call _callLazada. (UpdatePriceQuantityV2) param:".print_r($reqParams,true),"info");
	            write_log("UpdatePriceQuantityV2 feed:".$feed,"info");
// 	            $response = self::_callLazada($reqParams,$feed,true);
	            $response = self::_callLazada($reqParams,array('payload'=>$feed),true);
	            write_log("ST002.1: (UpdatePriceQuantityV2) response:".print_r($response,true),"info");
	            // network error or json decode error
	            if($response["success"] == false){
	                $results['message'] = $response["message"];
	                $errorCount++;
	                write_log("ST003 : ".$response["message"]." . (UpdatePriceQuantityV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	                sleep(1);
	                continue;
	            }
...
// 	                }
	                ...
	                $errorCount++;
	                write_log("ST003 : $errorMessage . (UpdatePriceQuantityV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	                $results['message'] = $errorMessage;
	        ...
	        $timeMS2=getCurrentTimestampMS();
	        write_log("UpdatePriceQuantityV2 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." t2_1=".($timeMS2-$timeMS1),"info");
	 ...
	 * dzt 2017/01/20
	 */
	public function lazadaProductDeleteOld($param){
		$attributes = array();
		$reqParams = array();
		$results = array();
		$results['success'] = false;
...
// 			    $reqParams["Action"] = 'RemoveProduct';
			    $reqParams["Action"] = '/product/remove';
			}else{
...
// 			    $reqParams["Action"] = 'RemoveProduct';
	        ...
	        $feed['seller_sku_list'] = json_encode(explode(',',$param["SellerSkus"]));//新的方式
	        	
	        $timeMS1=getCurrentTimestampMS();
	        do{
				write_log("ST002:before call _callLazada. (lazadaProductDeleteV2) param:".print_r($reqParams,true),"info");
// 				write_log("lazadaProductDelete feed:".$feed,"info");
// 				$response = self::_callLazada($reqParams);
				$response = self::_callLazada($reqParams,$feed,true);
				write_log("ST002.1: (lazadaProductDeleteV2) response:".print_r($response,true),"info");
					
				// network error or json decode error
				if($response["success"] == false){
					$results['message'] = $response["message"];
					$errorCount++;
					write_log("ST001 : ".$response["message"]. " . SellerSkus - ".join(',', $param["SellerSkus"])." (lazadaProductDeleteV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
					sleep(1);
					continue;
				}
				...
				if($response["response"]["code"] != '0'){
				    ...
				    $results['errors'] = $errorMessage;
					$errorCount++;
					write_log("ST001 : $errorMessage . SellerSkus - ".join(',', $param["SellerSkus"])."  (lazadaProductDeleteV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
					$results['message'] = $errorMessage;
			...
			$timeMS2=getCurrentTimestampMS();
			write_log("lazadaProductDeleteV2 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." SellerSkus - ".join(',', $param["SellerSkus"])." t2_1=".($timeMS2-$timeMS1),"info");
...
// 	        $reqParams = array_merge($reqParams,self::$CONFIG);
	        ...
	        $timeMS1=getCurrentTimestampMS();
	        do{
	            write_log("ST002:before call _callLazada. (setLazadaImageV2) param:".print_r($reqParams,true),"info");
	            write_log("setLazadaImageV2 feed:".$feed,"info");
// 	            $response = self::_callLazada($reqParams,$feed,true);
	            $response = self::_callLazada($reqParams,array('payload'=>$feed),true);
	            write_log("ST002.1: (setLazadaImageV2) response:".print_r($response,true),"info");
	            
	            // network error or json decode error
	            if($response["success"] == false){
	                $results['message'] = $response["message"];
	                $errorCount++;
	                write_log("ST001 : ".$response["message"]. " . url - ".$param["url"]." (setLazadaImageV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	                sleep(1);
	                continue;
	            }
	            ...
	            if($response["response"]["code"] != '0'){
	                $errorMessage = "RequestAction:".$reqParams["Action"].",ErrorType ".$response["response"]["type"].",ErrorCode:".$response["response"]["code"].",ErrorMessage:".$response["response"]["message"];
	                $errorCount++;
	                write_log("ST001 : $errorMessage . url - ".$param["url"]."  (setLazadaImageV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	                $results['message'] = $errorMessage;
	        ...
	        $timeMS2=getCurrentTimestampMS();
	        write_log("setLazadaImageV2 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE.",t2_1=".($timeMS2-$timeMS1),"info");
...
// 	        $reqParams["Action"] = 'MigrateImage';
	        $reqParams["Action"] = '/image/migrate';
	        $feed = <<<EOD
<?xml version="1.0" encoding="UTF-8" ?>
<Request>
<Image>
...
// 	        $reqParams = array_merge($reqParams,self::$CONFIG);
	        ...
	        do{
	            ...
	            if($response["success"] == false){
	                $results['message'] = $response["message"];
	                $errorCount++;
	                write_log("ST001 : ".$response["message"]. " . url - ".$param["url"]." (migrateLazadaImageV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	                sleep(1);
	                continue;
	            }
	            ...
	            if($response["response"]["code"] != '0'){
	                $errorMessage = "RequestAction:".$reqParams["Action"].",ErrorType ".$response["response"]["type"].",ErrorCode:".$response["response"]["code"].",ErrorMessage:".$response["response"]["message"];
	                $errorCount++;
	                write_log("ST001 : $errorMessage . url - ".$param["url"]."  (migrateLazadaImageV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	                $results['message'] = $errorMessage.':[url] => '.$param["url"].' , [Message] => '.$response["response"]["message"];
	            ...
	            $results['url'] = $successResponse['image']['url'];
	            break;
	        }while ($errorCount < 5);
	        $timeMS2=getCurrentTimestampMS();
	        write_log("migrateLazadaImageV2 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." origin url - ".$param["url"].",,,url:".$results['url']." t2_1=".($timeMS2-$timeMS1),"info");
...
// 			$results['apiCallTimes'] = self::$API_CALL_TIMES;
	...
	public function getLazadaCategoryTree($param){
		...
		$current_time = explode(" ",microtime());
		try{
			$reqParams["Action"] = '/category/tree/get';
			$reqParams["timestamp"] = round($current_time[0]*1000+$current_time[1]*1000);
// 			$Timestamp = new DateTime();
// 			$Timestamp->setTimezone ( new DateTimeZone('UTC'));
// 			$reqParams["Timestamp"]=$Timestamp->format(DateTime::ISO8601);
// 			$reqParams = array_merge($reqParams,self::$CONFIG);
			...
			do{
				...
				if($response["success"] == false){
					$errorCount++;
					write_log("ST001 : ".$response["message"]. " . (getLazadaCategoryTreeV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
					sleep(1);
					continue;
				}
				...
				if($response["response"]["code"] != '0'){
				    ...
				    $results['message'] = $errorMessage;
					$errorCount++;
					write_log("ST001 : $errorMessage . (getLazadaCategoryTreeV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
...
// 				$results['response'] = $successResponse;
				if(!empty($successResponse)){
				    ...
				    write_log("ST001 : ".$results["message"]." . (getLazadaCategoryTreeV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
				    sleep(1);
				    continue;
				}
			...
			$timeMS2=getCurrentTimestampMS();
			write_log("getLazadaCategoryTreeV2 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." t2_1=".($timeMS2-$timeMS1),"info");
...
// 			$results['apiCallTimes'] = self::$API_CALL_TIMES;
	...
	public function getLazadaCategoryAttributes($param){
		...
		$current_time = explode(" ",microtime());
		try{
			$reqParams["Action"] = '/category/attributes/get';
			$reqParams["timestamp"] = round($current_time[0]*1000+$current_time[1]*1000);
...
// 			$reqParams = array_merge($reqParams,self::$CONFIG);
			...
			$reqParams['primary_category_id'] = $param["PrimaryCategory"];
			$timeMS1=getCurrentTimestampMS();
			do{
				$response = self::_callLazada($reqParams);
				...
				if($response["success"] == false){
					$errorCount++;
					$results['message'] = $response["message"];
					write_log("ST001 : ".$response["message"]. " . PrimaryCategory - ".$param["PrimaryCategory"]." (getLazadaCategoryAttibutesV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
					sleep(1);
					continue;
				}
				...
				if($response["response"]["code"] != '0'){
				    ...
				    $results['message'] = $errorMessage;
					$errorCount++;
					write_log("ST001 : $errorMessage . PrimaryCategory - ".$param["PrimaryCategory"]."  (getLazadaCategoryAttibutesV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
...
// 				$results['response'] = $successResponse;
				if(!empty($successResponse)){
				    ...
				    write_log("ST001 : ".$results["message"]." . (getLazadaCategoryAttibutesV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
				    sleep(1);
				    continue;
				}
				$results['success'] = true;
			...
			$timeMS2=getCurrentTimestampMS();
			write_log("getLazadaCategoryAttibutesV2 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." PrimaryCategory - ".$param["PrimaryCategory"]." t2_1=".($timeMS2-$timeMS1),"info");
...
// 			$results['apiCallTimes'] = self::$API_CALL_TIMES;
	...
	public function getLazadaShipmentProviders($param){
		...
		$current_time = explode(" ",microtime());
		try{
			$reqParams["Action"] = '/shipment/providers/get';
			$reqParams["timestamp"] = round($current_time[0]*1000+$current_time[1]*1000);
// 			$Timestamp = new DateTime();
// 			$Timestamp->setTimezone ( new DateTimeZone('UTC'));
// 			$reqParams["Timestamp"]=$Timestamp->format(DateTime::ISO8601);
// 			$reqParams = array_merge($reqParams,self::$CONFIG);
			...
			do{
				...
				if($response["success"] == false){
					$errorCount++;
					$results['message'] = $response["message"];
					write_log("ST001 : ".$response["message"]. " . (getLazadaShipmentProvidersV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
					sleep(1);
					continue;
				}
			    ...
			    if($response["response"]["code"] != '0'){
			        ...
			        $results['message'] = $errorMessage;
					$errorCount++;
					write_log("ST001 : $errorMessage . (getLazadaShipmentProvidersV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	
					// no need to retry according to errorCode returned from lazada
					$skipRetryErrorCodeArr = array_merge(self::$LAZADA_GLOBAL_ERRORS,array('IllegalAccessToken'));
					if (isset($response["response"]["code"]) and in_array($response["response"]["code"],$skipRetryErrorCodeArr)) {
						$results['message'] = $errorMessage;
						break;
					}
				...
				$successResponse = $response["response"]["data"];
// 				$results['response'] = $successResponse;
				if (isset($successResponse['shipment_providers'])){
				    if(!empty($successResponse['shipment_providers']))
				        $shipments = $successResponse['shipment_providers'];
				}else{
				    $errorCount++;
				    $results['message'] = "Reture data format has changed,cannot get ShipmentProviders V2 info.";
				    write_log("ST001 : ".$results["message"]." . (getLazadaShipmentProvidersV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
				    sleep(1);
				    continue;
				}
			...
			$timeMS2=getCurrentTimestampMS();
			write_log("getLazadaShipmentProviders V2 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." t2_1=".($timeMS2-$timeMS1),"info");
			$results['errorCount'] = $errorCount;
			$results['state_code'] = $response['state_code'];
// 			$results['apiCallTimes'] = self::$API_CALL_TIMES;
...
// 			Number of Brands to skip (i.e., an offset into the result set; together with the Limit parameter, simple result set paging is possible; if you do page through results, note that the list of Brands might change during paging).
			...
			do{
				...
				$response = self::_callLazada($reqParams);
				$timeMS12=getCurrentTimestampMS();
				write_log("t12_11=".($timeMS12-$timeMS11).",memory usage:".round(memory_get_usage()/1024/1024)."M. (getLazadaBrandsV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"info");
				
				// network error or json decode error
				if($response["success"] == false){
					$errorCount++;
					$results['message'] = $response["message"];
					write_log("ST001 : ".$response["message"]. " . (getLazadaBrandsV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
					sleep(1);
					continue;
				}
				...
				if($response["response"]["code"] != '0'){
				    ...
				    $results['message'] = $errorMessage;
					$errorCount++;
					write_log("ST001 : $errorMessage . (getLazadaBrandsV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
...
// 				$results['response'] = $successResponse;
				if(!empty($successResponse)){
				    $brands = array_merge($brands,$successResponse);
				}else{
				    $errorCount++;
				    $results['message'] = "Reture data format has changed,cannot get Brands info.";
				    write_log("ST001 : ".$results["message"]." . (getLazadaBrandsV2) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
				    sleep(1);
				    continue;
				}
			...
			if($hasGotAll || count($brands) == $reqParams['limit'] * ($page-1)){
			    $results['success'] = true;
			}
// 			$results['count'] = count($brands);
// 			$results['limit'] = $reqParams['limit'];
// 			$results['page'] = $page;
			$timeMS2=getCurrentTimestampMS();
			write_log("getLazadaBrandsV2 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." t2_1=".($timeMS2-$timeMS1),"info");
	...
	private static function _getLazadaProductMainImage($shopSku,$sellerSku,$db,$purge=false){
		...
		// Order items must be from the same order
		if(empty($shopSku)){
			return array(false, "'shop_sku' is needed to get order item image.","");
		}
		...
		try{
			 $param = [];
			 $param['SkuSellerList'] = $sellerSku;
			 $param['Limit'] = 1;// 只查询一个sku 不加Limit 的话getLazadaProducts 加了默认值会报错
			 $rtn = self::getLazadaProducts($param);
			  
			 if(!empty($rtn['success']) && !empty($rtn['products'])){
				 // write_log("dzt111 : ".json_encode($rtn["products"])." . (_getLazadaProductMainImage) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE, "info");
				 $productUrl = $rtn['products'][0]['skus'][0]['Url'];
				 $mainImageUrl = $rtn['products'][0]['skus'][0]['Images'][0];
				 $nowTime = time();			 
				 if ($hasRecord===false){
			 ...
			 }elseif(!empty($rtn['message'])){
				 return array(false , $rtn['message'] , "");
			 }else{
				 return array(false , $sellerSku.":no product found" , "");
			 }
	...
	public function getAccessToken($param){
	    ...
	    $current_time = explode(" ",microtime());
	    try{
	        $reqParams["Action"] = '/auth/token/create';
	        $reqParams["timestamp"] = round($current_time[0]*1000+$current_time[1]*1000);
// 	        $Timestamp = new DateTime();
// 	        $Timestamp->setTimezone ( new DateTimeZone('UTC'));
// 	        $reqParams["Timestamp"]=$Timestamp->format(DateTime::ISO8601);
// 	        $reqParams = array_merge($reqParams,self::$CONFIG);
	        ...
	        if(!isset($param["code"])){
	            return array("success"=>false, "message"=>"'code' is needed to get access token.", "body"=>"");
	        }else{
	            $reqParams['code'] = $param['code'];
	        }
	        write_log("getAccessToken step1 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." t2_1=".print_r($reqParams,true),"info");
	        
	        $response = self::_callLazada($reqParams);
	        
	        if($response["success"] == false){
	            $results['message'] = $response["message"];
	            write_log("ST001 : ".$response["message"]. " . (getAccessToken) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	            return $results;
	        }
...
// 	            $errorResponse = $response["response"]["ErrorResponse"];
	            $errorMessage = "RequestAction:".$reqParams["Action"].",ErrorType ".$response["response"]["type"].",ErrorCode:".$response["response"]["code"].",ErrorMessage:".$response["response"]["message"];
	            $results['message'] = $errorMessage;
	            write_log("ST001 : $errorMessage . (getAccessToken) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	            return $results;
	        }else{//获取成功
	            if(empty($response['response']['access_token'])){
	                return array("success"=>false, "message"=>"access_token is empty.", "body"=>"");
	            }
	            $results['success'] = true;
	        ...
	        $timeMS2=getCurrentTimestampMS();
	        write_log("getAccessToken UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." t2_1=".($timeMS2-$timeMS1),"info");
	...
	public function refreshAccessToken($param){
	    $reqParams = array();
	    $results = array();
	    $results['success'] = false;
	    $results['message'] = '';
	    ...
	    $current_time = explode(" ",microtime());
	    try{
	        $reqParams["Action"] = '/auth/token/refresh';
	        $reqParams["timestamp"] = round($current_time[0]*1000+$current_time[1]*1000);
	        // 	        $Timestamp = new DateTime();
	        // 	        $Timestamp->setTimezone ( new DateTimeZone('UTC'));
	        // 	        $reqParams["Timestamp"]=$Timestamp->format(DateTime::ISO8601);
	        ...
	        if(!isset($param["refresh_token"])){
	            return array("success"=>false, "message"=>"'refresh_token' is needed to refresh token.", "body"=>"");
	        }else{
	            $reqParams['refresh_token'] = $param['refresh_token'];
	        }
	         
	        write_log("refreshAccessToken step1 UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." t2_1=".print_r($reqParams,true),"info");
	        
	        $response = self::_callLazada($reqParams);
	         
	        if($response["success"] == false){
	            $results['message'] = $response["message"];
	            write_log("ST001 : ".$response["message"]. " . (refreshAccessToken) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	            return $results;
	        }
	        ...
	        if($response["response"]["code"] != '0'){
	            ...
	            $errorMessage = "RequestAction:".$reqParams["Action"].",ErrorType".$response["response"]["type"].",ErrorCode:".$response["response"]["code"].",ErrorMessage:".$response["response"]["message"];
	            $results['message'] = $errorMessage;
	            write_log("ST001 : $errorMessage . (refreshtAccessToken) UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE,"error");
	            return $results;
	        }else{//获取成功
	            if(empty($response['response']['access_token'])){
	                return array("success"=>false, "message"=>"access_token is empty.", "body"=>"");
	        ...
	        $timeMS2=getCurrentTimestampMS();
	        write_log("refreshAccessToken UserID=".self::$CONFIG["UserID"]." MarketPlace=".self::$MARKETPLACE." t2_1=".($timeMS2-$timeMS1),"info");
