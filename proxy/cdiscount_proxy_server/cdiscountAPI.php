<?php
class CdiscountAPI{
	static $CDISCOUNT_LOCATION_URL = "https://wsvc.cdiscount.com/MarketplaceAPIService.svc";
	static $CDISCOUNT_ACTION_PREFIX_URL = "http://www.cdiscount.com/IMarketplaceAPIService/";
	static $CDISCOUNT_SOAP_URL = "https://wsvc.cdiscount.com/MarketplaceAPIService.svc?wsdl";
	
	static $CDISCOUNT_TOKEN_ID;
	
	static function set_cdiscount_config($config){
		self::$CDISCOUNT_TOKEN_ID = $config['tokenid'];
	}//end of set_cdiscount_config
	...
	static function AUTH_VALIDATION($username , $password){
		...
		curl_setopt($ch, CURLOPT_TIMEOUT, 240);//set up timeout 
...
SoapDocument;
			...
			$location = self::$CDISCOUNT_LOCATION_URL;
			$action = self::$CDISCOUNT_ACTION_PREFIX_URL.__function__;
...
SoapDocument;
			$location = self::$CDISCOUNT_LOCATION_URL;
			$action = self::$CDISCOUNT_ACTION_PREFIX_URL.'ValidateOrderList';
			$response = $soap_client->__doRequest($xmldata ,$location , $action , 0);
			if (trim($response) != "")
			$response = Utility::xml_to_array($response);
		}catch(Exception $e){
...
SoapDocument;
	...
	*/
	...
	static function GetOfferListPaginated($params = array()){
		$success = true;
		$message = "";
		try
		{
			...
			if(!empty($params['OfferFilterCriterion']))
				$OfferFilterCriterion = "<OfferFilterCriterion>".$params['OfferFilterCriterion']."</OfferFilterCriterion>";
			else
				$OfferFilterCriterion = "<OfferFilterCriterion>UsedOffersOnly</OfferFilterCriterion>";
			
			$OfferSortOrder = '';
			if(!empty($params['OfferSortOrder']))
				$OfferSortOrder = "<OfferSortOrder>".$params['OfferSortOrder']."</OfferSortOrder>";
			else
				$OfferSortOrder = "<OfferSortOrder>BySoldQuantityDescending</OfferSortOrder>";
			$PageNumber = '';
			if(isset($params['PageNumber']))
				$PageNumber = "<PageNumber>".$params['PageNumber']."</PageNumber>";
			else
...
<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
    <s:Body>
        <GetOfferListPaginated xmlns="http://www.cdiscount.com">
            ...
            <offerFilter>
                <!-- Optionnel : Valeurs possibles: NewOffersOnly, UsedOffersOnly. -->
                $OfferFilterCriterion
...
SoapDocument;
	...
	static function GetSellerInformation($params = array()){
		...
		try
		{
			...
			//convert xml to array 
			if (is_string($response) && trim($response) != "")// dzt20190801 发现$response返回已经是object了，这一步需要跳过
			$response = Utility::xml_to_array($response);
...
SoapDocument;
			$response = $soap_client->__doRequest($xml, $location, $action, 0);
			if (trim($response) != "")
				$response = Utility::xml_to_array($response);
		...
		return array('GetSellerInformation' => $response, 'message' => $message, 'success' => $success);

	}

	
}//end of class CdiscountAPI
