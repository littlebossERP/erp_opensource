...
class LB_WEISUYIOverseaWarehouseAPI extends BaseOverseaWarehouseAPI
{
	private $appToken = '';
	private $appKey = '';
	...
	public function __construct(){
		self::$wsdl = 'http://ru.gdwse.com/default/svc/wsdl';
...
// 					'forceVerify'=>0,		//是否强制审核(如欠费，缺货时是否审核到仓配系统),0不强制，1强制，默认为0 当verify==1时生效
			...
			\Yii::info('LB_WEISUYIOverseaWarehouseAPI,puid:'.$puid.',request,order_id:'.$order->order_id.' '.json_encode($request),"carrier_api");
			
			$response = $this->submitGate->mainGate(self::$wsdl, $request, 'soap', 'callService');
			
			\Yii::info('LB_WEISUYIOverseaWarehouseAPI,puid:'.$puid.',response,order_id:'.$order->order_id.' '.json_encode($response),"carrier_api");
...
// 		return self::getResult(1,'','物流接口申请订单号时就会返回跟踪号');
		try{
			...
			\Yii::info('LB_WEISUYIOverseaWarehouseAPI_getTrackingNO,puid:'.$puid.',response,order_id:'.$order->order_id.' '.json_encode($response),"carrier_api");
	...
	function getWarehouse(){
		$request = array(
				'paramsJson'=>'',
				'appToken'=>'d836186400053ed9ac13b2ab0211d8fa',
				'appKey'=>'df39b0708b618c5239d64600293ceb07',
