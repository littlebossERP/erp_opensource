...
class LB_YAOUKUAIYUN extends BaseCarrierAPI{
	public static $wsdl = null;		//订单操作
	public static $wsdltools = null;	//订单工具
	public $authToken=null;
	
	public function __construct(){
		self::$wsdl = 'http://api.tea-post.com/OrderOnline/ws/OrderOnlineService.dll?wsdl';
		self::$wsdltools = 'http://api.tea-post.com/OrderOnlineTool/ws/OrderOnlineToolService.dll?wsdl';
	...
	public function getOrderNO($data){
		try{
			...
			foreach ($data['DeclaredValue'] as $k=>$v){
				if($service->shipping_method_code == 'AT'){
					if(trim($data['packet_no'][$k]) == ''){
						return self::getResult(1,'','亚欧快运陆运平邮 小包单号为必填!');
					}
				}
				
				$product[$k]=[
					'eName' => $data['EName'][$k],	//海关申报英文品名
					'name' => ($service->shipping_method_code == 'AT') ? $data['packet_no'][$k] : $data['CName'][$k],	//海关申报中文品名
			...
			$postdata['stateOrProvince'] = $order->consignee_province;								//州/省
			...
			\Yii::info('lb_yaoukuaiyun,request,puid:'.$puid.',order_id:'.$order->order_id.' '.json_encode($request),"carrier_api");
			
			//数据组织完成 准备发送
			$response = $this->submitGate->mainGate(self::$wsdl, $request, 'soap', 'createAndPreAlertOrderService');
			
			\Yii::info('lb_yaoukuaiyun,response,puid:'.$puid.',order_id:'.$order->order_id.' '.json_encode($response),"carrier_api");
			
			if(isset($response['data'])){
				if($response['data']->return->ack == 'Success'){
					//上传成功后进行数据的保存 订单，运输服务，客户号，订单状态，跟踪号(选填),returnNo(选填)
					$r = CarrierAPIHelper::orderSuccess($order, $service, $response['data']->return->referenceNumber, OdOrder::CARRIER_WAITING_PRINT, $response['data']->return->trackingNumber);
	...
	public function doPrint($data){
		try {
			...
			$this->authToken = $account_api_params['authToken'];
			...
			$request = array(
					...
					'orderCode'=>$customer_number_str,
					'printMode'=>empty($service['carrier_params']['printMode']) ? 'v' : $service['carrier_params']['printMode'],
					'printWhich'=>$service['carrier_params']['printWhich']
			);
				
			$url = 'http://label.tea-post.com/orderprint/multiple?token='.$this->authToken.'&orderCode='.$customer_number_str.'&printMode='.$service['carrier_params']['printMode'].'&printWhich='.$service['carrier_params']['printWhich'];
			$response = Helper_Curl::post($url,$request);
