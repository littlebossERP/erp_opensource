 ...
 * @version		1.0
 +------------------------------------------------------------------------------
 */
class LB_QUANCHENGDONGLICarrierAPI extends BaseCarrierAPI
{
	...
	function __construct(){
		if(isset(\Yii::$app->params["currentEnv"]) and \Yii::$app->params["currentEnv"]=='production' ){
			$this->wsdl='http://114.55.132.96:8221/webservice/PublicService.asmx?WSDL';
		}else{
			$this->wsdl='http://114.55.132.96:8221/webservice/PublicService.asmx?WSDL';//测试接口
		}
	...
	public function getOrderNO($pdata){
		try{
			...
			$param['order_pieces'] = 1; // int 外包装件数,默认 1
			...
			$shipper = array(
				...
				'shipper_areacode'	=> '' , // string 发件人区域代码
			...
			$consignee = array(
				...
				'consignee_fax'					=> $order->consignee_fax , // string 收件人传真
			...
			if($responseData['success'] != 1){
				return self::getResult(1, '', '上传订单失败！错误信息：' . $responseData['cnmessage'] .';'.$responseData['enmessage'] );
			}else{
				...
				return  self::getResult(0,$r, "上传订单成功！服务商单号：".$responseData['data']['shipping_method_no']);
			}
	...
	public function getTrackingNO($data){
		try {
			...
			if ($responseData['success'] != 1) {
				return self::getResult(1,'', '获取跟踪号失败！错误信息:' . $responseData['cnmessage'] .';'. 'enmessage:'.$responseData['enmessage']);
			}else{
				...
				return self::getResult( 0 , $responseData['data'] ,'物流跟踪号：'.$responseData['data']['shipping_method_no']);
			}
...
// 				$listorder[]['reference_no'] = '68215539530355';
				...
				if ($responseData['success'] != 1) {
					...
					if( 2 == $configInfo['lable_file_type']){
						$fileType = 'pdf';
					}else{
						$fileType = 'jpg';
					}
					// lable_content_type string 文件内容类型代码 
					// lable_file string 标签文件内容 base64 字符串
					foreach ($responseData['data'] as $oneResult){
						$pdfUrl = CarrierAPIHelper::savePDF(Helper_Curl::get($oneResult['lable_file']),$puid,$account->carrier_code.'_'.$oneOrder->customer_number, 0 ,$fileType);
...
// 					$oneOrder->is_print_carrier = 1;
	...
	*/
	...
	public function getshippingmethod(){
		try {
			$this->appToken ='6e16ad3f00f9e02da69b3e00a4088568';
			$this->appKey = '0697cca0d4a282b6d80bb0ca87b976880697cca0d4a282b6d80bb0ca87b97688';
