 ...
 * 运通宝通用物流调用方式
 +------------------------------------------------------------------------------
 * @category	Interface
 * @package		carrierAPI/LB_RTBCOMPANYCarrierAPI
 * @subpackage  Exception
 * @author		hqw
 * @version		1.0
 +------------------------------------------------------------------------------
 */
class LB_RTBCOMPANYCarrierAPI extends BaseCarrierAPI
{
	...
	public static $rtbCompanyArr = array(
			'lb_yameiguojirtbcompany' => '112.74.18.63',
			'lb_baishiyundartbcompany' => '120.25.107.138',
			'lb_bangliguojirtbcompany' => '120.25.101.39',
			'lb_fuzhouhuixiangrtbcompany' => '112.74.132.17:8888',
			'lb_weidaguojirtbcompany' => '120.26.51.105:8888',
			'lb_369guojikuaidirtbcompany' => '112.74.205.202',
			'lb_haoxingtairtbcompany' => '120.25.147.194:8084',
			'lb_tuobangguojirtbcompany' => '120.25.147.194:8085',
			'lb_yiwuchanghertbcompany' => '121.40.225.128:9988',
			'lb_beijingyichengrtbcompany' => '101.200.1.127:81',
	);
	
	function __construct($rtbName = ''){
		if(!isset(self::$rtbCompanyArr[$rtbName])){
			return self::getResult(1,'','Not Docking:'.$rtbName);
		}
		
		if(isset(\Yii::$app->params["currentEnv"]) and \Yii::$app->params["currentEnv"]=='production' ){
			$this->wsdl='http://'.self::$rtbCompanyArr[$rtbName].'/webservice/PublicService.asmx?WSDL';
		}else{
			$this->wsdl='http://'.self::$rtbCompanyArr[$rtbName].'/webservice/PublicService.asmx?WSDL';//测试接口
		}
	...
	public function getOrderNO($pdata){
		try{
			...
			$param['shipping_method_no'] = ''; // string 服务商单号
				
			$param['order_pieces'] = $data['order_pieces']; // int 外包装件数,默认 1
			...
			$consignee = array(
					...
					'consignee_province'			=> $order->consignee_province , // string 收件人省
					'consignee_city'				=> $order->consignee_city , // string 收件人城市
			...
			foreach ($order->items as $j=>$vitem){
				if (empty($data['EName'][$j])){
					return self::getResult(1, '', '海关申报品名不能为空');
				}
				
				if (empty($data['DeclaredValue'][$j])){
					return self::getResult(1, '', '申报价值USD不能为空');
				}
			...
			// array 额外服务:10:出口退税;5Y:保险等级1;6P:保险等级2;8Y:保险等级3;G0:DDP;I0:代做发票;T1:海外退件需退回;
			$extra_service = array();
			if(!empty($data['order_extra_service'])){
				if($data['order_extra_service'] != 'setNo'){
					if(in_array($data['order_extra_service'], array('5Y','6P','8Y')) && empty($data['order_extra_value'])){
						return self::getResult(1, '', '该额外服务必须填写额外服务值');
					}
					
					$extra_service['extra_servicecode'] = $data['order_extra_service']; //  string required 额外服务类型代码
					$extra_service['extra_servicevalue'] = $data['order_extra_value']; // string required 额外服务值
					$extra_service['extra_servicenote'] = $data['order_extra_remark']; // string 备注
				}
			}
	...
	public function doPrint($data){
		try {
			...
			// 这里默认填了lable_content_type=>1 以防卖家没有在 运输服务里面选择。
			$configInfo = array(
					'lable_file_type'	=> '2' , // 标签文件类型2 ：PDF文件
					'lable_paper_type'	=> '1' , // string 纸张类型  1:label 标签纸
			...
			foreach ($data as $v) {
				...
				$response = $this->soapClient->ServiceEntrance(array('appToken'=>$this->appToken , 'appKey'=>$this->appKey , 'serviceMethod'=>"getnewlabel", 'paramsJson'=>json_encode($param)));
				...
				if ($responseData['success'] != 1) {
					...
					// lable_content_type string 文件内容类型代码
					foreach ($responseData['data'] as $oneResult){
						...
						$pdfUrl = CarrierAPIHelper::savePDF($responsePdf,$puid,$account->carrier_code.'_'.$oneOrder->customer_number.'_'. $oneResult['lable_content_type'], 0 , 'pdf');
