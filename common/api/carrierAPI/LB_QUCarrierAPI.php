...
+------------------------------------------------------------------------------
 * @category	Interface
 * @package		carrierAPI/QUCarrierAPI
 * @subpackage  Exception
 * @author		dwg
 * @version		1.0
+------------------------------------------------------------------------------
 */
class LB_QUCarrierAPI extends BaseCarrierAPI
{
...
//            $this->wsdl='http://kd.szice.net:8086/xms/services/order?wsdl';  //旧正式环境接口地址(旧互联易)
            $this->wsdl='http://api.goto56.com:8086/apic/services/order?wsdl';  //新正式环境接口地址（趣物流）
        }else{
            $this->wsdl='http://www.360chain.com:8086/xms/services/order?wsdl';//测试环境接口地址
        }
    ...
    public function getOrderNO($pdata){
        ...
        try{
            ...
            //设置用户密钥Token
            $this->userToken = isset($account_api_params['userToken']) ? $account_api_params['userToken']:'';
             ...
             */
//             $user_Token_Official = '0ee02d147f1e4cbc882e3fd42e826f2c';    //正式环境userToken
//             $user_Token_Test = '8a3ddab172a74ad887be919a2aa41641';    //测试环境userToken
            ...
            if($this->autoFetchTrackingNo=='N') {
                ...
                } else
                {
                    ...
                    else{$trackingNo =  $data['trackingNo'];}
                }
            }
...
//            print_r($createOrderRequest);exit;

            $responseData = $this->soapClient->createAndAuditOrder($this->userToken,$createOrderRequest);
            \Yii::info(print_r($responseData,true),"file");// 先记下结果，记下refrence_no，这个返回应该与上面提交refrence_no一样。

            if(($responseData->success==1) && (!empty($responseData->trackingNo)))
            {
                ...
                $r = CarrierApiHelper::orderSuccess( $order , $service , $responseData->id , OdOrder::CARRIER_WAITING_PRINT , $responseData->trackingNo);
                
                $print_param = array();
                $print_param['carrier_code'] = $service->carrier_code;
                $print_param['api_class'] = 'LB_HULIANYICarrierAPI';
                $print_param['userToken'] = $this->userToken;
                $print_param['tracking_number'] = $responseData->trackingNo;
                ...
                return  self::getResult(0,$r, "操作成功! 订单编号：".$responseData->id ."<br>服务商跟踪号：".$responseData->trackingNo);
...
//         return BaseCarrierAPI::getResult(1, '', '物流接口不支持取消物流单。');
        ...
        try{
            ...
            $carrier_order_id = $shipped->customer_number;
            $responseData = $this->soapClient->deleteOrder($this->userToken,$carrier_order_id);
            if(!empty($responseData->error)){
                $str ='';
                foreach($responseData->error->errorInfo as $v){
                    $str .= $v.'<br/>';
                }
                return BaseCarrierAPI::getResult(1, '', '操作失败！错误信息：'.$str);
            }
    ...
    public function doPrint($data){
        try {
            ...
            //取得打印参数
            $printOrderRequest = array(
                // 'trackingNo'       =>$tracking_number, //服务商跟踪号码
                'printSelect'      =>empty($normal_params['printSelect']) ?'3':$normal_params['printSelect'],  //选择打印样式
                'pageSizeCode'     =>empty($normal_params['pageSizeCode']) ?'6':$normal_params['pageSizeCode'],    //纸张尺寸
                'showCnoBarcode'   =>empty($normal_params['showCnoBarcode']) ?'0':$normal_params['showCnoBarcode'],    //是否显示客户单号
                'downloadPdf'      =>empty($normal_params['downloadPdf']) ?'0':$normal_params['downloadPdf'],  //是否下载PDF
            );
...
//                 $oneOrder->is_print_carrier = 1;
            ...
            $printOrderRequest['trackingNo'] = rtrim($tracking_number_list,',');

            $responseData = $this->soapClient->printOrder($this->userToken,$printOrderRequest);
            if($responseData->success==1) {
                $pdf_respond = self::get($responseData->url,null,null,false,null,null,true);
                // $pdf_respond = Helper_Curl::get($responseData->url);
                $pdfurl = CarrierAPIHelper::savePDF($pdf_respond,$puid,$oneOrder->order_source_order_id.'_'.$account->carrier_code,0);
                return self::getResult(0,['pdfUrl'=>$pdfurl['pdfUrl']],'连接已生成,请点击并打印');//访问URL地址

                // return self::getResult(0, ['pdfUrl' => $responseData->url], '连接已生成,请点击并打印');
            }
            else{
                return self::getResult(1, '', '打印失败！错误信息：'.$responseData->error->errorInfo);
            }
        }catch (CarrierException $e){
    ...
    public function getshippingmethod(){
        try {
            $user_Token_Official = '0ee02d147f1e4cbc882e3fd42e826f2c';    //正式环境
            $user_Token_Test = '8a3ddab172a74ad887be919a2aa41641';    //测试环境

            // $responseData = $this->soapClient->getTransportWayList($user_Token_Test);
            $responseData = $this->soapClient->getTransportWayList($user_Token_Official);
            $responseArr = $responseData->transportWays;
            foreach($responseArr as $response)
            {
                print_r($response->code.':'.$response->name.';');
            }
            return $responseArr;
        }catch (CarrierException $e){
            return self::getResult(1,'',$e->msg());
    ...
    public function getCarrierLabelApiPdf($SAA_obj, $print_param){
    	try {
    		...
    		//设置用户密钥Token
    		$this->userToken = $print_param['userToken'];
    		...
    		$printOrderRequest = array(
    			...
    			'pageSizeCode' => '6',    //纸张尺寸
    			'showCnoBarcode' => empty($normal_params['showCnoBarcode']) ?'0':$normal_params['showCnoBarcode'],    //是否显示客户单号
    			'downloadPdf' => 0,  //是否下载PDF
    		);

    		$printOrderRequest['trackingNo'] = $print_param['tracking_number'];
    		...
    		if($responseData->success==1) {
    			...
    			return ['error'=>1, 'msg'=>'打印失败！错误信息：'.$responseData->error->errorInfo, 'filePath'=>''];
    		}
    	}catch (CarrierException $e){
