...
class WishOrderController extends Controller{
	...
	public function actionList(){
		//check模块权限
		$permission = \eagle\modules\permission\apihelpers\UserApiHelper::checkPlatformPermission('wish');
		if(!$permission)
			return $this->render('//errorview_no_close',['title'=>'访问受限:没有权限','error'=>'您还没有订单模块的权限!']);
		
		AppTrackerApiHelper::actionLog("Oms-wish", "/order/wish/list");
		...
		$data = OdOrder::find()->where(['order_source' => 'wish' ]);
		
		
		$showsearch=0;
		$op_code = '';
		
		$tmpSellerIDList = PlatformAccountApi::getPlatformAuthorizeAccounts('wish');//添加账号权限	lzhl 2017-03
		$wishAccountList = [];
		$wishUsersDropdownList = array();
		foreach($tmpSellerIDList as $sellerloginid=>$store_name){
			$wishAccountList[] = $sellerloginid;
			$wishUsersDropdownList[$sellerloginid] = $store_name;
		}
		if(empty($wishAccountList)){
			//如果为测试用的账号就不受平台绑定限制
			...
			if (!in_array(\Yii::$app->user->identity->getParentUid(),$test_userid['yifeng'])){
				//无有效权限账号时
				return $this->render('//errorview_no_close',['title'=>'访问受限:没有权限','error'=>'您还没有获得任何 Wish 账号管理权限!']);
			}
		}
		$data->andWhere(['selleruserid'=>$wishAccountList]);
		...
		$addi_condition = ['order_source'=>'wish'];
		$addi_condition['sys_uid'] = \Yii::$app->user->id;
		$addi_condition['selleruserid_tmp'] = $wishAccountList;
	    ...
	    */
	    ...
	    if($isParent){
	    	$gotCache = RedisHelper::getOrderCache2($puid,$uid,'wish',"MenuStatisticData",$stroe) ;
	    }else{
	    	if (!empty($_REQUEST['selleruserid'])){
	    		$gotCache = RedisHelper::getOrderCache2($puid,$uid,'wish',"MenuStatisticData",$_REQUEST['selleruserid']) ;
	    	}else{
	    		$gotCache = RedisHelper::getOrderCache2($puid,$uid,'wish',"MenuStatisticData",'all') ;
	    	}
	    }
	    if (!empty($gotCache)){
	    ...
	    if ($hitCache <>"Hit"){
	    	if (!empty($_REQUEST['selleruserid'])){
	    		$counter = WishOrderHelper::getMenuStatisticData(['selleruserid'=>$_REQUEST['selleruserid']]);
	    	}else{
	    		if(!empty($wishAccountList)){
	    			$counter = WishOrderHelper::getMenuStatisticData(['selleruserid'=>$wishAccountList]);
	    		}else{
	    			//无有效绑定账号
	    			$counter=[];
	    			$claimOrderIDs=[];
	    	...
	    	if (!empty($_REQUEST['selleruserid'])){
	    		RedisHelper::setOrderCache2($puid,$uid,'wish',"MenuStatisticData",$_REQUEST['selleruserid'],$counter) ;
	    	}else{
	    		RedisHelper::setOrderCache2($puid,$uid,'wish',"MenuStatisticData",'all',$counter) ;
	    	}
	    }
	    ...
	    if (!empty($wishAccountList)){
	        ...
	        $counter = WishOrderHelper::getMenuStatisticData();
	    }
...
// 	        $order_nav_key_word='';
	    ...
	    $DashBoardCache = WishOrderHelper::getOmsDashBoardCache($uid);
...
// 	    }
	    ...
	    $nations_array = json_decode(ConfigHelper::getConfig("WishOMS/nations"),true);
...
// 	    }
	    ...
	    //获取wish账号别名
	    $selleruserids_new = \eagle\modules\platform\apihelpers\WishAccountsApiHelper::getWishAliasAccount($wishUsersDropdownList);
...
// 			'wishUsersDropdownList'=>$wishUsersDropdownList,
			'usertabs'=>$usertabs,
			'counter'=>$counter,
		    'warehouseids'=>$warehouseids,
		    'selleruserids'=>$wishUsersDropdownList,
...
// 			'warhouseArr'=>$warhouseArr,
		    ...
		    'doarr'=>WishOrderHelper::getWishCurrentOperationList($op_code,'b') ,
		    'doarr_one'=>WishOrderHelper::getWishCurrentOperationList($op_code,'s'),
...
// 			$order->save();
			...
			$order->save();
			AppTrackerApiHelper::actionLog("Oms-wish", "/order/wish/edit-save");
...
// 			echo "<script language='javascript'>window.opener.location.reload();</script>";
		...
		AppTrackerApiHelper::actionLog("Oms-wish", "/order/wish/edit-page");
	...
	public function actionSignshipped(){
		AppTrackerApiHelper::actionLog("Oms-wish", "/order/wish/signshipped");
		$wishShippingMethod = WishOrderInterface::getShippingCodeNameMap();
		if (\Yii::$app->request->getIsPost()){
			$orders = OdOrder::find()->where(['in','order_id',\Yii::$app->request->post()['order_id']])->all();
			return $this->render('signshipped',['orders'=>$orders,'wishShippingMethod'=>$wishShippingMethod]);
		}else {
	...
	public function actionSignshippedsubmit(){
		if (\Yii::$app->request->getIsPost()){
			AppTrackerApiHelper::actionLog("Oms-wish", "/order/wish/signshippedsubmit");
			$user = \Yii::$app->user->identity;
			$postarr = \Yii::$app->request->post();
			$tracker_provider_list  = WishOrderInterface::getShippingCodeNameMap();
	...
	function actionSyncmt(){
		$puid = \Yii::$app->user->identity->getParentUid();
		$sync = SaasWishUser::find()->where(['is_active'=>'1' , 'uid'=>$puid,])->all();
		//只显示有权限的账号，lrq20170828
		$account_data = \eagle\modules\platform\apihelpers\PlatformAccountApi::getPlatformAuthorizeAccounts('wish');
		foreach($sync as $key => $val){
			if(!array_key_exists($val->store_name, $account_data)){
				unset($sync[$key]);
			}
		}
	...
	function actionAjaxsyncmt(){
		if (\Yii::$app->request->isPost){
			try {
				...
				//检查账号是否存在
				$model = SaasWishUser::findOne(['site_id'=>$_POST['site_id']]);
				if (!empty($model)){
					$result = WishAccountsV2Helper::setManualRetrieveOrder($_POST['site_id']);
	...
	function actionUnlockOrderQueue(){
		...
		if (!empty($_REQUEST['site_id'])   ){
			...
			WishOrderHelper::unlockWishOrderQueue($_REQUEST['site_id'],$msg , $status);
			exit('OK');
		}else{
			exit('no site id');
		}
	        ...
	        if (count($orderids)>0){
	            try {
	                ...
	                foreach ($orderids as $order_id){
	                    ...
	                    $tmpRT = OrderHelper::requestRefreshOrderQueue($uid,  $order_id, 'wish');
	...
	public function actionUserDashBoard(){
	    ...
	    $cacheData = WishOrderHelper::getOmsDashBoardData($uid,$isRefresh);
	    ...
	    $AccountProblems = WishOrderHelper::getUserAccountProblems($uid); //账号信息统计
	...
	public function actionOrderSyncInfo(){
	    ...
	    $detail = WishOrderHelper::getOrderSyncInfoDataList($status,$last_sync_time );
	    ...
	    //sellerid
// 	    $selleruserids=Helper_Array::toHashmap(SaasWishUser::find()->where(['uid'=>\Yii::$app->user->identity->getParentUid()])->select(['store_name'])->asArray()->all(),'store_name','store_name');
	    ...
	    */
// 	    $counter = WishOrderHelper::getMenuStatisticData(['selleruserid'=>$selleruserids]);
