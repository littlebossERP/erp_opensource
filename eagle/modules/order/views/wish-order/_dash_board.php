	...
	<table style="width:100%">
			...
			<td width="<?=empty($advertData)?'100%':'75%' ?>" style="float:left;">
					...
					<div id="cd-oms-reminder">
						<div id="cd-oms-reminder-content" class="bg-warning">
							<div style="float:left;width:100%;clear:both;">
								...
								<span style="width:100%;"><a href="<?=Url::to(['/order/wish-order/list','weird_status'=>'wfs'])?>" target="_blank">您有 <b style="font-weight:600;"><?=$reminder['YiFuKuan'] ?> </b>单已付款订单超过2天未处理</a></span><span qtipkey="oms_order_status_prolonged_paid"></span><br>
								<?php endif;
									  if(!empty($reminder['FaHuoZhong'])):?>
								<span style="width:100%;"><a href="<?=Url::to(['/order/wish-order/list','weird_status'=>'wfd'])?>" target="_blank">您有 <b style="font-weight:600;"><?=$reminder['FaHuoZhong'] ?> </b>单发货中订单超过2天未处理</a></span><span qtipkey="oms_order_status_prolonged_in delivery"></span><br>
								<?php endif;
									  if(!empty($reminder['YiFaHuo'])):?>
								<span style="width:100%;"><a href="<?=Url::to(['/order/wish-order/list','weird_status'=>'wfss'])?>" target="_blank">您有 <b style="font-weight:600;"><?=$reminder['YiFaHuo'] ?> </b>单已发货单超过2天未处理</a></span><span qtipkey="oms_order_status_prolonged_shipped"></span><br>
								<?php endif;
									  if(!empty($reminder['ZhuangTaiYiChang'])):?>
								<span style="width:100%;"><a href="<?=Url::to(['/order/wish-order/list','weird_status'=>'sus'])?>" target="_blank">您有 <b style="font-weight:600;"><?=$reminder['ZhuangTaiYiChang'] ?> </b>单订单小老板状态与CD后台状态不匹配。</a></span><span qtipkey="cd_order_weird_status_sus"></span><br>
								<?php endif;
									  if(!empty($reminder['WuTianWeiShangWang'])):?>
								<span style="width:100%;"><a href="<?=Url::to(['/order/wish-order/list','weird_status'=>'tuol'])?>" target="_blank">您有 <b style="font-weight:600;"><?=$reminder['WuTianWeiShangWang'] ?> </b>单订单超过五天物流仍未上网。</a></span><span qtipkey="cd_order_weird_status_tuol"></span><br>
								<?php endif;?>
							</div>
						</div>
					</div>
				...
				<div style="font-size: 12px;">
					...
					if(!empty($AccountProblems)){
						foreach ($AccountProblems as $initial_failed_account){
					?>
					<?php if(isset($initial_failed_account['is_timeout'])):?>
    					<span style="width:100%;float:left;clear:both;">您绑定的wish账号：<?=$initial_failed_account['store_name'] ?> 的验证信息已经过期！&#8594;
    						<a href="<?=$platformUrl?>" target="_blank">编辑绑定设置</a>
    					</span>
					<?php endif;?>
					<?php if(isset($initial_failed_account['order_retrieve_message'])):?>
					    <span style="width:100%;float:left;clear:both;">您绑定的wish账号：<?=$initial_failed_account['store_name'] ?> 获取最近订单失败：<?=$initial_failed_account['order_retrieve_message'] ?>，请联系技术人员查询原因</span>
					<?php endif;?>
					<?php if(isset($initial_failed_account['is_active'])):?>
    					<span style="width:100%;float:left;clear:both;">
    						您绑定的wish账号：<?=$initial_failed_account['store_name'] ?> 未开启同步！&#8594;
    						<a href="<?=$platformUrl?>" target="_blank">编辑绑定设置</a>
    					</span>
					<?php endif;?>
					<?php }
		...
		<!-- 推荐/新功能列 -->
			<td width="<?=empty($advertData)?'0px':'25%' ?>" style="margin-top: 10px;float:right;display:<?=empty($advertData)?'none':'block' ?>">
				<div style="width:100%;overflow-y: auto;" id="wish_oms_adver">
