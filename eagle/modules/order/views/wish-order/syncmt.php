...
<div class="modal-body">
	...
	<tr>
		<td data-store-name="<?=$s->store_name ?>"> <?=(($s->store_name_alias == '') ? $s->store_name : $s->store_name_alias) ?></td>
...
// 		var store_name = $(obj).parent().prev().prev().text();
		var store_name = $(obj).parent().prev().prev().attr('data-store-name');
		var sit_id = $(obj).parent().data('site-id');
		var Url='<?=Url::to(['/order/wish-order/ajaxsyncmt'])?>';
		$.ajax({
	        type : 'post',
	        cache : 'false',
