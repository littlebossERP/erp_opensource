...
<div class="content-wrapper" >
    <?php 
        $problemAccounts = WishOrderHelper::getUserAccountProblems($uid);
    ?>
    <!-- 账号异常提示区域 -->
    <?php if(!empty($problemAccounts)):?>
	<div class="alert alert-danger" role="alert" style="width:100%;">
	   <?php foreach ($problemAccounts as $detailAccounts):?>
	       <?php if(isset($detailAccounts['is_timeout'])):?>
		      <span>您绑定的Wish账号：<?=(isset($selleruserids_new[$detailAccounts["store_name"]]) ? $selleruserids_new[$detailAccounts["store_name"]] : $detailAccounts["store_name"]) ?> 的验证信息已经过期！</span><br />
		   <?php endif;?>
		   <?php if(isset($detailAccounts['order_retrieve_message'])):?>
		      <span>您绑定的Wish账号：<?=(isset($selleruserids_new[$detailAccounts["store_name"]]) ? $selleruserids_new[$detailAccounts["store_name"]] : $detailAccounts["store_name"]).$detailAccounts['order_retrieve_message']?></span><br />
		   <?php endif;?>
	   <?php endforeach;?>
	</div>
	<?php endif;?>
	...
	<?php 
		...
		echo OrderFrontHelper::displayOrderPaidProcessHtml($counter,'wish',[(string)OdOrder::PAY_PENDING]);
	...
	<div>
		<!-- 搜索区域 -->
		<form class="form-inline" id="form1" name="form1" action="/order/wish-order/list" method="post">
...
// 			$omsPlatformFinds[] = array('event'=>'OrderCommon.order_platform_find(\'oms\',\'wish\',\'\')','label'=>'全部');
			...
			if(count($selleruserids_new) > 0){
				...
				foreach ($selleruserids_new as $tmp_selleruserKey => $tmp_selleruserid){
					$omsPlatformFinds[] = array('event'=>'OrderCommon.order_platform_find(\'oms\',\'wish\',\''.$tmp_selleruserKey.'\')','label'=>$tmp_selleruserid);
				}
				
				$pcCombination = OrderListV3Helper::getPlatformCommonCombination(array('type'=>'oms','platform'=>'wish'));
				if(count($pcCombination) > 0){
					foreach ($pcCombination as $pcCombination_K => $pcCombination_V){
						$omsPlatformFinds[] = array('event'=>'OrderCommon.order_platform_find(\'oms\',\'wish\',\''.'com-'.$pcCombination_K.'-com'.'\')',
							'label'=>'组合-'.$pcCombination_K, 'is_combined'=>true, 'combined_event'=>'OrderCommon.platformcommon_remove(this,\'oms\',\'wish\',\''.$pcCombination_K.'\')');
					}
				}
			}
			echo OrderListV3Helper::getDropdownToggleHtml('卖家账号', $omsPlatformFinds);
			if(!empty($_REQUEST['selleruserid_combined'])){
				echo '<span onclick="OrderCommon.order_platform_find(\'oms\',\'wish\',\'\')" class="glyphicon glyphicon-remove" style="cursor:pointer;font-size: 10px;line-height: 20px;color: red;margin-right:5px;" aria-hidden="true" data-toggle="tooltip" data-placement="top" data-html="true" title="" data-original-title="清空卖家账号查询条件"></span>';
			}
			//店铺查找代码 E
			?>
			...
			<div class="input-group iv-input">
		        <?php $sel = [
		        	'order_source_order_id'=>'Wish订单号',
					'sku'=>'SKU',
					'order_source_itemid'=>'Wish Product ID',
					'tracknum'=>'物流号',
					'buyeid'=>'买家账号',
		        	'consignee'=>'买家姓名',
					'email'=>'买家Email',
	    	...
	    	<a title="同步订单" class="iv-btn btn-important" onclick="dosyncorder();" style="color:white;">同步订单</a>
	    	<a target="_blank"  title="手工订单" class="iv-btn btn-important" href="/order/order/manual-order-box?platform=wish" >手工订单</a>
			
	    	<!----------------------------------------------------------- 同步情况 ----------------------------------------------------------->
			<?php if (!empty($_REQUEST['menu_select']) && $_REQUEST['menu_select'] =='all'):?>
			<div class="pull-right" style="height: 40px;">
				<?=Html::button(TranslateHelper::t('同步情况'),['class'=>"iv-btn btn-important",'onclick'=>"OrderCommon.showAccountSyncInfo(this);",'name'=>'btn_account_sync_info','data-url'=>'/order/wish-order/order-sync-info'])?>
			</div>
			<?php endif;?>
	    	<!----------------------------------------------------------- 常用筛选 ----------------------------------------------------------->
			<!--  
			...
			<?=Html::dropDownList('order_source_status',@$_REQUEST['order_source_status'],$order_source_status_search,['class'=>'iv-input','prompt'=>'Wish状态','id'=>'order_source_status','style'=>'width:200px;margin:0px'])?>
			...
			if (@$_REQUEST['order_status'] != OdOrder::STATUS_CANCEL){
				\eagle\modules\order\helpers\OrderFrontHelper::displayOrderSyncShipStatusToolbar(@$_REQUEST['order_sync_ship_status'],'wish');
			}
			?>
...
// 			echo "&nbsp;";
		...
		<?=OrderListV3Helper::getDropdownToggleHtml('批量操作', $doarr, 'orderCommonV3.doaction3');?>
...
// 			break;
		...
		case 'refreshOrder':
			...
			$.post('<?=Url::to(['/order/wish-order/refreshorder'])?>',{orders:thisOrderList},function(result){
		...
		case 'checkorder':
			...
			$.post('<?=Url::to(['/order/wish-order/checkorderstatus'])?>',{orders:idstr , 'refresh_force':refresh_force},function(result){
				bootbox.alert(result);
				location.reload();
			});
			break;
...
// 			break;
		...
		case 'editOrder':
			idstr = [];
			idstr.push(orderid);
			window.open("<?=Url::to(['/order/wish-order/edit'])?>"+"?orderid="+orderid,'_blank')
			break;
		case 'suspendDelivery':
			idstr = [];
			idstr.push(orderid);
...
// 			break;
		...
		case 'refreshOrder':
			idstr = [];
			idstr.push(orderid);
			$.post('<?=Url::to(['/order/wish-order/refreshorder'])?>',{orders:idstr},function(result){
				bootbox.alert(result);
				location.reload();
			});
			break; 
		...
		case 'checkorder':
			...
			$.post('<?=Url::to(['/order/wish-order/checkorderstatus'])?>',{orders:orderid , 'refresh_force':refresh_force},function(result){
				bootbox.alert(result);
				location.reload();
			});
			break;
...
function movestatus(val){
	...
	$.post('<?=Url::to(['/order/wish-order/movestatus'])?>',{orderids:idstr,status:val},function(result){
		bootbox.alert('操作已成功');
	});
}
//上传物流单号
function importordertracknum(){
	if($("#order_tracknum").val()){
		$.ajaxFileUpload({  
			 url:'<?=Url::to(['/order/wish-order/importordertracknum'])?>',
...
function changemanual(orderid,obj){
	$.post('<?=Url::to(['/order/wish-order/changemanual'])?>',{orderid:orderid},function(result){
		if(result == 'success'){
			bootbox.alert('操作已成功');
			var str;
			str=$(obj).html();
...
function setusertab(orderid,tabobj){
	...
	$.post('<?=Url::to(['/order/wish-order/setusertab'])?>',{orderid:orderid,tabid:tabid},function(result){
		if(result == 'success'){
			bootbox.alert('操作已成功');
		}else{
			bootbox.alert(result);
...
function ajaxdesc(obj){
	 ...
	 var oiid=$(obj).attr('oiid');
	  $.post('<?=Url::to(['/order/wish-order/ajaxdesc'])?>',{desc:desc,oiid:oiid},function(r){
...
function dosyncorder(){
	var Url=global.baseUrl +'order/wish-order/syncmt';
	$.ajax({
        type : 'get',
        cache : 'false',
        data : {},
...
function showDashBoard(autoShow){
	...
	$.ajax({
		type: "GET",
		url:'/order/wish-order/user-dash-board?autoShow='+autoShow, 
		success: function (result) {
			$.hideLoading();
			$("#dash-board-enter").toggle();
			bootbox.dialog({
				className : "dash-board",
				title: Translator.t('Wish订单监控面板'),
				message: result,
				closeButton:false,
				buttons:{
...
function requestGenerateDashBoardData(){
	...
	$.ajax({
		type: "GET",
		url:'/order/wish-order/genrate-user-dash-board', 
		success: function (result) {
			if ($('#dash-board-enter').css('display') == 'none'){
				$('#dash-board-enter').toggle(1000);
			}
