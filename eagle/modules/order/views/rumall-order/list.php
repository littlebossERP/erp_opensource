...
<div class="content-wrapper" >
	...
	<?php foreach ($allAccounts as $detailAccounts):?>
		<span>您绑定的Rumall账号：<?php echo $detailAccounts["name"];?>获取最近订单失败：<?php echo $detailAccounts["order_retrieve_message"];?></span><br />
	<?php endforeach;?>
	</div> 
	<?php endif;?>
	<!-- ----------------------------------已付款订单分类搜索 start--------------------------------------------------------------------------------------------- -->	
...
// 			$omsPlatformFinds[] = array('event'=>'OrderCommon.order_platform_find(\'oms\',\'rumall\',\'\')','label'=>'全部');
				...
				if(count($pcCombination) > 0){
					foreach ($pcCombination as $pcCombination_K => $pcCombination_V){
						$omsPlatformFinds[] = array('event'=>'OrderCommon.order_platform_find(\'oms\',\'rumall\',\''.'com-'.$pcCombination_K.'-com'.'\')',
							'label'=>'组合-'.$pcCombination_K, 'is_combined'=>true, 'combined_event'=>'OrderCommon.platformcommon_remove(this,\'oms\',\'rumall\',\''.$pcCombination_K.'\')');
					}
				}
			}
			echo OrderListV3Helper::getDropdownToggleHtml('卖家账号', $omsPlatformFinds);
			if(!empty($_REQUEST['selleruserid_combined'])){
				echo '<span onclick="OrderCommon.order_platform_find(\'oms\',\'rumall\',\'\')" class="glyphicon glyphicon-remove" style="cursor:pointer;font-size: 10px;line-height: 20px;color: red;margin-right:5px;" aria-hidden="true" data-toggle="tooltip" data-placement="top" data-html="true" title="" data-original-title="清空卖家账号查询条件"></span>';
			}
			//店铺查找代码 E
			?>
			...
			<div class="input-group iv-input">
		        <?php $sel = [
		        	'order_source_order_id'=>'Rumall订单号',
					'sku'=>'SKU',
	    	...
	    	-->
	    	<a target="_blank"  title="手工订单" class="iv-btn btn-important" href="/order/order/manual-order-box?platform=rumall" >手工订单</a>
			
	    	<!----------------------------------------------------------- 同步情况 ----------------------------------------------------------->
			<?php if (!empty($_REQUEST['menu_select']) && $_REQUEST['menu_select'] =='all'):?>
			<div class="pull-right" style="height: 40px;">
				<?=Html::button(TranslateHelper::t('同步情况'),['class'=>"iv-btn btn-important",'onclick'=>"OrderCommon.showAccountSyncInfo(this);",'name'=>'btn_account_sync_info','data-url'=>'/order/rumall-order/order-sync-info'])?>
			</div>
			<?php endif;?>
	    	<!----------------------------------------------------------- 常用筛选 ----------------------------------------------------------->
			<!--  
			...
			<?=Html::dropDownList('order_source_status',@$_REQUEST['order_source_status'],$order_source_status_search,['class'=>'iv-input','prompt'=>'Rumall状态','id'=>'order_source_status','style'=>'width:200px;margin:0px'])?>
			
			 <?php 
			    $PayOrderTypeList = [''=>'已付款订单类型'];
...
// 			echo "&nbsp;";
			...
			$PayBtnHtml = Html::button('指定运输服务',['class'=>"iv-btn btn-important",'onclick'=>"orderCommonV3.oaction('changeWHSM');"])."&nbsp;";
	...
	$.ajax({
		...
		success: function (result) {
			...
			bootbox.dialog({
				...
				title: Translator.t('Rumall订单监控面板'),
				message: result,
				closeButton:false,
				buttons:{
