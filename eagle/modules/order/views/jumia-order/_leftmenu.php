...
TranslateHelper::t('所有订单')=>[
	'url'=>Url::to(['/order/jumia-order/list'])."?menu_select=all"
],
];
if (!empty($counter['custom_condition'])){
	...
	foreach($counter['custom_condition'] as $custom_condition_name=>$thisCondition):		
		$thisToUrl = Url::to(['/order/jumia-order/list']);
...
$deliveryMenu = DeliveryHelper::getDeliveryMenuByPlatform('jumia');

$menu = [
	TranslateHelper::t('jumia业务待处理')=>[
		'icon'=>'icon-stroe',
		'items'=>[
				TranslateHelper::t('所有订单')=>[
					'url'=>Url::to(['/order/jumia-order/list'])."?menu_select=all"
...
// 					'url'=>Url::to(['/order/jumia-order/list','order_status'=>OdOrder::STATUS_NOPAY]),
// 					 'tabbar'=>$counter[OdOrder::STATUS_NOPAY]
// 					],
			TranslateHelper::t('已付款')=>[
					'url'=>Url::to(['/order/jumia-order/list','order_status'=>OdOrder::STATUS_PAY , 'pay_order_type'=>'all']),
					'tabbar'=>empty($counter[OdOrder::STATUS_PAY])?0:$counter[OdOrder::STATUS_PAY]
				],		
			TranslateHelper::t('发货中')=>[
// 					'url'=>DeliveryHelper::getDeliveryModuleUrl('jumia'),
// 					'target'=>'_blank',
