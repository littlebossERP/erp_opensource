...
<div>
	...
	<br>
	<div>
		<font color="#8b8b8b" font-size="12px">小老板订单流水号:<b><?=$order->order_id?></b>
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<div>
	...
	<tr style="background-color: #fff">
		<td style="background-color: #f4f9fc" ><b>eBay地址</b></td>
		<td style="border:1px solid #ddd"><?=$order->consignee?></td>
		<td style="border:1px solid #ddd"><?=$order->consignee_email?></td>
		<td style="border:1px solid #ddd"><?=$order->consignee_country?></td>
		<td style="border:1px solid #ddd"><?=$order->consignee_province?></td>
		<td style="border:1px solid #ddd"><?=$order->consignee_city?></td>
		<td style="border:1px solid #ddd"><?=$order->consignee_address_line1?></td>
		<td style="border:1px solid #ddd"><?=$order->consignee_address_line2?></td>
		<td style="border:1px solid #ddd"><?=$order->consignee_postal_code?></td>
		<td style="border:1px solid #ddd"><?=$order->consignee_phone?></td>
	</tr>
	<!-- paypal地址信息 -->
	<?php if (!is_null($paypal)):?>
	<tr style="background-color: #fff">
		<td style="background-color: #f4f9fc" ><b>paypal地址</b></td>
		<td style="border:1px solid #ddd"><?=$paypal->shiptoname?></td>
		<td style="border:1px solid #ddd"><?=$paypal->email?></td>
		<td style="border:1px solid #ddd"><?=$paypal->shiptocountrycode?></td>
		<td style="border:1px solid #ddd"><?=$paypal->shiptostate?></td>
		<td style="border:1px solid #ddd"><?=$paypal->shiptocity?></td>
		<td style="border:1px solid #ddd"><?=$paypal->shiptostreet?></td>
		<td style="border:1px solid #ddd"><?=$paypal->shiptostreet2?></td>
		<td style="border:1px solid #ddd"><?=$paypal->shiptozip?></td>
	...
	<tr>
		...
		<td>
			<?=Html::dropDownList('consignee_country_code',$order->consignee_country_code,$countrys,['prompt'=>'国家','style'=>'width:100px'])?><?=Html::textInput('consignee_country',$order->consignee_country)?>
		</td>
...
<div>
	...
	<tr>
		...
		<td style="border:1px solid #ddd"><?=Html::textInput('item[order_source_srn][]',$item->order_source_srn)?></td>
...
<tr id="new" style="display:none">
		...
		<td style="border:1px solid #ddd"><?=Html::textInput('item[order_source_srn][]')?></td>
