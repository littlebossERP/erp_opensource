...
if(isset($tmpCustomsort['fulfill_deadline']))
...
//自动刷新左侧菜单统计数
$this->registerJs("OMSLeftMenuAutoLoad();" , \yii\web\View::POS_READY);
?>
...
<!-- oms 2.1 nav end  -->
	...
	<!-- ----------------------------------已付款订单分类筛选 start--------------------------------------------------------------------------------------------- -->	
		...
		echo OrderFrontHelper::displayOrderPaidProcessHtml($counter,'customized',[(string)OdOrder::PAY_PENDING]);
	...
	<!-- ----------------------------------搜索区域 start--------------------------------------------------------------------------------------------- -->	
	<div>
		<form class="form-inline" id="form1" name="form1" action="/order/customized-order/list" method="post">
			...
			<div style="margin:10px 0px 0px 0px">
				...
				if(count($selleruserids) > 0){
					...
					foreach ($selleruserids as $tmp_selleruserKey => $tmp_selleruserid){
						$omsPlatformFinds[] = array('event'=>'OrderCommon.order_platform_find(\'oms\',\'customized\',\''.$tmp_selleruserKey.'\')','label'=>$tmp_selleruserid);
					}
					
					$pcCombination = OrderListV3Helper::getPlatformCommonCombination(array('type'=>'oms','platform'=>'customized'));
					if(count($pcCombination) > 0){
						foreach ($pcCombination as $pcCombination_K => $pcCombination_V){
							$omsPlatformFinds[] = array('event'=>'OrderCommon.order_platform_find(\'oms\',\'customized\',\''.'com-'.$pcCombination_K.'-com'.'\')',
								'label'=>'组合-'.$pcCombination_K, 'is_combined'=>true, 'combined_event'=>'OrderCommon.platformcommon_remove(this,\'oms\',\'customized\',\''.$pcCombination_K.'\')');
						}
					}
				}
				echo OrderListV3Helper::getDropdownToggleHtml('卖家账号', $omsPlatformFinds);
				if(!empty($_REQUEST['selleruserid_combined'])){
					echo '<span onclick="OrderCommon.order_platform_find(\'oms\',\'customized\',\'\')" class="glyphicon glyphicon-remove" style="cursor:pointer;font-size: 10px;line-height: 20px;color: red;margin-right:5px;" aria-hidden="true" data-toggle="tooltip" data-placement="top" data-html="true" title="" data-original-title="清空卖家账号查询条件"></span>';
				}
				//店铺查找代码 E
				?>
	    		...
	    		<a target="_blank" title="手工订单" class="iv-btn btn-important" href="/order/order/manual-order-box?platform=customized">手工订单</a>
			</div>
   		...
   		<div style="display: inline-block;width:100%;margin:20px 0px 0px 0px;">
...
//  			echo "&nbsp;";
		...
		if(isset($doarr['setSyncShipComplete']))
			unset($doarr['setSyncShipComplete']);
...
<!-- ----------------------------------导入跟踪号start--------------------------------------------------------------------------------------------- -->	
		...
		if (isset($_REQUEST['order_status']) && $_REQUEST['order_status'] =='200' ){
			...
			echo Html::button(TranslateHelper::t('导入跟踪号'),['class'=>"iv-btn btn-important",'onclick'=>"OrderCommon.importTrackNoBox('customized')"]);
			echo "</div>";
		}
		?>
<!-- ----------------------------------导入跟踪号end--------------------------------------------------------------------------------------------- -->			
...
<!-- ----------------------------------分页end--------------------------------------------------------------------------------------------- -->	
	...
	<ul>
		...
		<li style="float: left;line-height: 60px;"><?php echo OrderFrontHelper::displayOrderPaidProcessHtml($counter,'customized')?></li>
	</ul>
...
function doaction(val){
	...
	switch(val){
		...
		case 'signpayed':
			...
			$.post('<?=Url::to(['/order/customized-order/signpayed'])?>',{orders:idstr},function(result){
				bootbox.alert(result);
				location.reload();
			});
			break;
...
function doactionone(val,orderid){
	...
	switch(val){
		...
		case 'editOrder':
			idstr = [];
			idstr.push(orderid);
			window.open("<?=Url::to(['/order/customized-order/edit'])?>"+"?orderid="+orderid,'_blank')
			break;
		case 'suspendDelivery':
			idstr = [];
			idstr.push(orderid);
		...
		case 'refreshOrder':
			idstr = [];
			idstr.push(orderid);
			$.post('<?=Url::to(['/order/customized-order/refreshorder'])?>',{orders:idstr},function(result){
				bootbox.alert(result);
				location.reload();
			});
			break; 
		...
		case 'signshipped':
			...
			break;
		case 'signpayed':
			$.post('<?=Url::to(['/order/customized-order/signpayed'])?>',{orders:orderid},function(result){
				bootbox.alert(result);
				location.reload();
			});
			break;
...
function movestatus(val){
	...
	$.post('<?=Url::to(['/order/customized-order/movestatus'])?>',{orderids:idstr,status:val},function(result){
		bootbox.alert('操作已成功');
	});
}
//上传物流单号
function importordertracknum(){
	if($("#order_tracknum").val()){
		$.ajaxFileUpload({  
			 url:'<?=Url::to(['/order/customized-order/importordertracknum'])?>',
...
function changemanual(orderid,obj){
	$.post('<?=Url::to(['/order/customized-order/changemanual'])?>',{orderid:orderid},function(result){
		if(result == 'success'){
			bootbox.alert('操作已成功');
			var str;
			str=$(obj).html();
...
function setusertab(orderid,tabobj){
	...
	$.post('<?=Url::to(['/order/customized-order/setusertab'])?>',{orderid:orderid,tabid:tabid},function(result){
		if(result == 'success'){
			bootbox.alert('操作已成功');
		}else{
			bootbox.alert(result);
...
function saveCustomCondition(modalbox , filter_name){
	$.ajax({
		type: "POST",
			dataType: 'json',
			url:'/order/customized-order/append-custom-condition?custom_name='+filter_name, 
...
function OMSLeftMenuAutoLoad(){
	$.ajax({
		url: "/order/customized-order/left-menu-auto-load",
		type: 'post',
		dataType:"json",
		success: function(result) {
			var tmpLabelList = {'100':'未付款','200':'已付款','300':'发货中','500':'已完成','600':'已取消','601':'暂停发货','602':'缺货','all':'所有订单'};
