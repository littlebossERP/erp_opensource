...
function dofeedback(){
	...
	$('.result').each(function(){
		...
		$(document).queue('ajaxRequests',function(){
			...
			$.post('<?=Url::to(['/order/ebay-order/ajax-feedback'])?>',{orderid:$(obj).prev().attr('id'),feedbacktype:$('#feedbacktype_'+orderid).val(),feedbackval:$('#feedbackval_'+orderid).val()},function(result){
				if(result=='success'){
