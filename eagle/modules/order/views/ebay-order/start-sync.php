...
<form class="form-horizontal" style="width:100%;height:230px;">
    <div class="form-group">
    	<p></p>
        <label for="ebay_uid" class="col-xs-2 col-sm-2 col-lg-2" style="text-align:right;line-height:30px;min-width:80px;">选择店铺</label>
        <div class="col-xs-4 col-sm-4 col-lg-4" style="text-align:left;">
            <select class="p0 form-control" id="ebay_uid" style="margin-top:0;">
                <?php if(count($accounts)> 1): ?>
                    <option value="0">请选择ebay店铺</option>
                <?php endif;?>
                <?php foreach($accounts as $vs):?>
                    <option value="<?=$vs->ebay_uid?>" data-type="ebay_uid" <?php if(count($accounts) == 1): ?> selected="selected" <?php endif;?>><?=(($vs->store_name != '') ? $vs->store_name : $vs->selleruserid) ?></option>
                <?php endforeach;?>
            </select>
        </div>

        <label for="sync_order_time" class="col-xs-2 col-sm-2 col-lg-2" style="text-align:right;line-height:30px;min-width:80px;padding:0px">订单平台时间<span qtipkey="cd_manual-sync-order_time"></label>
        <div class="col-xs-4 col-sm-4 col-lg-4" style="text-align:left;">
            <input type="date" class="p0 form-control" id="sync_order_time" style="margin-top:0;" value="<?=date("Y-m-d",time()-3600*24) ?>" required min="<?=date("Y-m-d",time()-3600*24*7) ?>" >
        </div>
    </div>
    <div class="form-group">
