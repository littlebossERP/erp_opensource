...
<div class="content-wrapper container-fluid" >
	<?php $problemAccounts = PlatformAccountApi::getUserNeweggProblemAccounts($uid); ?>
	<?php if(!empty($problemAccounts['order_retrieve_failed'])){ $problemAccountNames=[];?>
	<?php foreach ($problemAccounts['order_retrieve_failed'] as $account){
		$problemAccountNames[] = $account['store_name'];
	}?>
	<!-- 账号异常提示区域 -->
	<div class="alert alert-danger" role="alert" style="width:100%;">
		<span>您绑定的Newegg账号：<?=implode(' , ', $problemAccountNames) ?> 同步订单出现问题，请联系客服。</span>
	</div>
	<?php } ?>
<!-- oms 2.1 nav start  -->
	<!-- 
...
<!-- oms 2.1 nav end  -->
	...
	<!-- ----------------------------------已付款订单分类筛选 start--------------------------------------------------------------------------------------------- -->	
		...
		echo OrderFrontHelper::displayOrderPaidProcessHtml($counter,'newegg',[(string)OdOrder::PAY_PENDING]);
	...
	<!-- ----------------------------------搜索区域 start--------------------------------------------------------------------------------------------- -->	
	<div>
		<form class="form-inline" id="form1" name="form1" action="/order/newegg-order/list" method="post">
...
// 			$omsPlatformFinds[] = array('event'=>'OrderCommon.order_platform_find(\'oms\',\'newegg\',\'\')','label'=>'全部');
			
			if(count($selleruserids) > 0){
				$selleruserids['select_shops_xlb'] = '选择账号';
	
				foreach ($selleruserids as $tmp_selleruserKey => $tmp_selleruserid){
					$omsPlatformFinds[] = array('event'=>'OrderCommon.order_platform_find(\'oms\',\'newegg\',\''.$tmp_selleruserKey.'\')','label'=>$tmp_selleruserid);
				}
				
				$pcCombination = OrderListV3Helper::getPlatformCommonCombination(array('type'=>'oms','platform'=>'newegg'));
				if(count($pcCombination) > 0){
					foreach ($pcCombination as $pcCombination_K => $pcCombination_V){
						$omsPlatformFinds[] = array('event'=>'OrderCommon.order_platform_find(\'oms\',\'newegg\',\''.'com-'.$pcCombination_K.'-com'.'\')',
							'label'=>'组合-'.$pcCombination_K, 'is_combined'=>true, 'combined_event'=>'OrderCommon.platformcommon_remove(this,\'oms\',\'newegg\',\''.$pcCombination_K.'\')');
					}
				}
			}
			echo OrderListV3Helper::getDropdownToggleHtml('卖家账号', $omsPlatformFinds);
			if(!empty($_REQUEST['selleruserid_combined'])){
				echo '<span onclick="OrderCommon.order_platform_find(\'oms\',\'newegg\',\'\')" class="glyphicon glyphicon-remove" style="cursor:pointer;font-size: 10px;line-height: 20px;color: red;margin-right:5px;" aria-hidden="true" data-toggle="tooltip" data-placement="top" data-html="true" title="" data-original-title="清空卖家账号查询条件"></span>';
			}
			//店铺查找代码 E
			?>
			...
			<div class="input-group iv-input">
		        <?php $sel = [
		        	'order_source_order_id'=>'newegg订单号',
					'sku'=>'SKU',
					'tracknum'=>'物流号',
					'buyeid'=>'买家账号',
		        	'consignee'=>'买家姓名',
	    	...
	    	<a target="_blank" title="手工订单" class="iv-btn btn-important" href="/order/order/manual-order-box?platform=newegg">手工订单</a>
			
	    				
	    	<!----------------------------------------------------------- 同步情况 ----------------------------------------------------------->
			<?php if (!empty($_REQUEST['menu_select']) && $_REQUEST['menu_select'] =='all'):?>
			<div class="pull-right" style="height: 40px;">
				<?=Html::button(TranslateHelper::t('同步情况'),['class'=>"iv-btn btn-important",'onclick'=>"OrderCommon.showAccountSyncInfo(this);",'name'=>'btn_account_sync_info','data-url'=>'/order/newegg-order/order-sync-info'])?>
			</div>
			<?php endif;?>
			...
			foreach (array_keys(SaasNeweggAutoFetchApiHelper::$NEWEGG_EAGLE_ORDER_STATUS_MAP) as $source_status){
				$search_order_source_status[$source_status] = $source_status;
			}
			?>
			
			<?=Html::dropDownList('order_source_status',@$_REQUEST['order_source_status'],$search_order_source_status,['class'=>'iv-input','prompt'=>'newegg状态','id'=>'order_source_status','style'=>'width:200px;margin:0px'])?>
			...
			if (@$_REQUEST['order_status'] != OdOrder::STATUS_CANCEL){
				\eagle\modules\order\helpers\OrderFrontHelper::displayOrderSyncShipStatusToolbar(@$_REQUEST['order_sync_ship_status'],'newegg');
			}
			?>
			<div style="margin:20px 0px 0px 0px">
...
// 			echo Html::button('取消订单',['class'=>"iv-btn btn-important",'onclick'=>"javascript:doaction('cancelorder');"]);
		...
		<?php 
			...
			foreach ($doarr as $doarrKey => $doarrVal){
				if($doarrKey == '')
					continue;
				$doDownListArr[$doarrKey] = array('event'=>'orderCommonV3.doaction3(\''.$doarrKey.'\',\''.'newegg'.'\')','label'=>$doarrVal);
			}
...
<!-- ----------------------------------导入跟踪号start--------------------------------------------------------------------------------------------- -->	
		...
		if (isset($_REQUEST['order_status']) && $_REQUEST['order_status'] =='200' ){
			...
			echo Html::button(TranslateHelper::t('导入跟踪号'),['class'=>"iv-btn btn-important",'onclick'=>"OrderCommon.importTrackNoBox('newegg')"]);
			echo "</div>";
		}
		?>
<!-- ----------------------------------导入跟踪号end--------------------------------------------------------------------------------------------- -->			
...
<!-- ----------------------------------分页end--------------------------------------------------------------------------------------------- -->	
	...
	<ul>
		...
		<li style="float: left;line-height: 60px;"><?php echo OrderFrontHelper::displayOrderPaidProcessHtml($counter,'newegg')?></li>
	</ul>
...
function doaction(val){
	...
	switch(val){
		...
		case 'refreshOrder':
			...
			$.post('<?=Url::to(['/order/newegg-order/refreshorder'])?>',{orders:thisOrderList},function(result){
		...
		case 'signpayed':
			...
			$.post('<?=Url::to(['/order/newegg-order/signpayed'])?>',{orders:idstr},function(result){
				bootbox.alert(result);
				location.reload();
			});
			break;
...
function doactionone(val,orderid){
	...
	switch(val){
		...
		case 'editOrder':
			idstr = [];
			idstr.push(orderid);
			window.open("<?=Url::to(['/order/newegg-order/edit'])?>"+"?orderid="+orderid,'_blank')
			break;
		case 'suspendDelivery':
			idstr = [];
			idstr.push(orderid);
		...
		case 'refreshOrder':
			idstr = [];
			idstr.push(orderid);
			$.post('<?=Url::to(['/order/newegg-order/refreshorder'])?>',{orders:idstr},function(result){
				bootbox.alert(result);
				location.reload();
			});
			break; 
		...
		case 'signshipped':
			...
			break;
		case 'signpayed':
			$.post('<?=Url::to(['/order/newegg-order/signpayed'])?>',{orders:orderid},function(result){
				bootbox.alert(result);
				location.reload();
			});
			break;
...
function movestatus(val){
	...
	$.post('<?=Url::to(['/order/newegg-order/movestatus'])?>',{orderids:idstr,status:val},function(result){
		bootbox.alert('操作已成功');
	});
}
//上传物流单号
function importordertracknum(){
	if($("#order_tracknum").val()){
		$.ajaxFileUpload({  
			 url:'<?=Url::to(['/order/newegg-order/importordertracknum'])?>',
...
function changemanual(orderid,obj){
	$.post('<?=Url::to(['/order/newegg-order/changemanual'])?>',{orderid:orderid},function(result){
		if(result == 'success'){
			bootbox.alert('操作已成功');
			var str;
			str=$(obj).html();
...
function setusertab(orderid,tabobj){
	...
	$.post('<?=Url::to(['/order/newegg-order/setusertab'])?>',{orderid:orderid,tabid:tabid},function(result){
		if(result == 'success'){
			bootbox.alert('操作已成功');
		}else{
			bootbox.alert(result);
...
function updatedesc(orderid,obj){
	var desc=$(obj).prev().find('font[color="red"]');
...
function manualSync(sellerloginid,startdate,enddate,synctype){
	$.showLoading();
	$.post('<?=Url::to(['/order/newegg-order/monual-sync'])?>',{sellerloginid:sellerloginid,startdate:startdate,enddate:enddate,synctype:synctype},function(result){
		$.hideLoading();
...
function saveCustomCondition(modalbox , filter_name){
	$.ajax({
		type: "POST",
			dataType: 'json',
			url:'/order/newegg-order/append-custom-condition?custom_name='+filter_name, 
...
function showExtendsBuyerAcceptGoodsTimeBoxOMS(orderIdList){
	$.ajax({
		type: "GET",
			dataType: 'html',
			url:'/order/newegg-order/show-extends-buyer-accept-goods-time-box', 
...
function ExtendsBuyerAcceptGoodsTimeOMS(){
	...
	$.ajax({
		type: "POST",
			dataType: 'json',
			url:'/order/newegg-order/extends-buyer-accept-goods-time', 
...
// 		type: "POST",
// 			dataType: 'text',
// 			url:'/order/newegg-order/clear-left-menu-cache', 
// 			success: function (result) {
// 				location.reload();
// 			},
// 			error: function(){
...
function showDashBoard(autoShow){
	...
	$.ajax({
		type: "GET",
		url:'/order/newegg-order/user-dash-board?autoShow='+autoShow, 
		success: function (result) {
			$.hideLoading();
...
function requestGenerateDashBoardData(){
	...
	$.ajax({
		type: "GET",
		url:'/order/newegg-order/genrate-user-dash-board', 
		success: function (result) {
			if ($('#dash-board-enter').css('display') == 'none'){
				$('#dash-board-enter').toggle(1000);
			}
