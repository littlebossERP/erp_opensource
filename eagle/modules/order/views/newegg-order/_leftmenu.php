...
TranslateHelper::t('所有订单')=>[
	'url'=>Url::to(['/order/newegg-order/list'])
],
];
if (!empty($counter['custom_condition'])){
	...
	foreach($counter['custom_condition'] as $custom_condition_name=>$thisCondition):		
		$thisToUrl = Url::to(['/order/newegg-order/list']);
...
$deliverMenu = DeliveryHelper::getDeliveryMenuByPlatform('newegg');

$menu = [
	TranslateHelper::t('newegg业务待处理')=>[
		'icon'=>'icon-stroe',
		'items'=>[
			TranslateHelper::t('所有订单')=>[
				'url'=>Url::to(['/order/newegg-order/list','menu_select'=>'all'])
			],
		]
	],
...
// 					'url'=>Url::to(['/order/newegg-order/list','order_status'=>OdOrder::STATUS_NOPAY]),
// 					 'tabbar'=>$counter[OdOrder::STATUS_NOPAY]
// 					],
			TranslateHelper::t('已付款')=>[
				'url'=>Url::to(['/order/newegg-order/list','order_status'=>OdOrder::STATUS_PAY , 'pay_order_type'=>'all']),
