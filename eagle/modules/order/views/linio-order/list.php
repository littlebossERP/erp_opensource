...
<div class="content-wrapper container-fluid" >
	...
	<div class="alert alert-danger" role="alert" style="width:100%;">
		<span>您的linio账号有<?=$SignShipWarningCount?>张订单通知平台发货失败，请尽快确认！<a href="/order/linio-order/list?order_sync_ship_status=F">点击</a><br>
	</div>
	<?php } ?>

	<!-- ----------------------------------搜索区域 start--------------------------------------------------------------------------------------------- -->	
	<div>
		<form class="form-inline" id="form1" name="form1" action="/order/linio-order/list" method="post">
...
// 			$omsPlatformFinds[] = array('event'=>'OrderCommon.order_platform_find(\'oms\',\'linio\',\'\')','label'=>'全部');
			
			if(count($selleruserids) > 0){
				$selleruserids['select_shops_xlb'] = '选择账号';
	
				foreach ($selleruserids as $tmp_selleruserKey => $tmp_selleruserid){
					$omsPlatformFinds[] = array('event'=>'OrderCommon.order_platform_find(\'oms\',\'linio\',\''.$tmp_selleruserKey.'\')','label'=>$tmp_selleruserid);
				}
				
				$pcCombination = OrderListV3Helper::getPlatformCommonCombination(array('type'=>'oms','platform'=>'linio'));
				if(count($pcCombination) > 0){
					foreach ($pcCombination as $pcCombination_K => $pcCombination_V){
						$omsPlatformFinds[] = array('event'=>'OrderCommon.order_platform_find(\'oms\',\'linio\',\''.'com-'.$pcCombination_K.'-com'.'\')',
							'label'=>'组合-'.$pcCombination_K, 'is_combined'=>true, 'combined_event'=>'OrderCommon.platformcommon_remove(this,\'oms\',\'linio\',\''.$pcCombination_K.'\')');
					}
				}
			}
			echo OrderListV3Helper::getDropdownToggleHtml('卖家账号', $omsPlatformFinds);
			if(!empty($_REQUEST['selleruserid_combined'])){
				echo '<span onclick="OrderCommon.order_platform_find(\'oms\',\'linio\',\'\')" class="glyphicon glyphicon-remove" style="cursor:pointer;font-size: 10px;line-height: 20px;color: red;margin-right:5px;" aria-hidden="true" data-toggle="tooltip" data-placement="top" data-html="true" title="" data-original-title="清空卖家账号查询条件"></span>';
			}
			//店铺查找代码 E
			?>
			...
			<div class="input-group iv-input">
		        <?php $sel = [
		        	'order_source_order_id'=>'linio订单号',
					'sku'=>'SKU',
					'tracknum'=>'物流号',
					'buyeid'=>'买家账号',
		        	'consignee'=>'买家姓名',
	    	...
	    	<a target="_blank"  title="手工订单" class="iv-btn btn-important" href="/order/order/manual-order-box?platform=linio" >手工订单</a>
			...
			<?=Html::dropDownList('order_source_status',@$_REQUEST['order_source_status'],$search_order_source_status,['class'=>'iv-input','prompt'=>'linio状态','id'=>'order_source_status','style'=>'width:200px;margin:0px'])?>
			...
			if (@$_REQUEST['order_status'] != OdOrder::STATUS_CANCEL){
				\eagle\modules\order\helpers\OrderFrontHelper::displayOrderSyncShipStatusToolbar(@$_REQUEST['order_sync_ship_status'],'linio');
			}
			?>
...
// 			echo Html::dropDownList('do','',$doarr,['onchange'=>"orderCommonV3.doaction2(this);",'class'=>'iv-input do','style'=>'width:200px;margin:0px']);
			$doDownListHtml = \eagle\modules\order\helpers\OrderListV3Helper::getDropdownToggleHtml('批量操作', $doarr, 'orderCommonV3.doaction3');
			echo $doDownListHtml;
		}
		?>
		...
		$excelDownListHtml = \eagle\modules\order\helpers\OrderListV3Helper::getDropdownToggleHtml('导出订单', $excelActionItems);
		...
		<?php
			if(@$_REQUEST['order_status'] == OdOrder::STATUS_PAY){?>
				<div class="pull-right" style="height: 40px;">
					<button type="button" class="iv-btn btn-important" onclick="OrderCommon.importTrackNoBox('linio')">导入跟踪号</button>	
				</div>
		<?php	}
		?>
<!-- ----------------------------------导入跟踪号end--------------------------------------------------------------------------------------------- -->			
...
function doaction(val){
	...
	switch(val){
		...
		case 'refreshOrder':
			...
			$.post('<?=Url::to(['/order/linio-order/refreshorder'])?>',{orders:thisOrderList},function(result){
		...
		case 'signshipped':
			...
			document.a.action="<?=Url::to(['/order/linio-order/signshipped'])?>";
			document.a.submit();
			document.a.action="";
			break;
		case 'signwaitsend':
		...
		case 'signpayed':
			...
			$.post('<?=Url::to(['/order/linio-order/signpayed'])?>',{orders:idstr},function(result){
				bootbox.alert(result);
				location.reload();
			});
			break;
...
function doactionone(val,orderid){
	...
	switch(val){
		...
		case 'editOrder':
			idstr = [];
			idstr.push(orderid);
			window.open("<?=Url::to(['/order/linio-order/edit'])?>"+"?orderid="+orderid,'_blank')
			break;
		case 'suspendDelivery':
			idstr = [];
			idstr.push(orderid);
		...
		case 'refreshOrder':
			idstr = [];
			idstr.push(orderid);
			$.post('<?=Url::to(['/order/linio-order/refreshorder'])?>',{orders:idstr},function(result){
				bootbox.alert(result);
				location.reload();
			});
			break; 
		...
		case 'checkorder':
			...
			break;
		case 'signshipped':
			window.open("<?=Url::to(['/order/linio-order/signshipped'])?>"+"?order_id="+orderid,'_blank')
			break;
		case 'signpayed':
			$.post('<?=Url::to(['/order/linio-order/signpayed'])?>',{orders:orderid},function(result){
				bootbox.alert(result);
				location.reload();
			});
			break;
...
function movestatus(val){
	...
	$.post('<?=Url::to(['/order/linio-order/movestatus'])?>',{orderids:idstr,status:val},function(result){
		bootbox.alert('操作已成功');
	});
}
//上传物流单号
function importordertracknum(){
	if($("#order_tracknum").val()){
		$.ajaxFileUpload({  
			 url:'<?=Url::to(['/order/linio-order/importordertracknum'])?>',
...
function changemanual(orderid,obj){
	$.post('<?=Url::to(['/order/linio-order/changemanual'])?>',{orderid:orderid},function(result){
		if(result == 'success'){
			bootbox.alert('操作已成功');
			var str;
			str=$(obj).html();
...
function setusertab(orderid,tabobj){
	...
	$.post('<?=Url::to(['/order/linio-order/setusertab'])?>',{orderid:orderid,tabid:tabid},function(result){
		if(result == 'success'){
			bootbox.alert('操作已成功');
		}else{
			bootbox.alert(result);
...
function manualSync(sellerloginid,startdate,enddate,synctype){
	$.showLoading();
	$.post('<?=Url::to(['/order/linio-order/monual-sync'])?>',{sellerloginid:sellerloginid,startdate:startdate,enddate:enddate,synctype:synctype},function(result){
		$.hideLoading();
...
function saveCustomCondition(modalbox , filter_name){
	$.ajax({
		type: "POST",
			dataType: 'json',
			url:'/order/linio-order/append-custom-condition?custom_name='+filter_name, 
...
function showExtendsBuyerAcceptGoodsTimeBoxOMS(orderIdList){
	$.ajax({
		type: "GET",
			dataType: 'html',
			url:'/order/linio-order/show-extends-buyer-accept-goods-time-box', 
...
function ExtendsBuyerAcceptGoodsTimeOMS(){
	...
	$.ajax({
		type: "POST",
			dataType: 'json',
			url:'/order/linio-order/extends-buyer-accept-goods-time', 
...
// 		type: "POST",
// 			dataType: 'text',
// 			url:'/order/linio-order/clear-left-menu-cache', 
// 			success: function (result) {
// 				location.reload();
// 			},
// 			error: function(){
...
function showDashBoard(autoShow){
	...
	$.ajax({
		type: "GET",
		url:'/order/linio-order/user-dash-board?autoShow='+autoShow, 
		success: function (result) {
			$.hideLoading();
...
function requestGenerateDashBoardData(){
	...
	$.ajax({
		type: "GET",
		url:'/order/linio-order/genrate-user-dash-board', 
		success: function (result) {
			if ($('#dash-board-enter').css('display') == 'none'){
				$('#dash-board-enter').toggle(1000);
			}
