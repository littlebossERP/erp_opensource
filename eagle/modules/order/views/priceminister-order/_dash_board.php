	...
	<table style="width:100%">
		...
		<!-- 统计与错误提示列 -->
			<td width="<?=empty($advertData)?'100%':'75%' ?>" style="float:left;">
				...
				$reminder=[];
				if(!empty($uid)){
					$ret=PriceministerOrderInterface::getPriceministerReminder($uid);
					if($ret['success'] && !empty($ret['remind']))
						$reminder = $ret['remind'];
				}
				if(!empty($reminder)){
				?>
					<div id="pm-oms-reminder">
						<div id="pm-oms-reminder-content" class="bg-warning">
							<div style="float:left;width:100%;clear:both;">
								<?php if(!empty($reminder['YiFuKuan'])): ?>
								<span style="width:100%;"><a href="<?=Url::to(['/order/priceministe-order/list','weird_status'=>'wfs'])?>" target="_blank">您有 <b style="font-weight:600;"><?=$reminder['YiFuKuan'] ?> </b>单已付款订单超过2天未处理</a></span><span qtipkey="oms_order_status_prolonged_paid"></span><br>
								<?php endif;
									  if(!empty($reminder['FaHuoZhong'])):?>
								<span style="width:100%;"><a href="<?=Url::to(['/order/priceministe-order/list','weird_status'=>'wfd'])?>" target="_blank">您有 <b style="font-weight:600;"><?=$reminder['FaHuoZhong'] ?> </b>单发货中订单超过2天未处理</a></span><span qtipkey="oms_order_status_prolonged_in delivery"></span><br>
								<?php endif;
									  if(!empty($reminder['YiFaHuo'])):?>
								<span style="width:100%;"><a href="<?=Url::to(['/order/priceministe-order/list','weird_status'=>'wfss'])?>" target="_blank">您有 <b style="font-weight:600;"><?=$reminder['YiFaHuo'] ?> </b>单已发货单超过2天未处理</a></span><span qtipkey="oms_order_status_prolonged_shipped"></span><br>
								<?php endif;
									  if(!empty($reminder['ZhuangTaiYiChang'])):?>
								<span style="width:100%;"><a href="<?=Url::to(['/order/priceministe-order/list','weird_status'=>'sus'])?>" target="_blank">您有 <b style="font-weight:600;"><?=$reminder['ZhuangTaiYiChang'] ?> </b>单订单小老板状态与PM后台状态不匹配。</a></span><span qtipkey="pm_order_weird_status_sus"></span><br>
								<?php endif;
									  if(!empty($reminder['WuTianWeiShangWang'])):?>
								<span style="width:100%;"><a href="<?=Url::to(['/order/priceministe-order/list','weird_status'=>'tuol'])?>" target="_blank">您有 <b style="font-weight:600;"><?=$reminder['WuTianWeiShangWang'] ?> </b>单订单超过五天物流仍未上网。</a></span><span qtipkey="pm_order_weird_status_tuol"></span><br>
								<?php endif;?>
							</div>
						</div>
					</div>
				...
				<div style="font-size: 12px;">
					<?php 
					$AccountProblems = PriceministerOrderInterface::getUserAccountProblems($uid);
					if(!empty($AccountProblems['initial_failed'])){
						foreach ($AccountProblems['initial_failed'] as $initial_failed_account){
					?>
					<span style="width:100%;float:left;clear:both;">您绑定的Priceminister账号：<?=$initial_failed_account['store_name'] ?> 初次绑定时获取一个月前订单失败，请联系技术人员查询原因</span>
					<?php }
					}
					...
					<span style="width:100%;float:left;clear:both;">
						您绑定的Priceminister账号：<?=$token_expired['store_name'] ?> 授权已经过期！&#8594;
					...
					?>
					<span style="width:100%;float:left;clear:both;">您绑定的Priceminister账号：<?=$retrieve_failed['store_name'] ?> 获取最近订单失败：<?=$retrieve_failed['order_retrieve_message'] ?>，请联系技术人员查询原因</span>
					<?php }
					}
					...
					?>
					<span style="width:100%;float:left;clear:both;">
						您绑定的Priceminister账号：<?=$unActive['store_name'] ?> 未开启同步！&#8594;
						<a href="<?=$platformUr ?>" target="_blank">编辑绑定设置</a>
					</span>
					<?php }
					}
		...
		<!-- 推荐/新功能列 -->
			<td width="<?=empty($advertData)?'0px':'25%' ?>" style="margin-top: 10px;float:right;display:<?=empty($advertData)?'none':'block' ?>">
				<div style="width:100%;overflow-y: auto;" id="Priceminister_oms_adver">
	...
	$(".bootbox.modal").animate(
		{
	 	...
	 	function(){
			...
			$.post('/order/priceminister-order/hide-dash-board',{},function(){return true;});
		}
	);
}
</script>