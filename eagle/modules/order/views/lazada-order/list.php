...
// oms 2.1 的订单状态
$orderStatus21 = OdOrder::getOrderStatus('oms21');
...
$this->title = "Lazada 订单列表";

$this->registerJsFile(\Yii::getAlias('@web')."/js/lib/clipboard.min.js?v=".\eagle\modules\order\helpers\OrderListV3Helper::$OrderCommonJSV3, ['depends' => ['yii\web\JqueryAsset']]);
...
$this->registerJsFile(\Yii::getAlias('@web')."/js/project/order/copy.js?v=".\eagle\modules\order\helpers\OrderListV3Helper::$OrderCommonJSV3, ['depends' => ['yii\web\JqueryAsset']]);
?>
...
<div class="content-wrapper container-fluid" >
	<!-- ----------------------------------搜索区域 start--------------------------------------------------------------------------------------------- -->	
	<div>
		<form class="form-inline" id="form1" name="form1" action="/order/lazada-order/list" method="post">
...
// 			$omsPlatformFinds[] = array('event'=>'OrderCommon.order_platform_find(\'oms\',\'lazada\',\'\')','label'=>'全部');
			
			if(count($selleruserids) > 0){
				$selleruserids['select_shops_xlb'] = '选择账号';
	
				foreach ($selleruserids as $tmp_selleruserKey => $tmp_selleruserid){
					$omsPlatformFinds[] = array('event'=>'OrderCommon.order_platform_find(\'oms\',\'lazada\',\''.$tmp_selleruserKey.'\')','label'=>$tmp_selleruserid);
				}
				
				$pcCombination = OrderListV3Helper::getPlatformCommonCombination(array('type'=>'oms','platform'=>'lazada'));
				if(count($pcCombination) > 0){
					foreach ($pcCombination as $pcCombination_K => $pcCombination_V){
						$omsPlatformFinds[] = array('event'=>'OrderCommon.order_platform_find(\'oms\',\'lazada\',\''.'com-'.$pcCombination_K.'-com'.'\')',
							'label'=>'组合-'.$pcCombination_K, 'is_combined'=>true, 'combined_event'=>'OrderCommon.platformcommon_remove(this,\'oms\',\'lazada\',\''.$pcCombination_K.'\')');
					}
				}
			}
			echo OrderListV3Helper::getDropdownToggleHtml('卖家账号', $omsPlatformFinds);
			if(!empty($_REQUEST['selleruserid_combined'])){
				echo '<span onclick="OrderCommon.order_platform_find(\'oms\',\'lazada\',\'\')" class="glyphicon glyphicon-remove" style="cursor:pointer;font-size: 10px;line-height: 20px;color: red;margin-right:5px;" aria-hidden="true" data-toggle="tooltip" data-placement="top" data-html="true" title="" data-original-title="清空卖家账号查询条件"></span>';
			}
			//店铺查找代码 E
			?>
			...
			<div class="input-group iv-input">
		        <?php $sel = [
		        	'order_source_order_id'=>'Lazada订单号',
					'sku'=>'SKU',
					'product_name'=>'商品标题',
					'tracknum'=>'物流号',
					'buyeid'=>'买家账号',
	    	...
	    	<a id="simplesearch" href="#" style="font-size:12px;text-decoration:none;" onclick="OrderList.mutisearch();">高级搜索<span class="glyphicon glyphicon-menu-down"></span></a>
			<?=Html::submitButton('搜索',['class'=>"iv-btn btn-search btn-spacing-middle",'id'=>'search'])?>
	    	<a target="_blank"  title="手工订单" class="iv-btn btn-important" href="/order/order/manual-order-box?platform=lazada" >手工订单</a>
	    	

			<a id="sync-btn" href="sync-order-ready" target="_modal" title="同步订单" class="iv-btn btn-important hidden" auto-size style="color:white;">同步订单</a>
			<!----------------------------------------------------------- 常用筛选 ----------------------------------------------------------->
			...
			<div class="input-group"  name="div-select-nation"  data-role-id="0"  style='margin:0px'>
				<?=Html::textInput('country',@$_REQUEST['country'],['id'=>'country', 'class'=>'iv-input','placeholder'=>'收件人国家','style'=>'width:200px;margin:0px'])?>
			</div>
			...
			<?=Html::dropDownList('shipmethod',@$_REQUEST['shipmethod'],$carriers,['id'=>'shipmethod', 'class'=>'iv-input','prompt'=>'运输服务','id'=>'shipmethod','style'=>'width:200px;margin:0px'])?>
			...
			<?=Html::dropDownList('order_type',@$_REQUEST['order_type'],$order_types,['class'=>'iv-input','prompt'=>'Lazada订单类型','id'=>'order_type','style'=>'width:200px;margin:0px'])?>
			
			<?php 
    			$search_order_source_status = array();
    			foreach (array_keys(SaasLazadaAutoFetchApiHelper::$LAZADA_EAGLE_ORDER_STATUS_MAP) as $source_status){
    				$search_order_source_status[$source_status] = $source_status;
    			}
			?>
			<?=Html::dropDownList('order_source_status',@$_REQUEST['order_source_status'],$search_order_source_status,['class'=>'iv-input','prompt'=>'Lazada状态','id'=>'order_source_status','style'=>'width:200px;margin:0px'])?>
			...
			if (@$_REQUEST['order_status'] != OdOrder::STATUS_CANCEL){
				\eagle\modules\order\helpers\OrderFrontHelper::displayOrderSyncShipStatusToolbar(@$_REQUEST['order_sync_ship_status'],'lazada');
			}
			?>
...
// 			echo Html::dropDownList('do','',$doarr,['onchange'=>"orderCommonV3.doaction2(this);",'class'=>'iv-input do','style'=>'width:200px;margin:0px']);
			$doDownListArr = array();
			foreach ($doarr as $doarrKey => $doarrVal){
				if($doarrKey == '')
					continue;
				$doDownListArr[$doarrKey] = array('event'=>'orderCommonV3.doaction3(\''.$doarrKey.'\',\''.'lazada'.'\')','label'=>$doarrVal);
			}
			
			$doDownListHtml = OrderListV3Helper::getDropdownToggleHtml('批量操作', $doDownListArr);
// 			$doDownListHtml = OrderListV3Helper::getDropdownToggleHtml('批量操作', $doarr, 'orderCommonV3.doaction3');
			echo $doDownListHtml;
// 		}
		?>
		
		<?php // Html::dropDownList('do','',$excelmodels,['onchange'=>"exportorder($(this).val());",'class'=>'iv-input do','style'=>'width:200px;margin:0px']);?>
		<?php // Html::dropDownList('orderExcelprint','',['-1'=>'导出订单','0'=>'按勾选导出','1'=>'按所有页导出'],['onchange'=>"OrderCommon.orderExcelprint($(this).val())",'class'=>'iv-input do','style'=>'width:200px;margin:0px']);?>
		...
		<?php
			if(@$_REQUEST['order_status'] == OdOrder::STATUS_PAY){?>
				<div class="pull-right" style="height: 40px;">
					<button type="button" class="iv-btn btn-important" onclick="OrderCommon.importTrackNoBox('lazada')">导入跟踪号</button>	
				</div>
		<?php	}
		?>
<!-- ----------------------------------导入跟踪号end--------------------------------------------------------------------------------------------- -->			
	...
	<br>
			<?php 
				echo $this->render('../order/order_list_v3',[ 'carriers'=>$carriers, 'models'=>$models, 'countrys'=>$countryArr,
					...
					'platform_type'=>'lazada','selleruserids'=>['lazada'=>$selleruserids]]);
			?>
		</form>
...
function doaction(val){
	...
	switch(val){
		...
		case 'refreshOrder':
			...
			$.post('<?=Url::to(['/order/lazada-order/refreshorder'])?>',{orders:thisOrderList},function(result){
		...
		case 'signshipped':
			...
			document.a.action="<?=Url::to(['/order/lazada-order/signshipped'])?>";
			document.a.submit();
			document.a.action="";
			break;
		case 'signwaitsend':
		...
		case 'signpayed':
			...
			$.post('<?=Url::to(['/order/lazada-order/signpayed'])?>',{orders:idstr},function(result){
				bootbox.alert(result);
				location.reload();
			});
			break;
...
function doactionone(val,orderid){
	...
	switch(val){
		...
		case 'editOrder':
			idstr = [];
			idstr.push(orderid);
			window.open("<?=Url::to(['/order/lazada-order/edit'])?>"+"?orderid="+orderid,'_blank')
			break;
		case 'suspendDelivery':
			idstr = [];
			idstr.push(orderid);
		...
		case 'refreshOrder':
			idstr = [];
			idstr.push(orderid);
			$.post('<?=Url::to(['/order/lazada-order/refreshorder'])?>',{orders:idstr},function(result){
				bootbox.alert(result);
				location.reload();
			});
			break; 
		...
		case 'checkorder':
			...
			break;
		case 'signshipped':
			window.open("<?=Url::to(['/order/lazada-order/signshipped'])?>"+"?order_id="+orderid,'_blank')
			break;
		case 'signpayed':
			$.post('<?=Url::to(['/order/lazada-order/signpayed'])?>',{orders:orderid},function(result){
				bootbox.alert(result);
				location.reload();
			});
			break;
...
function movestatus(val){
	...
	$.post('<?=Url::to(['/order/lazada-order/movestatus'])?>',{orderids:idstr,status:val},function(result){
		bootbox.alert('操作已成功');
	});
}
//上传物流单号
function importordertracknum(){
	if($("#order_tracknum").val()){
		$.ajaxFileUpload({  
			 url:'<?=Url::to(['/order/lazada-order/importordertracknum'])?>',
...
function changemanual(orderid,obj){
	$.post('<?=Url::to(['/order/lazada-order/changemanual'])?>',{orderid:orderid},function(result){
		if(result == 'success'){
			bootbox.alert('操作已成功');
			var str;
			str=$(obj).html();
...
function setusertab(orderid,tabobj){
	...
	$.post('<?=Url::to(['/order/lazada-order/setusertab'])?>',{orderid:orderid,tabid:tabid},function(result){
		if(result == 'success'){
			bootbox.alert('操作已成功');
		}else{
			bootbox.alert(result);
...
function manualSync(sellerloginid,startdate,enddate,synctype){
	$.showLoading();
	$.post('<?=Url::to(['/order/lazada-order/monual-sync'])?>',{sellerloginid:sellerloginid,startdate:startdate,enddate:enddate,synctype:synctype},function(result){
		$.hideLoading();
...
function saveCustomCondition(modalbox , filter_name){
	$.ajax({
		type: "POST",
			dataType: 'json',
			url:'/order/lazada-order/append-custom-condition?custom_name='+filter_name, 
...
function showExtendsBuyerAcceptGoodsTimeBoxOMS(orderIdList){
	$.ajax({
		type: "GET",
			dataType: 'html',
			url:'/order/lazada-order/show-extends-buyer-accept-goods-time-box', 
...
function ExtendsBuyerAcceptGoodsTimeOMS(){
	...
	$.ajax({
		type: "POST",
			dataType: 'json',
			url:'/order/lazada-order/extends-buyer-accept-goods-time', 
...
// 		type: "POST",
// 			dataType: 'text',
// 			url:'/order/lazada-order/clear-left-menu-cache', 
// 			success: function (result) {
// 				location.reload();
// 			},
// 			error: function(){
...
function showDashBoard(autoShow){
	...
	$.ajax({
		type: "GET",
		url:'/order/lazada-order/user-dash-board?autoShow='+autoShow, 
		success: function (result) {
			$.hideLoading();
...
function requestGenerateDashBoardData(){
	...
	$.ajax({
		type: "GET",
		url:'/order/lazada-order/genrate-user-dash-board', 
		success: function (result) {
			if ($('#dash-board-enter').css('display') == 'none'){
				$('#dash-board-enter').toggle(1000);
			}
