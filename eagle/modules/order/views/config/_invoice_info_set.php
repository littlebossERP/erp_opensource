...
<div>
	<form id="seller_invoice_info_form">
		...
		<table class="table">
			...
			<tr>
				<td>税费计算方式:</td>
				<td colspan="3"><input type="radio" name="tax_formula" id="tax_formula_1" value="1" <?=(empty($infos['tax_formula']) || $infos['tax_formula']==1)?'checked':'' ?> ><label for="tax_formula_1" style="margin-right: 20px;">不含税价格=总价/（ 1+增值税率）</label><input type="radio" name="tax_formula" id="tax_formula_2" value="2" <?=(!empty($infos['tax_formula']) && $infos['tax_formula']==2)?'checked':'' ?> ><label for="tax_formula_2">不含税价格=总价*(1-增值税率)</label></td>
			</tr>
