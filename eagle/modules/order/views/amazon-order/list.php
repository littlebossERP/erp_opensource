...
?>
	...
	<div class="alert alert-danger" role="alert" style="width:100%;">
	   <?php foreach ($problemAccounts as $detailAccounts):?>
		<span>您绑定的Amazon账号：<?=$detailAccounts["sellerloginid"]?> 的验证信息已经过期且自动更新失败！<br>请检查您的绑定信息(如账号密码/API账号API密码)是否有误。</span><br />
	   <?php endforeach;?>
	</div>
	<?php endif;?>
...
<!------------------------------------第一行------------------------------------------>
		...
		<?php // echo Html::dropDownList('amzStoreDropdownList',@$_REQUEST['amzStoreDropdownList'],$amzStoreDropdownList,['class'=>'iv-input','id'=>'amzStoreDropdownList','style'=>'margin:0px;width:150px','prompt'=>'amazon店铺'])?> 
...
// 		$omsPlatformFinds[] = array('event'=>'OrderCommon.order_platform_find(\'oms\',\'amazon\',\'\')','label'=>'全部');
		
		if(count($amzStoreDropdownList) > 0){
			$amzStoreDropdownList['select_shops_xlb'] = '选择账号';
			...
			if(count($pcCombination) > 0){
				foreach ($pcCombination as $pcCombination_K => $pcCombination_V){
					$omsPlatformFinds[] = array('event'=>'OrderCommon.order_platform_find(\'oms\',\'amazon\',\''.'com-'.$pcCombination_K.'-com'.'\')',
						'label'=>'组合-'.$pcCombination_K, 'is_combined'=>true, 'combined_event'=>'OrderCommon.platformcommon_remove(this,\'oms\',\'amazon\',\''.$pcCombination_K.'\')');
				}
			}
		}
		echo OrderListV3Helper::getDropdownToggleHtml('卖家账号', $omsPlatformFinds);
		if(!empty($_REQUEST['selleruserid_combined'])){
			echo '<span onclick="OrderCommon.order_platform_find(\'oms\',\'amazon\',\'\')" class="glyphicon glyphicon-remove" style="cursor:pointer;font-size: 10px;line-height: 20px;color: red;margin-right:5px;" aria-hidden="true" data-toggle="tooltip" data-placement="top" data-html="true" title="" data-original-title="清空卖家账号查询条件"></span>';
		}
		//店铺查找代码 E
		?>
			...
			<div class="input-group iv-input">
		        <?php $sel = [
		        	'order_source_order_id'=>'Amazon订单号',
					'sku'=>'SKU',
					'tracknum'=>'物流号',
					'buyeid'=>'买家账号',
		        	'consignee'=>'买家姓名',
		   ...
		   <?=Html::checkbox('fuzzy',@$_REQUEST['fuzzy'],['label'=>TranslateHelper::t('模糊搜索')])?>
			...
			<a target="_blank"  title="手工订单" class="iv-btn btn-important" href="/order/order/manual-order-box?platform=amazon" >手工订单</a>
...
<!-- 			<div class="pull-right" style="height: 40px;">
			...
			<?php $orderTypeArr = ["AFN"=>"AFN(FBA)" , "MFN"=>"MFN(非FBA)"];?>
			<?=Html::dropDownList('order_type',@$_REQUEST['order_type'],$orderTypeArr,['class'=>'iv-input','prompt'=>'amazon订单类型(MFN+AFN)','id'=>'amazonordertype','style'=>'width:200px;margin:0px'])?>
			...
			<?=Html::dropDownList('order_source_status',@$_REQUEST['order_source_status'],$search,['class'=>'iv-input','prompt'=>'Amazon状态','id'=>'order_source_status','style'=>'width:200px;margin:0px'])?>
...
<!-- 		<?php
		...
		?> -->
		<?php $doDownListHtml = OrderListV3Helper::getDropdownToggleHtml('批量操作', $doarr, 'orderCommonV3.doaction3');
			echo $doDownListHtml; ?>
...
<!-- 		<?//=Html::dropDownList('orderExcelprint','',['-1'=>'导出订单','0'=>'按勾选导出','1'=>'按所有页导出'],['onchange'=>"OrderCommon.orderExcelprint($(this).val())",'class'=>'iv-input do','style'=>'width:200px;margin:0px']);?> -->
		<?php $excelActionItems = array('0'=>array('event'=>'OrderCommon.orderExcelprint(0)','label'=>'按勾选导出'), '1'=>array('event'=>'OrderCommon.orderExcelprint(1)','label'=>'按所有页导出'));
	...
	$.ajax({
		...
		success: function (result) {
			...
			bootbox.dialog({
				...
				title: Translator.t('Amazon订单监控面板'),
				message: result,
				closeButton:false,
				buttons:{
		...
		error :function () {
			$.hideLoading();
			bootbox.alert(Translator.t('打开Amazon订单监测面板失败'));
			return false;
		}
	});
}
