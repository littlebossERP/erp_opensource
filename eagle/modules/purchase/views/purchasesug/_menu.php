	...
	<ul class="ul-sidebar-one">
		...
		<li class="ul-sidebar-li<?=((yii::$app->controller->id == 'purchasesug') and (yii::$app->controller->action->id == 'sugindex'))?' active':''?>">
			<a class="" href="<?= Url::to(['/purchase/purchasesug/sugindex'])?>">
				...
				<span qtipkey="how-purchasesug-work"><?= TranslateHelper::t('缺货采购建议')?></span>
			</a>
		</li>
		<li style="display: none" class="ul-sidebar-li<?=((yii::$app->controller->id == 'purchasesug') and (yii::$app->controller->action->id == 'sug_strategies'))?' active':''?>">
