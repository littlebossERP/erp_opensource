...
if ($_REQUEST['list_picking_order']!=1){
...
<!------------------------------ oms 2.1 左侧菜单   end  ----------------------------------------->
<?php if ($_REQUEST['list_picking_order']==0){?>
...
<ul class="nav nav-tabs" >
  <li role="presentation" class="<?php echo ($_REQUEST['is_print_picking']==0 && $_REQUEST['list_picking_order']==0)?'active':'';?>"><a href="<?php echo Url::to(['/delivery/order/listpicking','warehouse_id'=>$_REQUEST['default_warehouse_id'],'delivery_status'=>2,'list_picking_order'=>0,'is_print_picking'=>0]);?>">未生成拣货单的订单(<?php echo $counter['listpicking'][0];?>)</a></li>
  <li role="presentation" class="<?php echo ($_REQUEST['is_print_picking']==1 && $_REQUEST['list_picking_order']==0)?'active':'';?>"><a href="<?php echo Url::to(['/delivery/order/listpicking','warehouse_id'=>$_REQUEST['default_warehouse_id'],'delivery_status'=>2,'list_picking_order'=>0,'is_print_picking'=>1]);?>">已生成拣货单的订单(<?php echo $counter['listpicking'][1];?>)</a></li>
  <li role="presentation" class="<?php echo $_REQUEST['list_picking_order']==1?'active':'';?>"><a href="<?php echo Url::to(['/delivery/order/listpicking','warehouse_id'=>$_REQUEST['default_warehouse_id'],'delivery_status'=>2,'list_picking_order'=>1]);?>">拣货单</a></li>
</ul>
...
<!-- ----------------------------------------------------------------------------------------------------------------- -->
	<div style="margin-top:10px;">
		...
		<?=Html::hiddenInput('is_print_picking',$_REQUEST['is_print_picking']);?>
		<?=Html::hiddenInput('list_picking_order',$_REQUEST['list_picking_order']);?>
		...
		<!----------------------------------------------------------- 精确搜索 ----------------------------------------------------------->
			<div class="input-group iv-input">
				<?=Html::dropDownList('keys',isset($query_condition['keys'])?$query_condition['keys']:'',$keys,['class'=>'iv-input','style'=>'width:110px;'])?>
...
<!-- --------------------------------------------列表  begin--------------------------------------------------------------- -->
	...
	<div>
		<div class="pull-left" style="height: 40px;">
		<?php
		if ($_REQUEST['is_print_picking']==0){
		echo Html::button(TranslateHelper::t('生成拣货单'),['class'=>"iv-btn btn-important",'onclick'=>"doactionnew(this)",'value'=>'buildDeliveryId_btn','style'=>'margin-right:3px;']);

		//批量操作 
		$doAction=OrderHelper::getCurrentOperationListNew(OdOrder::STATUS_WAITSEND,'b','未拣货');
		//单独操作
		$doActionOne=OrderHelper::getCurrentOperationListNew(OdOrder::STATUS_WAITSEND,'s','未拣货');
		}else{
		//批量操作
		$doAction=OrderHelper::getCurrentOperationListNew(OdOrder::STATUS_WAITSEND,'b','拣货');
		//单独操作
		$doActionOne=OrderHelper::getCurrentOperationListNew(OdOrder::STATUS_WAITSEND,'s','拣货');
		}
		?>
...
<?php }?>
<!-- ------------------------------------------------------------拣货单列表------------------------------------------------------------------------------ -->
<?php if ($_REQUEST['list_picking_order']==1){?>
<div class="content-wrapper container-fluid" >
<!---------------------------------------- oms 2.1 nav start  --------------------------------------------------->
<?php echo $order_nav_html;?>
<!---------------------------------------- oms 2.1 nav end  ------------------------------------------------------>
  ...
  <li role="presentation" class="<?php echo $_REQUEST['list_picking_order']==1 ?'active':'';?>"><a href="<?php echo Url::to(['/delivery/order/listpicking','warehouse_id'=>$_REQUEST['default_warehouse_id'],'delivery_status'=>2,'list_picking_order'=>1]);?>">拣货单</a></li>
    ...
    <div>
        <ul class="nav nav-pills"><!-- main-tab -->
            <li role="presentation" ><a class ="iv-btn <?php if (isset($_REQUEST['picking_status'])&&$_REQUEST['picking_status'] == OdDelivery::PICKING_PRINT_NO){echo 'btn-important';}?>" href="/delivery/order/listpicking?warehouse_id=<?=$_REQUEST['default_warehouse_id']?>&delivery_status=2&picking_status=0&list_picking_order=1"><?=TranslateHelper::t('未打印').'('.$counter['pickingStatus'][0].')'?></a></li>
            <li role="presentation" ><a class ="iv-btn glyphicon glyphicon-arrow-right" href="#"></a></li>
            <li role="presentation" ><a class ="iv-btn <?php if (isset($_REQUEST['picking_status'])&&$_REQUEST['picking_status'] == OdDelivery::PICKING_PRINT_ALREADY){echo 'btn-important';}?>" href="/delivery/order/listpicking?warehouse_id=<?=$_REQUEST['default_warehouse_id']?>&delivery_status=2&picking_status=1&list_picking_order=1"><?=TranslateHelper::t('已打印').'('.$counter['pickingStatus'][1].')'?></a></li>
            <li role="presentation" ><a class ="iv-btn glyphicon glyphicon-arrow-right" href="#"></a></li>
            <li role="presentation" ><a class ="iv-btn <?php if (isset($_REQUEST['picking_status'])&&$_REQUEST['picking_status'] == OdDelivery::PICKING_ALREADY){echo 'btn-important';}?>" href="/delivery/order/listpicking?warehouse_id=<?=$_REQUEST['default_warehouse_id']?>&delivery_status=2&picking_status=2&list_picking_order=1" ><?=TranslateHelper::t('完成拣货').'('.$counter['pickingStatus'][2].')'?></a></li>
        </ul>
    </div>
    ...
    <!-- --------------------------------------------------列表  begin------------------------------------------------------->
    <div>
        <form name="listpicking" id="listpicking" action=""  >
            <br>
            <table class="table table-condensed table-bordered" style="font-size:12px;">
                <tr>
                    <th width="8%"><b>拣货单号</b></th>
                    <th width="6%"><b>订单数</b></th>
                    <th width="6%"><b>SKU数</b></th>
                    <th width="6%"><b>商品数</b></th>
                    <th width="15%"><b>创建时间/创建人</b></th>
                    <th width="15%"><b>打印时间/打印人</b></th>
                    <th width="15%"><b>拣货人</b></th>
                    <th><b>操作</b></th>
                </tr>
                <?php $carriers=CarrierApiHelper::getShippingServices(false); ?>
                <?php if (count($models)):foreach ($models as $delivery_order):?>
                    <tr style="background-color: #f4f9fc" class="line-<?php echo $delivery_order->deliveryid;?>">
                        <td><?=$delivery_order->deliveryid?></td>
                        <td><?=$delivery_order->ordercount?></td>
                        <td><?=$delivery_order->skucount?></td>
                        <td><?=$delivery_order->goodscount?></td>
                        <td><?=$delivery_order->create_picking_time>0?date('Y/m/d H:i:s',$delivery_order->create_picking_time):''?>&nbsp;<?=$delivery_order->creater?></td>
                        <td> <?=$delivery_order->print_picking_time>0?date('Y/m/d H:i:s',$delivery_order->print_picking_time):''?>&nbsp;<?=$delivery_order->print_picking_operator?></td>
                        <td><?=$delivery_order->picking_operator?></td>
                        <td>
                        <a href="<?php echo Url::to(['/delivery/order/printpicking','deliveryid'=>$delivery_order->deliveryid])?>" target="_blank">打印</a>
                        <?php if(isset($_REQUEST['picking_status']) && $_REQUEST['picking_status']<2){?>
                        <a href="<?php echo Url::to(['/delivery/order/editpicking','deliveryid'=>$delivery_order->deliveryid])?>" target="_blank">编辑</a>
                        <a href="#" onclick="doaction('delete','<?php echo $delivery_order->deliveryid;?>')">删除</a>
                        <a href="#" onclick="doaction('finish_picking','<?php echo $delivery_order->deliveryid;?>','<?php echo $_REQUEST['default_warehouse_id'];?>')">完成拣货</a>
                        <?php }?>
                        </td>
                    </tr>
...
//批量操作
function doaction(val,deliveryid,warehouse_id){
    switch(val){
        case'delete': //删除(对话框)
            var event = $.confirmBox('确定删除拣货单？删除之后发货订单会重新回到拣货 --》未生成拣货单的订单');
    		event.then(function(){
    				 $.maskLayer(true);
    				 $.post('/delivery/order/deletepicking',{deliveryid:deliveryid},function(result){
    					 var r = $.parseJSON(result);
    					 if(r.success){
    						 $.alertBox('<p class="text-success">删除成功！订单重新设为未生成拣货单！</p>');
    						 $('.line-'+deliveryid).remove();
    						 $.maskLayer(false);
    					 }else{
    						 $.alertBox('<p class="text-warn">删除失败！</p>');
    						 $.maskLayer(false); 
        				 }
        ...
        case'finish_picking':   //完成拣货
            $.ajax({
                type: "GET",
                dataType: 'html',
                url:'/delivery/order/finishpicking',
                data: {deliveryid : deliveryid , warehouse_id :warehouse_id},
                success: function (result) {
                    bootbox.dialog({
                        title: Translator.t("确认拣货完成"),
                        className: "isfinishpicking",
                        message: result
                    });
                }
            });
