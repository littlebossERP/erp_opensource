...
<!------------------------------ oms 2.1 左侧菜单  start  ----------------------------------------->
<?php if($delivery_picking_mode == 1) if ($warehouseCount>1){ echo $this->render('_leftmenu',['counter'=>$counter]);}?>
<!------------------------------ oms 2.1 左侧菜单   end  ----------------------------------------->

<div class="content-wrapper container-fluid">
<?php if (($warehouseCount==1) || ($delivery_picking_mode != 1)){echo $order_nav_html;}?>
<!-- --------------------------------------------搜索 bigin--------------------------------------------------------------- -->
	<div>
		<!-- 搜索区域 -->
		...
		<div class="input-group iv-input">
			<?php unset($keys['delivery_id']);unset($keys['tracknum']); ?>
		...
		<?php //echo Html::dropDownList('order_source',isset($query_condition['order_source'])?$query_condition['order_source']:'',OdOrder::$orderSource,['class'=>'iv-input','prompt'=>'销售平台','id'=>'order_source','style'=>'width:200px;margin:0px'])?>
			...
			<?php //Html::dropDownList('default_shipping_method_code',isset($query_condition['default_shipping_method_code'])?$query_condition['default_shipping_method_code']:'',$allshippingservices,['class'=>'iv-input','prompt'=>'运输服务','id'=>'shipmethod','style'=>'width:200px;margin:0px'])?>
			
<!-- 			</div> -->
			<!-- ----------------------------------第三行--------------------------------------------------------------------------------------------- -->
<!--			<div style="margin:20px 0px 0px 0px"> -->
			<?php // Html::dropDownList('fuhe',isset($query_condition['fuhe'])?$query_condition['fuhe']:'',$search,['class'=>'iv-input','prompt'=>'快速筛选','id'=>'fuhe','style'=>'width:200px;margin:0px'])?>
			...
			<?php $tmpOdtimetype = Odorder::$timetype; unset($tmpOdtimetype['printtime']); unset($tmpOdtimetype['shiptime']); ?>
			...
			<?php $tmpCustomsort = Odorder::$customsort; unset($tmpCustomsort['printtime']); unset($tmpCustomsort['shiptime']); ?>
...
<!-- --------------------------------------------列表  begin--------------------------------------------------------------- -->
	...
	<div>
		<div class="pull-left" style="height: 40px;">
		<?php echo Html::button(TranslateHelper::t('指定运输服务'),['class'=>"iv-btn btn-important",'onclick'=>"doactionnew(this)",'value'=>'changeWHSM_btn']);?>
		<?php echo Html::button(TranslateHelper::t('匹配运输服务'),['class'=>"iv-btn btn-important",'onclick'=>"doactionnew(this)",'value'=>'autocarrierservice_btn']);?>
		<?php
		//批量操作 
		$doAction=OrderHelper::getCurrentOperationListNew(OdOrder::STATUS_WAITSEND,'b','指定发货仓库和运输服务');
		//单独操作
		$doActionOne=OrderHelper::getCurrentOperationListNew(OdOrder::STATUS_WAITSEND,'s','指定发货仓库和运输服务');
		?>
		<?=Html::dropDownList('do','',$doAction,['onchange'=>"doactionnew(this);",'class'=>'iv-input do','style'=>'width:200px;margin:0px']);?> 
		<?php echo (count($excelmodels) > 0) ? Html::dropDownList('do','',$excelmodels,['onchange'=>"exportorder($(this).val());",'prompt'=>'自定义格式Excel导出','class'=>'iv-input do','style'=>'width:200px;margin:0px']) : '';?>
...
<form action="" method="post" target="_blank" id="ordersform" name = 'a'>
	<table class="table table-condensed table-bordered" style="font-size:12px;margin-top:20px;">
		...
		<tr style="background-color: #f4f9fc;border:1px solid #d1d1d1;" class="line-<?php echo $order->order_id;?>">
			...
			?>
			<br>
			运输服务：<a onclick="doactiononenew('changeWHSM','<?=$order->order_id ?>');">指定运输服务</a>
			</td>
			<td>
...
// 								echo '<br/>';
			...
			<td colspan="2" rowspan="<?=count($order->items)?>" width="150" style="word-break:break-all;word-wrap:break-word;border:1px solid #d1d1d1">
