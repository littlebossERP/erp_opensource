  ...
  <div class="tab-content">
    <div role="tabpanel" class="tab-pane active" id="home"><br>
		...
		<form id="form1" name="form1" action="" method="post">
			...
			<?=Html::hiddenInput('peihuo_status',@$_REQUEST['peihuo_status'],['id'=>'peihuo_status'])?>
		</form>
				
		<form name="a" id="a" action="" method="post">
		<?=Html::hiddenInput('return_url',$return_url);?>
		<div class="panel panel-default" style="margin-top:5px;">
			<?=Html::button('完成配货',['class'=>'btn btn-success','onclick'=>"javascript:mutidopeihuoorder()"])?>
				...
				<tr>
					...
					<th>拣货单号</th>
					<th class="text-center text-nowrap">订单总数</th>
					...
					<th class="text-center text-nowrap">
						<?=Html::dropDownList('peihuo_status',@$_REQUEST['peihuo_status'],['0'=>'未配货','1'=>'已配货'],['prompt'=>'配货状态','onchange'=>"dosearch('peihuo',$(this).val());"])?>
					</th>
				...
				<tr>
					...
					<td><?=$order->peihuo_status=='1'?'已配货':'未配货'?></td>
					<td>
						<a target="_blank" href="<?=Url::to(['/delivery/order/jianhuoviewedit','id'=>$order->deliveryid,'view'=>1])?>">查看</a>|
						<a target="_blank" href="<?=Url::to(['/delivery/order/jianhuoviewedit','id'=>$order->deliveryid,'view'=>0])?>">编辑</a>|
						<a href="#" onclick="javascript:dopeihuoorder('<?=$order->id?>')">配货完成</a>|
						<a href="#" onclick="javascript:cancelpickingorder('<?=$order->id?>')">取消配货</a>
					</td>
				</tr>
