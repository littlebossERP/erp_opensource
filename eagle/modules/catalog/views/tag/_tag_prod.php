...
$this->registerJsFile(\Yii::getAlias('@web')."/js/project/catalog/tag.js", ['depends' => ['yii\web\JqueryAsset']]);

$this->registerJs("tag.list.innerInit();" , \yii\web\View::POS_READY);

?>
<style>
.product_in_tag .modal-body {
	max-height: 500px;
	overflow-y: auto;
}

.product_in_tag .modal-dialog {
	width: 900px;
}
#prodInTag_list_table th,#prodInTag_list_table td {
	padding:4px 0px;
}
</style>
<div style="width: 100%;">
  	<FORM id="prodInTag_filter" style="padding-bottom: 10px;">
  		<span id="tag_name_span" style="display:none"><?=$tag_name ?></span>
  		<table style="width:100%;">
  			<tr style="width:100%;">
  					...
  					<div class="div-input-group" style="width:250px">
  						<div style="" class="input-group" style="float:left;">
							...
							<span class="input-group-btn" style="">
								<button type="button" class="btn btn-default" id="btn_prodInTag_search" style="" onclick="tag.list.tagProdSearch()">
				...
				<td style="border:0px;width:50%;padding:0px">
				    <button type="button" class="btn-xs btn-transparent font-color-1" onclick="tag.list.batchProdRemoveTag(<?=$tag_id ?>)" style="float:right;padding-right: 20px;">
				   		<span class="egicon-trash" aria-hidden="true" style="height:16px;"></span>
				   		<?= TranslateHelper::t('批量移除产品标签') ?>
				   	</button>
				</td>
		    </tr>
	    </table>
	...
	<table id= "prodInTag_list_table" cellspacing="0" cellpadding="0" class="table table-hover" style="font-size:12px;width:100%;">
		<tr class="list-firstTr">
			<th width="30px">
				<input type="checkbox" name="chk_tagProd_all">
			</th>
			<th style="display:none;width:0px;">tag_name</th>
			<th width="60px"><?=TranslateHelper::t('图片') ?></th>
			<th width="150px"><?=TranslateHelper::t('sku') ?></th>
			<th width="300px"><?=TranslateHelper::t('名称') ?></th>
			<th width="80px"><?=TranslateHelper::t('类型') ?></th>
			<!--
	    ...
	    <tr>
	    	<td>
	    		<input type="checkbox" name="chk_tagProd_info" value="<?=$aData['product_id']?>">
	    	</td>
	    	<td style="display:none;width:0px;padding:0px;"><?=htmlentities($tag_name) ?></td>
	        ...
	        <td>
	        	<button type="button" class="btn-xs btn-transparent font-color-1" 
	        		onclick="tag.list.prodRemoveTag(<?=$aData['product_id'] ?>,<?=$tag_id ?>,this,'','one')" 
	        		title="<?=TranslateHelper::t('产品删除该标签') ?>">
...
$options = array();
$options['pagerId'] = 'prodInTag_view_pager';// 下方包裹 分页widget的id
$options['action'] = \Yii::$app->request->getPathInfo()."?tag_id=$tag_id"; // ajax请求的 action
...
$this->registerJs('$("#prodInTag_list_table").initGetPageEvent('.json_encode($options).')' , \Yii\web\View::POS_READY);
