...
<script>
	...
	paramsUrl = "<?= \Yii::$app->urlManager->createUrl('/configuration/carrierbackstage/params') ?>";
	indexUrl = "<?= \Yii::$app->urlManager->createUrl('/configuration/carrierbackstage/index') ?>";
...
<h4 class="modal-title" id="myModalLabel" style="display:inline-block">物流商参数</h4>
<h5>说明:前端不显示暂时分开两种情况：</h5>
<h5>1.认证参数为1时表示增加状态下不显示出来,主要用于不想用户见到不相关的非必要信息。</h5>
<h5>2.订单/商品参数为1时物流上传界面不能看到该参数,只是隐藏该显示。</h5>
<br>
<form action="<?= \Yii::$app->urlManager->createUrl('/configuration/carrierbackstage/params') ?>" method="post" id="create_form" style="margin-bottom:100px">
...
<div class="table-responsive">
    ...
    <table cellspacing="0" cellpadding="0" class="table table-striped">
    	<tr style="background-color:#abcdef">
	    	...
	    	<td>
	        	<select name="display_type[]" >
	        		...
	        		<option value="hidden" <?= @$p['display_type']=='hidden'?'selected':'' ?>>隐藏域</option>
	        	</select>
	    	</td>
	    	<td><?=TranslateHelper::t('前端不显示') ?></td>
	    	<td>
	        	<select name="is_hidden[]">
	        		<option value="0" <?= isset($p['is_hidden'])&&$p['is_hidden']==0?'selected':'' ?>>否</option>
	        		<option value="1" <?= isset($p['is_hidden'])&&$p['is_hidden']==1?'selected':'' ?>>是</option>
	        	</select>
	        </td>
	        <td><?=TranslateHelper::t('前端类型') ?></td>
	    	<td>
	        	<select name="ui_type[]">
	        		<option value='' <?= isset($p['ui_type'])&&$p['ui_type']==''?'selected':'' ?>></option>
	        		<option value="param_print" <?= isset($p['ui_type'])&&$p['ui_type']=='param_print'?'selected':'' ?>>打印参数</option>
	        		<option value="param_set" <?= isset($p['ui_type'])&&$p['ui_type']=='param_set'?'selected':'' ?>>普通货代设置参数</option>
	        	</select>
	        </td>
	    ...
	    <tr>
	        <td><?=TranslateHelper::t('参数描述') ?></td>
	        <td colspan="14">
				<textarea name="param_describe[]" id="" cols="130" rows="1"><?= @$p['param_describe'] ?></textarea>
	        </td>
	    </tr>
	    <?php
    ...
    <table cellspacing="0" cellpadding="0" class="table table-striped">
    	<tr style="background-color:#abcdef">
	    	...
	    	<td>
	        	<select name="ui_type[]">
	        		<option value='' ></option>
	        		<option value="param_print">打印参数</option>
	        		<option value="param_set">普通货代设置参数</option>
	        	</select>
	        </td>
	    	<td><?=TranslateHelper::t('排序') ?></td>
	    	<td>
	    ...
	    <tr>
	        <td><?=TranslateHelper::t('参数描述') ?></td>
	        <td colspan="14">
				<textarea name="param_describe[]" id="" cols="130" rows="1"></textarea>
	        </td>
	    </tr>
