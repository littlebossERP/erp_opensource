...
<!-- table -->
    <table cellspacing="0" cellpadding="0" width="100%" class="table table-hover">
        <tr class="list-firstTr">
	        ...
	        <td class="text-nowrap"><?= TranslateHelper::t('是否启用') ?></td>
        ...
        <?php foreach($carriers as $c):?>
            <tr>
	            ...
	            <td class="text-nowrap"><?=($c['is_active'] == 1 ? '启用' : '关闭') ?></td>
				...
				<button onclick="location.href='<?= \Yii::$app->urlManager->createUrl(['/configuration/carrierbackstage/params','code'=>$c['carrier_code']]) ?>'" class="btn btn-default btn-sm"><?=TranslateHelper::t('参数') ?></button>
				<button onclick="location.href='<?= \Yii::$app->urlManager->createUrl(['/configuration/carrierbackstage/channel-list','code'=>$c['carrier_code'],'carrier_name'=>$c['carrier_name']]) ?>'" class="btn btn-default btn-sm"><?=TranslateHelper::t('运输方式') ?></button>
	            </td>
	        </tr>
...
<script>
	function editCarrier(carrierCode,op) {//添加订单标签
		var reUrl = '';
		if(op=='carrier')reUrl='<?= \Yii::$app->urlManager->createUrl("/configuration/carrierbackstage/create") ?>';
		$.ajax({
	        type : 'get',
	        cache : 'false',
