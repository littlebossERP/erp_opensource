...
$this->registerJs("platform.PriceMinisterAccountsList.initWidget();" , \yii\web\View::POS_READY);
?>
<style>
.PriceMinisterAccountInfo .modal-body{
	max-height: 400px;
	overflow-y: auto;
}
</style>
...
<a id="new-PriceMinister-account-btn" href="#" class="btn btn-info" style="margin-top: 15px;margin-bottom:15px;"><?= TranslateHelper::t('PriceMinister账号绑定列表')?></a>
...
<div class="account-list bs-callout bs-callout-warning">
    <table cellspacing="0" cellpadding="0" width="100%" class="table table-hover">
		<tr class="list-firstTr">
			...
			<td width="20%"><?=$sort->link('token',['label'=>TranslateHelper::t('token')]) ?></td>
        ...
        <?php 
        $rowIndex = 1;
        foreach($PriceministerUserInfoList as $PriceMinisterUser):?>
            <tr>
	            <td><?=$rowIndex ?></td>
                <td><?=$PriceMinisterUser['store_name'] ?></td>
	            <td style="word-break:break-all;"><?=$PriceMinisterUser['token'] ?></td>
	            <td><?=$PriceMinisterUser['is_active'] ?></td>
	            <td><?=$PriceMinisterUser['create_time'] ?></td>
	            <td><?=$PriceMinisterUser['update_time'] ?></td>
	            ...
	            <td><?=$PriceMinisterUser['last_product_retrieve_time'] ?></td>
	            <td>
					<a class="btn btn-default btn-xs" href="#" onclick="platform.PriceMinisterAccountsList.openViewWindow('<?=$PriceMinisterUser['site_id'] ?>')"><?=TranslateHelper::t('查看') ?></a> 
					<a class="btn btn-default btn-xs" href="#" onclick="platform.PriceMinisterAccountsList.openEditWindow('<?=$PriceMinisterUser['site_id'] ?>')"><?=TranslateHelper::t('编辑') ?></a>
					<a class="btn btn-default btn-xs" href="#" onclick="platform.PriceMinisterAccountsList.delPriceMinisterAccount('<?=$PriceMinisterUser['site_id'] ?>')"><?=TranslateHelper::t('删除') ?></a>
	            </td>
	        </tr>
	       <?php $rowIndex++;?>
        <?php endforeach;?>
