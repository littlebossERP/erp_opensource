...
use eagle\modules\delivery\controllers\OrderController;
...
$this->title = TranslateHelper::t('待打印物流单');
...
<button type="button" id="do-carrier" value='doprint'  style="height: 28px;" class="btn btn-success btn-sm"><?= TranslateHelper::t('批量打印')?></button>
 &nbsp;
...
<?=Html::textInput('tracking_number',isset($search_data['tracking_number'])?$search_data['tracking_number']:'',['class'=>'eagle-form-control','placeholder'=>"物流号",'title'=>'物流号'])?>&nbsp;
...
<table cellspacing="0" cellpadding="0" width="100%" class="table table-hover table-striped" style="font-size: 12px;">
		<tr class="list-firstTr">
			...
			<th><?php echo TranslateHelper::t('物流号');?></th>
        ...
        <tr>
            ...
            <td><?=isset($services[$data['default_shipping_method_code']])?$services[$data['default_shipping_method_code']]:''; ?></td>
             <td><?php
             $shipped_obj = OdOrderShipped::findOne(['order_id'=>$data['order_id'],'customer_number'=>$data['customer_number']]);
             if (!$shipped_obj==null){
				echo $shipped_obj->tracking_number;
			 }
             ?></td>
            <td>
            <font color="orange"><?php echo strlen($data['carrier_error'])?$data['carrier_error']:'未打印';?></font>
            <?php ?>
            </td>
            <td>
            <a href="<?=Url::to(['/carrier/carrieroperate/doprint2','orders'=>$data['order_id']])?>"  target="_blank"><?php echo TranslateHelper::t('打印物流单');?></a>
            <a href="<?=Url::to(['/carrier/carrieroperate/doprint-saitu','orders'=>$data['order_id']])?>"  target="_blank"><?php echo TranslateHelper::t('赛兔模式打印物流单');?></a>
