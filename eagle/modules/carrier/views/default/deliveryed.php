...
$this->title = TranslateHelper::t('已交运');
?>
<style>
...
<table cellspacing="0" cellpadding="0" width="100%"  class="table table-hover table-striped" style="font-size: 12px;">
        ...
        <tr data="<?=$data['order_id'] ?>" ems="<?=key($orders['ems'])?>">
            ...
            <td>
	            <font color="orange"><?php echo strlen($data['carrier_error'])?$data['carrier_error']:'待打印';?></font>
            </td>
            <td>
...
function suspend(id){
	...
	event.then(function(){
		  // 确定
			 $.maskLayer(true);
			 $.post('<?=Url::to(['/carrier/default/suspend'])?>',{order_id:ids,m:'carrier',a:'已交运->暂停发货'},function(result){
