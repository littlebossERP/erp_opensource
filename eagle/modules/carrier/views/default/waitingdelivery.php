...
$this->title = TranslateHelper::t('待交运');
?>
<style>
...
<!---------------------------------------- oms 2.1 nav end  ------------------------------------------------------>
	<div style="padding-top:5px;height:40px;">
		<button type="button" id="do-carrier" onclick="doaction('dodispatch')"  style="height: 30px;width:100px;" class="btn btn-success btn-sm"><?= TranslateHelper::t('交运')?></button>
		 &nbsp;
...
<table cellspacing="0" cellpadding="0" width="100%"  class="table table-hover table-striped" style="font-size: 12px;">
		<tr class="list-firstTr">
			...
			<th width="12%"><?=TranslateHelper::t('运输服务/跟踪号')?></th>
			<th width="10%"><?=TranslateHelper::t('客户参考号')?></th>
			<th width="10%"><?=TranslateHelper::t('交运结果');?></th>
			<th width="6%"><?=TranslateHelper::t('操作')?></th>
		</tr>
        <?php if(count($orders)>0): //print_r($orders);?>
        ...
        <tr data="<?=$data['order_id'] ?>" ems="<?=key($orders['ems'])?>">
            ...
            <td>
	            <font color="orange"><?php echo strlen($data['carrier_error'])?$data['carrier_error']:'待交运';?></font>
            </td>
            <td>
...
function suspend(id){
	...
	event.then(function(){
		  // 确定
			 $.maskLayer(true);
			 $.post('<?=Url::to(['/carrier/default/suspend'])?>',{order_id:ids,m:'carrier',a:'待交运->暂停发货'},function(result){
