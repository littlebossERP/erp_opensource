...
<div>
			...
			if((isset($data['result']['error'])) && ($data['result']['error'])){
				echo "<p class='text-center' style='margin-top:100px; font-size:16px;line-height:30px;'>物流运输服务：".$data['carrier_name'].'<br>物流单打印失败 错误原因:'.$data['result']['msg']."<br><input type='button' value='关闭页面' class='btn btn-warning' onclick='javascript:window.close();'/></p>";
			}else{
					...
					if(isset($data['result']['data']['html_str'])){
						echo $data['result']['data']['html_str'];
					}else if(isset($data['result']['data']['pdfUrl'])){
