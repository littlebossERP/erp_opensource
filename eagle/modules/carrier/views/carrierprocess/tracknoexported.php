...
<div class="form-group">
	<ul class="nav nav-pills"><!-- btn-tab -->
		<li role="presentation" ><input type="button" class ="iv-btn btn-important" onclick="moveToUpload()" value="重新分配" /></li>
...
<table class="table table-condensed table-bordered" style="font-size:12px;table-layout: fixed;word-break: break-all;">
			...
			<tr style="background-color: #f4f9fc" data="<?= $order->order_id?>" deliveryid="<?= $order->delivery_id?>">
				...
				<td>
					<?php 
						$shipped = OdOrderShipped::find()->where(['order_id'=>$order->order_id ])->andwhere( " ifnull(tracking_number, '') <> '' " )->orderby('id desc')->one();
						$customer_number = empty($shipped)?'':$shipped->tracking_number;
