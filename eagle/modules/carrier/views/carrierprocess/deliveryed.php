...
<div class="form-group">
	<ul class="nav nav-pills"><!-- btn-tab -->
		<li role="presentation" ><input type="button" class ="iv-btn btn-important" onclick="setFinished()" value="确认发货完成" /></li>
		<li role="presentation" >
		<?php 
			echo Html::checkbox('ignore_inventory_processing',false,['value'=>true,'label'=>TranslateHelper::t('忽略库存处理')]);
			?>
		</li>
		<li role="presentation" >
		<?php 
			echo Html::checkbox('is_shipped',true,['value'=>true,'label'=>TranslateHelper::t('通知平台发货')]);
			?>
		</li>
		<li role="presentation" ><input type="button" class ="iv-btn btn-important" onclick="moveToUpload()" value="重新上传" /></li>
		<li role="presentation" ><input type="button" class ="iv-btn btn-important" onclick="changeServerToUpload()" value="更改运输服务" /></li>
		<li role="presentation" ><input type="button" class ="iv-btn btn-important" onclick="getTrackNo()" value="获取跟踪号" /></li>
		<li role="presentation" ><?= Html::dropdownlist('shipmethod',@$_REQUEST['default_shipping_method_code'],[''=>'请先选择运输服务再打印']+$shippingServices,['class'=>'iv-input','style'=>'width:260px','onchange'=>'changeShippingService(this)'])?></li>
		<li role="presentation" ><input type="button" class="iv-btn <?= (isset($printMode['is_api_print']) && !empty($printMode['is_api_print']))?'btn-important':'disabled'?>" onclick="<?= (isset($printMode['is_api_print']) && !empty($printMode['is_api_print']))?"doprint('api')":''?>" value="API获取标签打印"/></li>
		<li role="presentation" ><input type="button" class="iv-btn <?= (isset($printMode['is_print']) && !empty($printMode['is_print']))?'btn-important':'disabled'?>" onclick="<?= (isset($printMode['is_print']) && !empty($printMode['is_print']))?"doprint('gaofang')":''?>" value="高仿标签打印" /></li>
		...
		<li role="presentation" ><input type="button" class="iv-btn <?= (isset($printMode['is_api_print']) && !empty($printMode['is_api_print']))?'btn-important':'disabled'?>" onclick="<?= (isset($printMode['is_api_print']) && !empty($printMode['is_api_print']))?"doprint('integrationlabel')":''?>" value="一体化标签打印"/></li>
...
<table class="table table-condensed table-bordered" style="font-size:12px;table-layout: fixed;word-break: break-all;">
			<tr>
                ...
                <th width="8%"><b>物流单打印</b></th>
				<th width="6%"><b>操作</b></th>
			</tr>
			...
			<tr style="background-color: #f4f9fc" data="<?= $order->order_id?>" deliveryid="<?= $order->delivery_id?>">
                ...
                <td>
                    <?php echo ($order->is_print_carrier == '1')?'已打印':'未打印';?>
                </td>
				<td>
					<input type="button" value="暂停" class="iv-btn btn-primary" onclick="javascript:doactionForOneOrder('suspenddelivery','<?= $order->order_id?>')" />
				</td>
			</tr>
			<?php endforeach;endif;?>
			</table>
