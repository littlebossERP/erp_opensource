...
class SaasLazadaAutoSyncApiHelperV4 {
	...
	* 获取所有lazada用户的api访问信息。 email,token,销售站点
	...
	private static function getAllLazadaAccountInfoMap(){
		$lazadauserMap=array();
		$nowTime = time();
// 		$lazadaUsers=SaasLazadaUser::find()->where(['version'=>'v2'])->andWhere('status<>3')->all();
...
// 						$sku['Images'] = $images;
				...
				foreach ($products['update'] as $lazada_uid=>$sameAccountUpProds){
				    foreach($sameAccountUpProds as $publishListingId=>$sameAccountUpProd){
						...
						if(!empty($tempConfig["version"])){//新授权
						    $response = LazadaInterface_Helper_V2::productUpdate($config,array('product'=>$sameAccountUpProd));
						}else{
				...
				foreach ($products['create'] as $lazada_uid=>$sameAccountCrtProds){
					foreach($sameAccountCrtProds as $publishListingId=>$sameAccountCrtProd){
						...
						if(!empty($tempConfig["version"])){//新授权
						    $response = LazadaInterface_Helper_V2::productCreate($config,array('product'=>$sameAccountCrtProd));
						}else{
			...
			return array(false , $e->getMessage().",line:".$e->getLine());
		}
						...
						foreach ($imageUploadTargets as $imageUploadTarget){
							...
							foreach ($variantData as $oneVariant){
        						...
        						foreach ($imageInfo['product_photos'][$oneVariant['skuSelData']] as $imageSrc){
        						    ...
        						    if(!empty($tempConfig['version'])){//新授权
        						        $migrateResponse = LazadaInterface_Helper_V2::migrateImage($config,array('url'=>$imageSrc));
        						    }else{
...
// 							print_r($checkQCSkus);exit();
							...
							if(!empty($tempConfig['version'])){//新授权
							    $response = LazadaInterface_Helper_V2::setImage($config,array('product'=>$product));
							}else{
							...
							if($response['success'] != true){
								...
								if(!empty($tempConfig['version'])){//新授权
								    $qcResponse = LazadaInterface_Helper_V2::getQcStatus($config,array('SkuSellerList'=>$checkQCSkus));
								}else{
	...
	public static function batchUpdateQuantity($products){
		...
		foreach ($updateProducts as $lazada_uid=>$sameAccountUpProds){
			...
			if(!empty($tempConfig['version'])){//新授权
			    $response = LazadaInterface_Helper_V2::productUpdatePriceQuantity($config,array('product'=>$product));
			}else{
	...
	public static function batchPutOnLine($products){
		...
		foreach ($updateProducts as $lazada_uid=>$sameAccountUpProds){
			...
			if(!empty($tempConfig['version'])){//新授权
			    $response = LazadaInterface_Helper_V2::productUpdate($config,array('products'=>$sameAccountUpProds));
			}else{
	...
	public static function getCategoryTree($config,$loadCache=true){
	    ...
	    if(empty($categoriesJsonStrObj) || empty($categoriesJsonStrObj->categories)
	        ...
	        $response = LazadaInterface_Helper_V2::getCategoryTree($config);
...
// 	            }
	            LazadaApiHelper::getCategoryInfoV4($category , 1 , 0 , $config['countryCode'] , $categories);
	        }
	...
	public static function getCategoryAttributes($config,$primaryCategory=-1,$loadCache=true){
	    ...
	    if(empty($categoryAttrsJsonStrObj) || $loadCache == false){
	        $response = LazadaInterface_Helper_V2::getCategoryAttributes($config,array('PrimaryCategory'=>$primaryCategory));
	        if($response['success'] != true){
	            return array(false , $response['message']);
	        }
	    ...
	    //数据key转换，新接口key不一样
	    $change_key = ["attribute_type"=>"attributeType","is_mandatory"=>"isMandatory","input_type"=>"inputType","is_sale_prop"=>"isSaleProp",];
	    foreach ($attributes as $key => $val){
	        foreach ($val as $k => $v){
	            if(isset($change_key[$k])){
	                $attributes[$key][$change_key[$k]] = $v;
	...
	public static function getBrands($config,$searchName="",$searchMode="like",$purge=false){
	    ...
	    if(empty($brands) || $purge){
	        $response = LazadaInterface_Helper_V2::getBrands($config);
	        if($response['success'] != true){
	            return array(false , $response['message']);
	        }
	        ...
	        //变换key值
	        $changeKey = ["name"=>"Name","brand_id"=>"BrandId","global_identifier"=>"GlobalIdentifier",];
	        foreach ($brands as &$brand){
	            //变换key值
	            foreach ($brand as $key => $val){
	                if(isset($changeKey[$key])){
	                    $brand[$changeKey[$key]] = $val;
	                    unset($brand[$key]);
	                }
	            }
	            $brand['site'] = $config['countryCode'];
	            $toAddListing = array();
	            Yii::info("step3 memory usage:".(memory_get_usage()/1024/1024)."M. (getBrandsV2)","file");
...
// 	        Yii::info("step6 memory usage:".(memory_get_usage()/1024/1024)."M. (getBrandsV2)","file");
	        unset($brands);//释放部分内存
// 	        Yii::info("step7 memory usage:".(memory_get_usage()/1024/1024)."M. (getBrandsV2)","file");
