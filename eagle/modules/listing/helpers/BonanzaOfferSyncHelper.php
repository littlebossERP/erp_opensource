...
use eagle\modules\listing\models\BonanzaProductList;

class BonanzaOfferSyncHelper{
	 ...
	 *
	 +---------------------------------------------------------------------------------------------
	 **/
	public static function GetOfferList($bonanzaAccount, $ProductEanList=[]){
		$timeout=600; //s
		$retInfo=[];
	...
	public static function webSiteInfoToDb($uid, $seller, $prodcutInfo, $is_base64=true){
		...
		try{
			...
			if(!empty($prodcutInfo['product_url']) && stripos($prodcutInfo['product_url'], 'http://www.bonanza.com')===false)
				$prodcutInfo['product_url'] = 'http://www.bonanza.com'.$prodcutInfo['product_url'];
			/*
			...
			*/
			...
			if(empty($prodcutInfo['sku']) && !empty($prodcutInfo['itemID'])){
				...
				$item->product_name = empty($prodcutInfo['title'])?'':$prodcutInfo['title'];
			...
			//商品有SKU的情况:
// 			$listing = BonanzaProductList::find()->where(['partnumber'=>$prodcutInfo['sku'],'seller_id'=>$seller ])->orderBy('id asc')->one();
// 			if(!empty($listing->id)){
// 				//删除重复的listing
// 				$command = Yii::$app->subdb->createCommand ( "DELETE FROM `bonanza_product_list` WHERE `partnumber`=:partnumber AND `seller_id`=:seller_id AND `id`<>:id" );
...
// 			}
	...
	public static function syncProdInfoWhenGetOrderDetail($uid,$seller,$product_sku=[],$itemid=[], $priority=2,$type='sku'){
		...
		try{
			$field_list=array('product_id','img','url','title');
			$site = '';
			$callback = 'eagle\modules\listing\helpers\BonanzaOfferSyncHelper::webSiteInfoToDb($puid,$seller,$prodcutInfo);';
			$rtn = HtmlCatcherHelper::requestCatchHtml($uid,$product_key,'bonanza',$field_list,$site,$callback,$falg=true,$priority,['store_name'=>$seller,'key_type'=>$type]);
