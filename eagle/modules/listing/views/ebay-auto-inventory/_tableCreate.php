    ...
    <table id="create" class="table_listing">
        ...
        <thead>
            <tr>
                ...
                <th style="width:2%">subsku</th>
                <th style="width:2%">属性值</th>
                <th style="width:1.5%">价格</th>
                <th style="width:1%">库存</th>
                <th style="width:1%">售出</th>
                <th>时间</th>
                <!-- <th>补库数量</th> -->
                <th>操作</th>
            </tr>
            ...
            <tr name="<?=$mkey?>">
                ...
                <td style="min-width:90px;max-width:250px;text-align:left;vertical-align:top;">
                    ...
                    <p class="m0_ebay"><!-- itemid -->
                        <a target="_blank" href=<?php echo $mval['viewitemurl']?>><?php echo $mval['itemid']; ?></a>
                    </p>
                    <p class="e_color e_m0"><!-- 卖家平台 -->
                        <?php echo '「'.$mval["selleruserid"].'」「'.$mval['site'].'」'?>
                    </p>
                </td>
                <!--col4-sku  -->
                ...
                <!--col5-variation info  -->
                <td colspan="6" style="vertical-align:top;padding:0;min-width:400px;">
                    <table class="subtable_var" style="border:0px">
                        <tbody>
                        <?php if ($mval['isvariation']): ?>
                        <!-- 遍历输出多属性 -->
                        <?php foreach ($details as $dkey => $dval): ?>
                            <?php if ($mval['itemid']==$dval['itemid']): ?>
                                <!-- <?php print_r($dval['variation'],0) ?>  -->
                                <?php foreach ($dval['variation'] as $key => $val): ?>
                                    <tr style="vertical-align: top;">
                                        <td style="width:5%">
                                            <input id="lv2_create<?=$key?>" type="checkbox" class="ck_sub" name="var<?=$mkey?>" value="<?=$key?>" onclick='autoInventory.selectlvTwo(this)'>
                                        </td>
                                        <td style="width:20%"><!-- 输出subsku -->
                                            <span class="e_sku" style="display:inline-block;">
                                                <?php echo isset($val['SKU'])?$val['SKU']:NULL ?>
                                            </span>
                                        </td>
                                        <td style="width:20%"><!-- 输出name value -->
                                            <span class="e_sku" style="display:inline-block;">
                        ...
                        <?php else: ?>
                            ...
                            <td style="width:15%"><!-- 输出价格 -->
                                <?php  echo $mval['currentprice'];  ?>
                            </td>
                            <td style="width:10%"><!-- 输出库存 -->
                                <?php  echo $mval['quantity'];  ?>
                            </td>
                            <td style="width:10%"><!-- 输出售出数量 -->
                                <?php  echo $mval['quantitysold'];  ?>
                            </td>
                        <?php endif ?>
                        </tbody>
                    </table>
                ...
                <!--col6-time-->
                <td style="width:110px;min-width:110px;text-align:left;box-sizing:content-box;">
                    <p class="e_m0 e_color">起始</p>
                    <p class="e_m0"><?php echo date('Y-m-d H:i:s',$mval['starttime']); ?></p>
                    <p class="e_m0 e_color">结束</p>
                    <p class="e_m0"><?php echo date('Y-m-d H:i:s',$mval['endtime']); ?></p>
                </td>
                <!--col7-action-->
                <td>
                    <a onclick="autoInventory.created_setting(<?=$mval['itemid']?>)">
                    <span class="egicon-edit" title="设置自动补库">
                    </span>
                    </a>
                </td>
