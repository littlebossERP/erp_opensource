...
<?php for($i=0;$i<4;$i++):?>
<div class="whole-onebox shipping" id="shippingservice_<?php echo $i ?>" <?php if ($i>0&&strlen(@$data['shippingdetails']['ShippingServiceOptions'][$i]['ShippingService'])==0){echo 'style="display:none"';}?>>
<?php if ($i>0):?>
...
<th>运输方式</th>
	...
	<?php echo Html::dropDownList('shippingdetails[ShippingServiceOptions]['.$i.'][ShippingService]',
			isset($data['shippingdetails']['ShippingServiceOptions'][$i]['ShippingService'])?$data['shippingdetails']['ShippingServiceOptions'][$i]['ShippingService']:'',$shippingservice,['class'=>'iv-input fixed-boxsize'])?>
	</td>
</tr>
<tr>
<th>主运费</th>
