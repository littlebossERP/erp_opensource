...
<?php 
	...
	//单选未必填
		...
		echo Html::dropDownList("specific[$specific->name]", $specificvalname, array_combine($spzhal,$spzhal),array('prompt'=>'','class'=>'iv-myinput'));
	//文本框
	}elseif ($specific->selectionmode == 'FreeText' && count($specific->value) ==0){
		echo Html::textInput("specific[$specific->name]",$specificvalname);		
	//combo 暂时只做成下拉选择
...
<?php 
		...
		echo Html::checkboxList("specific[$specific->name]", $specificvalname,$spzhal);	
	}
