...
?>
<style>
.jumia-listing-reference .modal-body {
	max-height: 500px;
	overflow-y: auto;
}

.jumia-listing-reference .modal-dialog {
	width: 830px;
}
...
<div>
	...
	<?=Html::dropDownList('lazada_uid',@$_REQUEST['lazada_uid'],$jumiaUsersDropdownList,['onchange'=>"jumiaListing.queryReferences(this);",'class'=>'eagle-form-control','id'=>'','style'=>'width:130px;','prompt'=>'全部jumia店铺'])?>
	...
	<div style="width:100%;float:left;">
		<FORM id="references-form" name="references-form" action="/listing/jumia-listing/use-reference">
		...
		<tr>
	        ...
	        <?php foreach($references['data'] as $row):?>
	        	<tr>
					...
					<td><?=$jumiaUsersDropdownList[$row['lazada_uid_id']] ?></td>
					<td><?=$row['Name'] ?></td>
					<td><?=$row['ParentSku'] ?></td>
					<td><?=$row['SellerSku'] ?> </td>
				</tr>
