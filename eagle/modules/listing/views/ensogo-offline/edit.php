<?php
	...
	$this->registerJs("Ensogo.fillVarianceData();",\yii\web\View::POS_READY);
?>
...
<div class="panel panel-default" style="border:0">
	<div class="panel-body col-xs-11 mbottom20">
		<form id="fanben-capture" name="fanben_capture" method="post"  class="block">
			<input type="hidden" name="ensogo_id" data-type="offline" value="<?=$EnsogoModel['id']?>"/>
			...
			<div class="col-xs-10 ">
				<div class="form-group">
					<div class="row mTop20 ensogo_category">
						...
						<?php if(empty($EnsogoModel['category_id']) || empty($EnsogoModel['category_info'])): ?>
				...
				<div class="form-group">
					<div class="row ensogo_website_list">
						...
						<div id="ensogo_website" class="col-xs-8">
							<span style="margin-right:10px;"><input type="checkbox" name="all_sites" value="all" style="margin-top:3px;margin-left:0px;"  <?php if(count($sel_sites) == count($sites)):?>checked="checked"<?php endif;?>/>全部</span>
							<?php foreach($sites as $k_s => $site): ?>
								<span style="margin-right:10px;"><input type="checkbox" name="sites" data-val="<?=$site?>" data-key="<?=$k_s?>" <?php if(in_array($k_s,$sel_sites)):?> checked="checked" <?php endif;?>/><?=$site?></span>
							<?php endforeach; ?>
						</div>
					</div>
				</div>
			...
			<div class="col-xs-10 ">
				...
				<div class="form-group">	
					<div class="row mTop10">
						...
						<div class="col-xs-4">
							<input class="col-xs-10 form-control" id="ensogo_product_parentSku" type="text" name="parent_sku" value="<?=$EnsogoModel['parent_sku']?>">
						</div>
