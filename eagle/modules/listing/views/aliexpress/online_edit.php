<?php 
	...
	$this->registerJs("$.aliexpress.skuAttr=".$productInfo->onlineDetail->aeopAeProductSKUs.";",\yii\web\View::POS_READY);
	$this->registerJs("$.aliexpress.productPropertys=".$productInfo->onlineDetail->aeopAeProductPropertys.";",\yii\web\View::POS_READY);
	$this->registerJs("$.aliexpress.initAliexpressOnlineEdit();",\yii\web\View::POS_READY);
?>
<?php 
	...
	<form>
		...
		<div class="selleruser_box left-spacing-m">
			...
			<div class="form-group">
				...
				<div class="input-spacing">
					<select class="iv-input select-width" name="categoryid" class="categoryid">
						<option value="">---请选择分类---</option>
						<?php if(isset($productInfo->onlineDetail->categoryName)): ?>
							<option value="<?=$productInfo->onlineDetail->categoryName['cateid']?>" selected><?=$productInfo->onlineDetail->categoryName['name_zh']?></option>
						<?php endif;?>
					</select>
					<a class="iv-btn btn-success left-spacing-s " target="#category-modal" title="选择类目">选择类目</a>
					<p class="input-spacing" style="color:#933;"><?=$productInfo->onlineDetail->categoryName['nameZh']?></p>
				</div>
			</div>
		</div>
		<?php 
			$str = '';
			foreach($productInfo->onlineDetail->categoryAttr as $k => $v):
				if($v['sku'] == 0):
		...
		<div class="product_info_box">
			...
			<div class="form-group left-spacing-m input-spacing">
				...
				<div class="product_group_list_box left-spacing-l" style="display:inline-block;">
						<?php if(empty($productInfo->onlineDetail->groupName)): ?>
							<span>未选择分组</span>
						<?php else: ?>
							<span data-id="<?=$productInfo->onlineDetail->product_groups?>"><?=$productInfo->onlineDetail->groupName;?></span>
						<?php endif;?>
			...
			<div class="form-group left-spacing-m input-spacing">
				<div class="sale_unit rows">
					...
					<div class="input-spacing">
						<select class="iv-input max-width" name="product_unit" class="product_unit" placeholder="件/个(piece/pieces)">
							<?php foreach($productInfo->onlineDetail->productUnit as $k => $unit): ?>
								<option value="<?=$unit['id']?>" data-name_zh="<?=$unit['zh']?>" data-name_en="<?=$unit['en']?>" <?php if($unit['id'] == $productInfo->onlineDetail->product_unit):?>selected<?php endif;?>><?=$unit['zh']?><?=$unit['en']?></option>
							<?php endforeach;?>
						</select>
					</div>
				</div>
				<div class="rows sale_ways input-spacing" style="font:12px/20px Microsoft Yahei;">
					...
					<div class="input-spacing">
						...
						<?php if($productInfo->onlineDetail->package_type == 0): ?>checked<?php endif;?>>
						...
						<?php if($productInfo->onlineDetail->package_type == 1): ?>checked<?php endif;?>>
						<span>打包出售</span>
						<label show-content="lot_num" class="left-spacing-l" style="<?php if($productInfo->onlineDetail->package_type == 0): ?>display:none<?php endif;?>">每包 
							<input class="iv-input" name="lot_num" value="<?=$productInfo->onlineDetail->lot_num?>" check-valid="number">
				...
				<div class="rows ">
					...
					<div class="input-spacing">
						<input class="iv-radio" type="radio" name="reduce_strategy" value="2" <?php if($productInfo->onlineDetail->reduce_strategy == 2): ?>checked<?php endif;?>>付款减库存
						<input class="iv-radio left-spacing-l" type="radio" name="reduce_strategy" value="1" <?php if($productInfo->onlineDetail->reduce_strategy == 1): ?>checked<?php endif;?>>
						下单减库存
					</div>
				</div>
				<div class="rows">
					...
					<div class="input-spacing">	
						<input class="iv-input max-width" type="text" name="delivery_time"value="<?=$productInfo->onlineDetail->delivery_time?>" check-valid="day"> 天
		...
		<div class="variance_info_box">
			<div class="form-group">
				...
				<?php if($productInfo->onlineDetail->is_bulk == 0):
						$is_bulk_1 = 'checked';
						$is_bulk_2 = '';
						...
						$bulk_discount = '--';
					  	...
					  	$bulk_discount = $productInfo->onlineDetail->bulk_discount; 
				...
				<div class="rows input-spacing">
					...
					<div class="input-spacing">
						...
						<p class="" show-content="bulk_detail" style="<?=$bulk_display?>">购买数量 <input class="iv-input short-input" type="text" name="bulk_order" value="<?=$productInfo->onlineDetail->bulk_order?>" check-valid="number"> 件及以上时，每件价格在零售价的基础上减免 
							<input class="iv-input short-input" type="text" name="bulk_discount" value="<?=$productInfo->onlineDetail->bulk_discount?>" check-valid="discount"> %,即
		...
		<div class="product_describe_box">
			<textarea class="ali-editor"><?=$productInfo->onlineDetail->detailInfo?></textarea>
		</div>
		<div class="ali-title">
		...
		<div class="logistics_service_info_box">
			<div class="form-group">
				<div class="rows">
					...
					<label>
						<input type="text" name="product_gross_weight" class="iv-input" value="<?=$productInfo->onlineDetail->product_gross_weight?>" check-valid="weight">&nbsp;公斤/<span class="unit">件</span>
						<p class="error-tips input-spacing product_gross_weight_tips" style="display:none;width:180px;"></p>
					</label>
					<?php if(!empty($productInfo->onlineDetail->isPackSell)):
						   $checked = 'checked';
					...
					<div class="input-spacing" style="margin-left:110px;">
						...
						买家购买 <input type="text" name="baseUnit" class="iv-input short-input" value="<?=$productInfo->onlineDetail->baseUnit?>" check-valid="number"> <span class="product_unit">件/个</span> 以内，按单件产品重量计算运费。	
						</p>
						<p show-sign="pack_sell" class="input-spacing" style="<?=$display?>">
						在此基础上，买家每多买 <input type="text" name="addUnit" class="iv-input short-input" value="<?=$productInfo->onlineDetail->addUnit?>" check-valid="number"> <span class="product_unit">件/个</span>，重量增加 <input type="text" name="addWeight" class="iv-input short-input" value="<?=$productInfo->onlineDetail->addWeight?>" check-valid="weight"> kg。
						</p>
					</div>
				</div>
				<div class="rows">
					...
					<label>
						<input type="text" class="iv-input short-input" name="product_length" placeholder="长（cm）" value="<?=$productInfo->onlineDetail->product_length?>" check-valid="size">
						&nbsp;x&nbsp;
						<input type="text" class="iv-input short-input" name="product_width" placeholder="宽（cm）"
						 value="<?=$productInfo->onlineDetail->product_width?>" check-valid="size">
						&nbsp;x&nbsp;
						<input type="text" class="iv-input short-input" name="product_height" placeholder="高（cm）" value="<?=$productInfo->onlineDetail->product_height?>" check-valid="size">
					</label>
				</div>
				<div class="rows input-spacing">
				...
				<div class="rows input-spacing">
					...
					<select class="iv-select" name="promise_templateid" style="min-width:170px;" data-id="<?=$productInfo->onlineDetail->promise_templateid?>">
		...
		<div class="rest_info_box">
			<label class="text-left">产品有效期</label>	
			<label>
				<input class="iv-radio left-spacing-l" type="radio" name="wsValidNum" value="14" <?php if($productInfo->onlineDetail->wsValidNum == 14) echo 'checked'; ?>>14天
			</label>
			<label>
				<input class="iv-radio left-spacing-l" type="radio" name="wsValidNum" value="30" <?php if($productInfo->onlineDetail->wsValidNum == 30) echo 'checked'; ?>>30天
			</label>
		</div>
	</form>
</div>
<div class="bottom_btn">
	<div>
		<a class="btn btn-success aliexpress_OnlineSave" data-productid="<?=$productInfo->productid?>">保存</a>
		<a class="btn btn-success" href="/listing/aliexpress/online-list?product_status=3">取消</a>
	</div>
</div>
