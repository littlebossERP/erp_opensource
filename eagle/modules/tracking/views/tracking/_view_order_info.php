		...
		<div class="panel-order-body">
			<div class="row row-nomargin">
				<div class="col-md-12 text-nowrap">
					<label style="font-size: 14px;"><?= TranslateHelper::t('物流号')?>:&nbsp;&nbsp;&nbsp;<?= (!empty($orderData['track_no'])?$orderData['track_no']:"")?></label>
				</div>
			</div>
		</div>
	</div>
		...
		<div class="panel-order-heading"><?= TranslateHelper::t('订单基本信息')?></div>
			<table class="table" style="">
				<tr>
					...
					<td width="73px"><?= TranslateHelper::t('付款日期/发货日期')?>:</td>
					<td width="360px">
						<small>
							<?= (!empty($orderData['paid_time'])?date("Y-m-d H:i:s",$orderData['paid_time']):"")?>
							/
							<?php $delivery_time = ''?>
							<?php if(!empty($orderData['delivery_time'])) $delivery_time=$orderData['delivery_time']; ?>
							<?php if(empty($delivery_time) && !empty($orderData['order_ship_time'])) $delivery_time=strtotime($orderData['order_ship_time']); ?>
							<?=(empty($delivery_time))?'--':date("Y-m-d",$delivery_time) ?>
						</small>
					</td>
				</tr>
				<tr>
