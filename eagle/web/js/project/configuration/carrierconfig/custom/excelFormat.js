...
//			alert(title2+','+type2+','+value2);
		...
		saveFormat:function(){
			if(excelFormat.lines <= 0){
				...
				var Url=global.baseUrl +'configuration/carrierconfig/save-excel-format';
				$.ajax({
			      type : 'post',
			      cache : 'false',
			      data : $form,
			    	  ...
			    	  if(response[0] == 0){
			        		...
			        		if(undefined == $('#search_tab_active').val()){
			        			...
			        			if(($('#search_tab_active').val() == 'customexcel') || ($('#search_tab_active').val() == 'customtracking')){
			        				...
			        				window.location.search='tab_active='+$('#search_tab_active').val()+'&twarehouseid='+$('#search_warehouse_id').val();
			        			}
			        		}
				        }
			        	else{
		...
		init:function($excel_mode,$excel_format,$array_Data){
			...
			$('#btn_import_product').click(function(){
				if (typeof(importFile) != 'undefined'){
					form_url = "/configuration/carrierconfig/excelupload";
					template_url = "/template/excel导入格式模版.xls";
