$(function() {
	...
	$("#sizeSel button").on('click', function() {
		...
		var t=$(this).text();
		if(t == '标准规格'){
			var size = $(this).val().split($('select[name=template_size]').val());
		...
		success:function(response){
			if(response){
				bootbox.alert({
					...
					callback:function(){
						if(version == 'new')
							location.href = '/configuration/carrierconfig/carrier-custom-label-list-new?tab_active=sys';
						else
							location.href = '/configuration/carrierconfig/carrier-custom-label-list?tab_active=sys';
					}
				});
...
function copytemplateToself_cus(id,name){
	if(id == '' || name == '' || id == undefined || name == undefined)return false;
	$('#template_id_cus').val(id);
	$('#copyToNewTemplate_cus input[name="template_name_cus"]').val('自定义:' + name);
}

function docopy_cus(version){
	var id = $('#template_id_cus').val(),
		name = $('#copyToNewTemplate_cus input[name="template_name_cus"]').val();
	if(id == '' || name == ''){
		bootbox.alert('复制失败');
	}
	var data = $('#copyToNewTemplate_cus form').serialize();

	$.ajax({
		url:'copytemplate-to-custom-cus',
		type:'post',
		data:data,
		success:function(response){
			if(response){
				bootbox.alert({
					...
					callback:function(){
						if(version == 'new')
							location.href = '/configuration/carrierconfig/carrier-custom-label-list-new?tab_active=self';
						else
							location.href = '/configuration/carrierconfig/carrier-custom-label-list?tab_active=self';
					}
				});
				$('#copyToNewTemplate_cus').modal('hide');
			}
			else bootbox.alert('复制失败,请检查模版名是否重复');
		}
	})
