...
productList.list={
		addTagHtml:function(obj){
			...
			//alert($(obj).text());
			var addContent = "<div class=\"form-group\" id=\"new_form_group\" style=\"float:left;width:100px;vertical-align:middle;margin:0px;\">"+
    ...
    openKeywordRule:function()
    {
        /**    前后缀关键字规则-点击开启：编辑->显示
         */

        $("#td_keyword_rule").css('display','table-cell');
        //$("#expired_time").removeAttr("disabled");
        //$('#expired_time').val("");
    },
    closeKeywordRule:function()
    {
        /**    前后缀关键字规则-点击关闭：编辑->隐藏 ;
         *     清空text值。
         */
        $("#td_keyword_rule").css('display','none');
        //$("#expired_time").removeAttr("disabled");
        $('#Product_tag').val("");
        $("#new_form_group").empty();
        //$("#Product_tag").attr("value","");
    },
    openSubstringRule:function()
    {
        /**    SKU截取规则-点击开启：编辑->显示
         */

        $("#td_substring_rule").css('display','table-cell');
        //$("#expired_time").removeAttr("disabled");
        //$('#expired_time').val("");
    },
    closeSubstringRule:function()
    {
        /**    SKU截取规则-点击关闭：编辑->隐藏
         *    清空text值。
         */
        $("#td_substring_rule").css('display','none');
        //$("#expired_time").removeAttr("disabled");
        $('#firstChar').val("");
        $('#secondChar').val("");
    },
    openSplitRule:function()
    {
        /**    拆分规则-点击开启：编辑->显示
         */

        $("#td_split_rule").css('display','table-cell');
        $('#quantityConnector').val("*");
        $('#skuConnector').val("+");
    },
    closeSplitRule:function()
    {
        /**    拆分规则-点击关闭：编辑->隐藏
         */
        $("#td_split_rule").css('display','none');
        ////$("#expired_time").removeAttr("disabled");
        $('#quantityConnector').val("");
        $('#skuConnector').val("");
    },
    testSkuRule:function(){
        $.showLoading();
        var values = $('#skuruleform').serialize();
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: '/configuration/productconfig/test-sku-rule',
            data: values ,
            success: function (result) {
                if (result.success){
                    $.hideLoading();
                    ...
                    //        }
                    //    },
                    //    message: Translator.t("成功！"),
                    //    callback: function() {
                    //        window.location.reload();
                    //    },
                    //});
                    //alert('22');
                    //    alert(result.dataFinal);

                    $('#sku_later').text(result.dataFinal);
                }
                else{
                    $.hideLoading();
                    $('#sku_later').text("");
