...
StationLetter = {
	...
	sendMessageByRole:function(callback){
		...
		*/
		if (no_send_html != ''){
			bootbox.confirm(no_send_html+'\n'+Translator.t("以上订单号将不发送!是否继续?"),function(r){
				if ( r){
	...
	*/
		...
		$.ajax({
			...
			success: function (result) {
				...
				thisHtml +='</div></div></div>'+'</div>'+
					...
					'<div class="modal-footer">'+orderNoHtml+'<button data-bb-handler="Ok" type="button" class="btn btn-transparent">关闭</button></div>'
	...
	batchShowMessageBox:function(){
		...
		if (selected_order_no_list==''){
			bootbox.alert(Translator.t("请选择需要发信的订单"));
			return;
		}
		...
		$.get('/order/od-lt-message/station-letter-detail?order_id='+selected_order_id_list+'&order_no='+selected_order_no_list+'&pos='+pos,
		   function (data){
				$.hideLoading();
				if (data == 'empty'){
					bootbox.alert('选中的订单没有对应有效的订单信息, 不能发信!');
					return false;
				}
				var thisbox = bootbox.dialog({
					title: Translator.t("发送站内信")+'<small style="color:red">'+Translator.t("目前只支持速卖通以及eBay的发信")+"</small>",
					className: "xlbox", 
					message: data,
					buttons:{
						Ok: {  
	...
	showMessageBox:function(order_id,order_no,track_no,show_method,count){
		...
		$.get('/order/od-lt-message/station-letter-detail?order_id='+order_id+'&track_no='+track_no+'&order_no='+order_no+'&show_method='+show_method+'&pos='+pos,
		   function (data){
				$.hideLoading();
				if (data == 'empty'){
					bootbox.alert('选中的订单号没有对应有效的订单信息, 不能发信!');
					return false;
				}
				var thisbox = bootbox.dialog({
					title: Translator.t("发信通知"),
					className: "xlbox", 
					message: data,
					buttons:{
						Btn2:{
	...
	setServiceDeliveryDays : function (method_code){
		if(method_code==undefined || method_code==''){
			bootbox.alert(Translator.t('错误操作!'));
			return false;
		}
		$.showLoading();
		$.ajax({
