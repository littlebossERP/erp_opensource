$.domReady(function($el){
    var $doc = this,
        sync = function(site_id){
            ...
            $el('.iv-progress').registerPlugin('Progress',function(Progress){
                ...
                // 开始同步
                $.get('/order/priceminister-order/get-queue',{
                    site_id:site_id,
                    ...
                    progress.url = '/order/priceminister-order/get-progress';
                    progress.params = {
                        site_id:site_id,
    ...
    $el(".sync-start").on('click',function(){
		if($el("#pm_site_id").val()=='' || $el("#pm_site_id").val()==0){
			bootbox.alert(Translator.t('请选择店铺'));
			return;
		}
		
        sync($el("#pm_site_id").val());
    });
    $el("#pm_site_id").on('change',function(){
        $el(".sync-done").hide();
        $el(".sync-cancel").hide();
        $el(".sync-start").show();
    });
