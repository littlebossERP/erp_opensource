...
(function($) {
	$.prompt = function(message, options) {
        ...
        $.prompt.bodyMargin = documentHeight > windowHeight || $('body').css('overflow-y') === 'scroll' ? $('body').css('margin-right') : false;
        $body.addClass("impromptu-lock");
        if ($.prompt.bodyMargin !== false) {
            $('body').css('margin-right', $.prompt.scrollbarWidth());
        }
	...
	$.prompt.position = function(e){
		...
		// standard prompt positioning
			...
			$.prompt.jqi.css({
                width: $.prompt.options.outerBoxWidth
            });
			$.prompt.jqi.css({
				position: "fixed",
				top: $.prompt.options.top,
	...
	$.prompt.goToState = function(state, callback) {
		...
		if(!promptstatechanginge.isDefaultPrevented()){
			...
			$('.'+ $.prompt.currentPrefix +'_state').slideUp('fast')
				.find('.'+ $.prompt.currentPrefix +'arrow').fadeOut();
			
			$('#'+ $.prompt.currentPrefix +'_state_'+ state).slideDown('fast',function(){
				var $t = $(this);
				$t.find('.'+ $.prompt.currentPrefix +'defaultbutton').focus();
				$t.find('.'+ $.prompt.currentPrefix +'arrow').fadeIn('fast');
	...
	$.prompt.close = function(callCallback, clicked, msg, formvals){
		$.prompt.jqib.fadeOut('fast',function(){
			...
			$.prompt.jqib.remove();
			$("body").removeClass("impromptu-lock");
            if ($.prompt.bodyMargin !== false) {
                $('body').css('margin-right', $.prompt.bodyMargin);
            }
    ...
    $.prompt.scrollbarWidth = function() {
