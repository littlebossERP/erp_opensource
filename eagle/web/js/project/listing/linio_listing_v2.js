			...
			if($("#copyType").val() == "singleCopy"){//单产品初始品牌
				$('[cid="Brand"]>.secondTd>input').autocomplete({
					source: function(request,response){
						$.ajax({
						   type: "post",
						   url: "/listing/linio-listing-v2/get-brands",
						...
						'<table>'+
							'<tbody>'+
								...
								'<td class="firstTd"><span class="fRed">*</span>运输天数:</td>'+
									'<td class="secondTd">'+
										'<input cid="MinDeliveryTime" ismust="1" type="text" id=""name="最小天数" value="" placeholder="最小天数" onkeyup="lazadaListing.replaceNumber(this);">'+
										'&nbsp;&nbsp;-&nbsp;&nbsp;<input cid="MaxDeliveryTime" ismust="1" type="text" id="" name="最大天数" value="" placeholder="最大天数" onkeyup="lazadaListing.replaceNumber(this);">'+
									'</td>'+
								'</tr>'+
								'<tr>'+
									'<td colspan="2" class="secondTd">'+
...
//				$('#edit_input').val('');
		...
		batchEditSubmit:function(){
			...
			if(type == ""){
				...
				switch(type){
					case 'price':
						post_url = "/listing/linio-v2/batch-update-price";
						break;
					case 'sale_message':
						post_url = "/listing/linio-v2/batch-update-sales-info";
						break;
					case 'quantity':
						post_url = "/listing/linio-v2/batch-update-quantity";
						break;
				}
			}
			...
			if($(obj).val()=="1"){
				...
				if(ids.length>0){
					$.ajax({
						type:'GET',
						async:false,
						url:'/listing/linio-listing-v2/do-publish',
						data:{
							ids:ids.join(',')
						},
						dataType:'json',
			...
			if($(obj).val()=="2"){//批量删除
				...
				if(ids.length>0){
					$.ajax({
						type:'POST',
						async:false,
						url:'/listing/linio-listing-v2/delete',
						data:{
							ids:ids.join(',')
						},
						dataType:'json',
...
//						$.fn.message({type:"error",msg:"网络错误, 请稍后再试！"});
		...
		SyncSubmit:function(){
			...
			if(lzd_uid == ""){
				...
				$.ajax({
					type:'GET',
					url:'/listing/linio-v2/manual-sync',
...
//					linioListing.pageBeginning();
			...
			$.ajax({
				type:'POST',
				async:false,
				url:'/listing/linio-listing-v2/get-selected-category-history',
				data:{
					lazada_uid:shopId
				},
				dataType:'json',
...
//			});				
		...
		selectHistoryCategory:function(obj){
			...
			if($.trim(primaryCategory) == ""){
				...
				$.ajax({
					type:'POST',
					async:false,
					url:'/listing/linio-listing-v2/get-all-categoryids',
					data:{
		...
		mutinitEditProduct:function(categoryId,shop_id,type){//type防止多站点复制的时候，数据回填
			...
			$.ajax({
				type:'POST',
				async:false,
				url:'/listing/linio-listing-v2/get-category-attrs',
				data:{
					lazada_uid:shop_id,
		...
		initCategory:function(categoryList,shop_id){
			...
			$.ajax({
				type:'POST',
				async:false,
				url:'/listing/linio-listing-v2/get-category-tree',
				data:{
...
//			debugger;
			...
			$.ajax({
				type:'POST',
				url: "/listing/linio-listing-v2/save-muti-site",
				data:{
//					id:productId,
...
//							$("#msgModal").modal("show");
							bootbox.alert({title:Translator.t('提示'),message:data.message,callback:function(){
									window.location.href="/listing/linio-listing-v2/publish";
									$.showLoading();
								}
						   });
						}
...
//				bootbox.alert("没有设置主图片！");
			...
			$.ajax({
				type:'POST',
				url: "/listing/linio-listing-v2/save-product",
				data:{
					id:productId,
					categories:categories,
					lazada_uid:shopId,
...
//				$.fn.message({type:"error", msg:"保存失败！促销结束时间不能小于促销开始时间！"});
		...
		listReferences:function(){
			...
			$.ajax({
				type:'get',
				url: "/listing/linio-listing-v2/list-references",
				timeout:60000,
				success:function(data){
					$.hideLoading();
					if(data != null){
		...
		userReference:function(){
			...
			$.showLoading();
			window.location.href="/listing/linio-listing-v2/use-reference?listing_id="+listingId;
		},
